...
static rspfTrace traceDebug(rspfString("rspfGdalTiledDataset:debug"));
                                       ...
                                       void * pImage )
{
   ...
   rspfIrect bufferRect = theDataset->theData->getImageRectangle();
   rspfIpt ul(theDataset->theAreaOfInterest.ul().x + nBlockXOff*nBlockXSize,
               theDataset->theAreaOfInterest.ul().y + nBlockYOff*nBlockYSize);
   rspfIrect requestRect(ul.x,
                          ul.y,
                          ul.x + nBlockXSize - 1,
                          ul.y + nBlockYSize - 1);
#if 0
   ...
   if ( (requestRect.completely_within(bufferRect) == false)  ||
        (theDataset->theData->getDataObjectStatus() == RSPF_EMPTY) )
   {
      loadBuffer = true;
   }
#if 0
   rspf_int32 scanlineTile = ((nBlockYOff*nBlockYSize)%theDataset->theTileSize.y);
   if (traceDebug())
   {
      rspfNotify(rspfNotifyLevel_DEBUG)
   ...
   if(loadBuffer)
   {
      ...
      rspf_uint32 numberOfTiles = (rspf_uint32)theDataset->theInterface->getNumberOfTilesHorizontal();
      for(rspf_uint32 i = 0; i < numberOfTiles; ++i)
      {
         rspfIpt tileOrigin(ul.x+theDataset->theTileSize.x*i, ul.y);
#if 0
         if (traceDebug())
         {
            rspfNotify(rspfNotifyLevel_DEBUG)
   ...
   const rspf_int32 BPP = static_cast<rspf_int32>(
      theDataset->theData->getScalarSizeInBytes());
   if(theDataset->theData->getDataObjectStatus() == RSPF_EMPTY)
   {
   ...
   else 
   {
      ...
      {
         rspf_uint32 offset = (ul.y - bufferRect.ul().y) *
...
MEMTiledDataset::MEMTiledDataset(rspfImageSourceSequencer* iface)
   :
   MEMDataset(),
   theData(),
   theInterface(iface),
...
/*                               Create()                               */
/************************************************************************/
void MEMTiledDataset::create(rspfImageSourceSequencer* iface)
{
   if (traceDebug())
   {
      rspfNotify(rspfNotifyLevel_DEBUG)
   ...
   if(theInterface)
   {
      ...
      theData = rspfImageDataFactory::instance()->create(NULL, // no owner specified
                                                          theInterface);
      
      GDALDataType gdalType = rspfGdalType().toGdal(theInterface->getOutputScalarType());
      ...
      rspfIrect bounds = theInterface->getBoundingRect();
      theTileSize = rspfIpt(theInterface->getTileWidth(),
...
void GDALRegister_MEMTiled()
{
    ...
    if( poMEMTiledDriver == NULL )
    {
        ...
        poDriver->SetMetadataItem( GDAL_DMD_LONGNAME, 
                                   "In Memory Raster RSPF tile bridge" );
   ...
   if (theDataset && pImage)
   {
      if (theDataset->theData.valid())
      {
         switch (theDataset->theData->getScalarType())
         {
            case RSPF_UINT8:
            {
               return copyNulls(pImage, count, rspf_uint8(0));
            }
            case RSPF_SINT8:
            {
               return copyNulls(pImage, count, rspf_sint8(0));
            }
            
            case RSPF_UINT16:
            case RSPF_USHORT11:
            {
               return copyNulls(pImage, count, rspf_uint16(0));
            }  
            case RSPF_SINT16:
            {
               return copyNulls(pImage, count, rspf_sint16(0));
            }
            
            case RSPF_UINT32:
            {
               return copyNulls(pImage, count, rspf_uint32(0));
            }  
            case RSPF_SINT32:
            {
               return copyNulls(pImage, count, rspf_sint32(0));
            }  
            case RSPF_FLOAT32:
            case RSPF_NORMALIZED_FLOAT:
            {
               return copyNulls(pImage, count, rspf_float32(0.0));
            }
            
            case RSPF_NORMALIZED_DOUBLE:
            case RSPF_FLOAT64:
            {
               return copyNulls(pImage, count, rspf_float64(0.0));
            }  
