...
rspfRunningSum::rspfRunningSum(int slowdim, int fastdim, const double* table) :
    theSd(slowdim), 
    theFd(fastdim),
...
rspfRunningSum::~rspfRunningSum()
{
}
   
double
rspfRunningSum::getRS(int p1, int p2)const
{
   //boundary correction
   if ((p1<0) || (p2<0))
   { 
...
double
rspfRunningSum::getRS2(int p1, int p2)const
{
   //boundary correction
   if ((p1<0) || (p2<0))
   { 
...
double
rspfRunningSum::getAverage()const
{
   return rs[theSd*theFd-1] * theNormFact;
}

double
rspfRunningSum::getStd()const
{
