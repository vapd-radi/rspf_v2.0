...
RTTI_DEF1(rspfOpenCVDilateFilter, "rspfOpenCVDilateFilter", rspfImageSourceFilter)

rspfOpenCVDilateFilter::rspfOpenCVDilateFilter(rspfObject* owner)
...
rspfOpenCVDilateFilter::~rspfOpenCVDilateFilter()
{
}

rspfRefPtr<rspfImageData> rspfOpenCVDilateFilter::getTile(const rspfIrect& tileRect, rspf_uint32 resLevel)
{
	if(!isSourceEnabled())
   	{
...
void rspfOpenCVDilateFilter::initialize()
{
	 if(theInputConnection)
	  {
...
rspfScalarType rspfOpenCVDilateFilter::getOutputScalarType() const
{
	if(!isSourceEnabled())
	{
...
rspf_uint32 rspfOpenCVDilateFilter::getNumberOfOutputBands() const
{
	if(!isSourceEnabled())
	{
...
bool rspfOpenCVDilateFilter::saveState(rspfKeywordlist& kwl,
...
bool rspfOpenCVDilateFilter::loadState(const rspfKeywordlist& kwl,
										const char* prefix)
{
...
void rspfOpenCVDilateFilter::runUcharTransformation(rspfImageData* tile)
{   
  IplImage *input;
  IplImage *output;
...
*/
void rspfOpenCVDilateFilter::setProperty(rspfRefPtr<rspfProperty> property)
{
        if(!property) return;
...
rspfRefPtr<rspfProperty> rspfOpenCVDilateFilter::getProperty(const rspfString& name)const
{
        if(name == "iterations")
        {
...
void rspfOpenCVDilateFilter::getPropertyNames(std::vector<rspfString>& propertyNames)const
{
