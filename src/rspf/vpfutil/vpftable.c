...
static char *cpy_del(char *src, char delimiter, rspf_int32 *ind )
{
  rspf_int32 i, skipchar ;
  char *temp, *tempstr ;
...
/***********Mody B*********/
char *get_string(rspf_int32 *ind,char *src,char delimeter )
{ char *temp;
...
/**********Mody B*************/
char vpf_get_char(rspf_int32 *ind, char *src)
{  char temp;
...
/***********Mody B***********/
rspf_int32 get_number(rspf_int32 *ind, char *src,char delimeter)
{  char *temp;
   rspf_int32  num;
   temp  = cpy_del(&src[*ind],delimeter, ind);
 ...
 *    ddlen <input> == (rspf_int32) length of the table's data definition.
 *
...
rspf_int32 parse_data_def( vpf_table_type *table )
{
   ...
   if ( table->mode == Read ) {
	...
	swap_four((char *)&k,(char *)&ddlen);
     ...
     table->ddlen = ddlen + sizeof (rspf_int32) ;
   ...
   for(i=0;i<n;i++) {
     ...
     if ( i == 0 )
       if ( rspf_strcasecmp ( table->header[0].name, "ID" ) ) {
	      return (rspf_int32)0 ;
       }
     ...
     case 'I':
       if ( reclen >= 0 )
	 reclen += (sizeof(rspf_int32)*table->header[i].count);
...
#ifdef __MSDOS__
static int vpfhandler(rspf_int32 errval, rspf_int32 ax, rspf_int32 bp, rspf_int32 si)
{
   if (ax < 0) {
...
#ifdef __MSDOS__
   memsize = (rspf_int32)min(farcoreleft(),__64K);
#else
   memsize = MAXINT;
#endif
