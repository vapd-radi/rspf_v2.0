				  ...
				  char *end_table, vpf_table_type fcs )
{
   ...
   while (1) {
	 ...
	 return rlist;
      ...
      if (rspf_strcasecmp(tablename,end_table)==0) break;
   }
 ...
 *   Parameters:
 *A
 *    srchval <input> == (rspf_int32) specified search value.
 *    field   <input> == (int) table sort field.
 *    table   <input> == (vpf_table_type) VPF table.
 *    vpf_binary_search <output> == (rspf_int32) first matching row.
 *E
 *:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
 *
 *   History:
 ...
 *E
 *************************************************************************/
rspf_int32 vpf_binary_search( rspf_int32 srchval,
			    int field,
			    vpf_table_type table )
{
   rspf_int32 left,right, ival, rowid, n;
   row_type row;
 ...
 *E
 *************************************************************************/
rspf_int32 related_row( void *keyval1,
		      vpf_table_type table2, char *key2,
		      int sort_flag )
{
   rspf_int32 rowid, i, ival, kval, n;
   row_type row;
   int KEY2_;
   char cval, *tval;

   if (rspf_strcasecmp(key2,"ID")==0) {
		       ...
		       ThematicIndex *idx )
{
   ...
   set_type rowset;
   rspf_int32 rowid, i, ival, kval, n, start,end;
   row_type row = 0;
   int KEY2_;
   char cval, *tval;
 ...
 *E
 *************************************************************************/
rspf_int32 fc_row_number( row_type row, fcrel_type fcrel, rspf_int32 tile )
{
   row_type relrow;
   rspf_int32 count;
   rspf_int32 i, rownum, keyval;
   ...
   if (rspf_strcasecmp(rcell.key2,"ID")==0)
      rownum = keyval;
   else
      rownum = related_row(&keyval,fcrel.table[i],rcell.key2,0);
