 ...
 *				       char * query_str ) ;
 *
 *    rspf_int32 read_gazetteer_index_directory (
 *					ThematicIndexDirectory ** gid,
 *					ThematicIndexHeader     * gi,
 *					FILE * idx_fp);
 *
 *    rspf_int32 write_thematic_index_header ( ThematicIndexHeader h ,
 *					     FILE *fp ) ;
 *
 *    rspf_int32 read_thematic_index_header ( ThematicIndexHeader *h ,
 *					    FILE *fp ) ;
 *
 *    rspf_int32 write_thematic_index_directory (
 *				       ThematicIndexHeader h ,
 *				       ThematicIndexDirectory *d,
 *				       rspf_int32 size,
 *				       FILE *fp ) ;
 *
 *    rspf_int32 write_gazetteer_index_directory (
 *				       ThematicIndexHeader h ,
 *				       ThematicIndexDirectory *d,
 *				       rspf_int32 size,
 *				       FILE *fp ) ;
 *
 *    rspf_int32 create_thematic_index ( char indextype,
 *				       char *tablename,
 *				       char *idxname ,
 *				       char *columnname,
 *				       char *idx_set );
 *
 *    rspf_int32 create_gazetteer_index (char *tablename,
 *				       char *idx_fname ,
 *				       char *columnname,
 *				       char *idx_set);
 *
...
#include <rspf/vpfutil/vpftidx.h>
#include <string.h>
#include <stdlib.h>
#include <memory.h>
...
#define DIR_ON_DISK_INCR (2 * sizeof(rspf_int32))
				 ...
				 char *idx_set ) 
{
  ...
  if ( table.nrows > MAX_ID ) {
    h.id_data_type = 'I' ;		/* rspf_int32s */
    idsize = sizeof ( rspf_int32 ) ;
  } else {
    h.id_data_type = 'S' ;		/* rspf_int32s */
...
/* Fill up id sets */
  ...
  if ( write_thematic_index_header ( h, ifp ) == (rspf_int32)0 )
    Whimper ( "error writing index header" ) ;

  /* Now write out the rest of the header directory */

  if ( write_thematic_index_directory ( h, d, idsize, ifp ) == (rspf_int32)0 )
			       ...
			       char *value )
{
  ...
  if ( read_thematic_index_header ( &h, ifp ) == (rspf_int32)0 )
  ...
  if ( value ) 		/* search for pattern */
    ...
    case	'I':
      memcpy ( &ival, value, sizeof (rspf_int32)) ;
      break ;
    case	'S':
...
ThematicIndex open_thematic_index ( char *idxname )
{
  ...
  if ( read_thematic_index_header ( &idx.h, idx.fp ) == (rspf_int32)0 )
    OWhimper ( "error reading index header" ) ;

  if ( idx.h.index_type == 'G' ) {
    /* gazetteer_index  */
     if (read_gazetteer_index_directory(&idx.gid,&idx.h,idx.fp) == (rspf_int32)0) {
	   fclose(idx.fp);
	   idx.fp = NULL;
     }
  }
				 ...
				 char *idx_set)
{
  ...
  if (write_thematic_index_header(gi, idx_fp) == (rspf_int32)0) {
    fclose(idx_fp);
    for (i = 0; i < gi.nbins; i++)
...
set_type read_gazetteer_index (char *idx_fname, char *query_str )
{
  ...
  if (read_thematic_index_header (&gi, idx_fp) == (rspf_int32)0) {
    fclose(idx_fp);
    return query_set;
  }

  if (read_gazetteer_index_directory (&gid, &gi, idx_fp) == (rspf_int32)0) {
    fclose(idx_fp);
    return query_set;
  }
...
#if UNIX
  if ( fseek ( idx_fp,sizeof (ThematicIndexHeader)-sizeof(rspf_int32), 0 )!= 0 )
#else
...
rspf_int32 read_thematic_index_header ( ThematicIndexHeader *h, FILE *ifp ) 
{
...
rspf_int32 write_thematic_index_header ( ThematicIndexHeader h, FILE *ifp ) 
{
  if ( fseek ( ifp, 0, 0 ) != 0 ) 	/* rewind, just in case */
    WWhimper() ;				/* error */
 ...
 *	idsize < input > == (rspf_int32) size of each data element.
...
rspf_int32 write_thematic_index_directory ( ThematicIndexHeader h, 
					  ThematicIndexDirectory *d,
					  rspf_int32 idsize , /* size of data */
					  FILE *ifp )
{
  ...
  /* rewind, just in case */
#if UNIX
  if ( fseek ( ifp, sizeof (ThematicIndexHeader)-sizeof(rspf_int32), 0 ) != 0 )
#else
...
rspf_int32 write_gazetteer_index_directory ( ThematicIndexHeader h, 
					  ThematicIndexDirectory *d, 
					  rspf_int32 idsize , /* size of data */
					  FILE *ifp )
{
