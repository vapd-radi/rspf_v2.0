...
#include <rspf/vpfutil/machine.h>
...
{
  rspf_int32 CurrentChar,   /* This is an int because fgetc returns an int */
      count ,
      NextBlock = 256 ,
      LineAllocation = 0 ;
...
/* This forever loop searches past all lines beginning with #,
  ...
  for(count = 0; CurrentChar != EOF; CurrentChar = fgetc(fp), count++) {
    ...
    if ( ( CurrentChar == (rspf_int32) LINE_CONTINUE ) ) {
      CurrentChar = fgetc(fp ) ;        /* read character after backslash */
      /* A newline will be ignored and thus skipped over */
      if ( CurrentChar == (rspf_int32) SPACE )  /* Assume line continue error */
        while ( fgetc (fp) != (rspf_int32) SPACE ) ;
      else if (CurrentChar != (rspf_int32) NEW_LINE ) {
    ...
    } else if (CurrentChar == (rspf_int32) NEW_LINE )     /* We're done */
        break;
    else
      CurrentLine[count] = (char)CurrentChar;
...
rspf_int32 VpfRead ( void *to, VpfDataType type, rspf_int32 count, FILE *from )
{
  rspf_int32 retval=0 , i ;
  ...
  case VpfInteger:
	...
	rspf_int32 itemp,
	  *iptr = (rspf_int32 *) to ;
	for ( i=0; i < count; i++ ) {
	  retval = (long)fread ( &itemp, sizeof (rspf_int32), 1, from ) ;
	  swap_four ( (char*)&itemp, (char*)iptr ) ;
	  iptr++ ;
	}
      } else {
	retval = (long)fread ( to, sizeof (rspf_int32), count, from ) ;
      }
    }  
    break ;
  case VpfFloat:
 ...
 *   Parameters:
 *A
 *    row_number <input> == (rspf_int32) row number in the table.
 *    table      <input> == (vpf_table_type) VPF table structure.
 *    return    <output> == (rspf_int32) length of the table row or 0 on error.
 *E
 *::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
 *
 *   History:
 ...
 *E
 *************************************************************************/
rspf_int32 index_length( rspf_int32 row_number,
		       vpf_table_type table )
{
   rspf_int32   recsize,len=0;
   rspf_uint32 ulen;
   long  int pos;
   ...
   switch (table.xstorage) {
	 ...
	 fseek( table.xfp, (rspf_int32)(row_number*recsize), SEEK_SET );
 ...
 *    row_number <input> == (rspf_int32) row number in the table.
 *    table      <input> == (vpf_table_type) VPF table structure.
 *    return    <output> == (rspf_int32) position of the table row 
 *                          or zero on error.
 *E
 *::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
 *
 ...
 *E
 *************************************************************************/
rspf_int32 index_pos( rspf_int32 row_number,
		    vpf_table_type table )
{
   rspf_int32   recsize;
   rspf_uint32 pos=0;
...
static rspf_int32 row_offset( int field, row_type row, vpf_table_type table)
{
   rspf_int32 offset,n,size;
   int i;
   id_triplet_type key;
   int keysize[] = {0,sizeof(char),sizeof(short int),sizeof(rspf_int32)};
   ...
   for (i=0;i<field;i++) {
      switch (table.header[i].type) {
	 case 'I':
	    offset += sizeof(rspf_int32)*row[i].count;
	    break;
	 case 'S':
...
id_triplet_type read_key( vpf_table_type table )
{
   ...
   switch (TYPE0(key.type)) {
	 ...
	 key.id = (rspf_int32)ucval;
	 break;
      case 2:

	 Read_Vpf_Short (&uival, table.fp, 1 ) ;
	 key.id = (rspf_int32)uival;
	 break;
      case 3:
   ...
   case 1:
     Read_Vpf_Char (&ucval, table.fp, 1 ) ;
     key.tile = (rspf_int32)ucval;
     break;
   case 2:
     Read_Vpf_Short (&uival, table.fp, 1 ) ;
     key.tile = (rspf_int32)uival;
     break;
   case 3:
   ...
   case 1:
     Read_Vpf_Char (&ucval, table.fp, 1 ) ;
     key.exid = (rspf_int32)ucval;
     break;
   case 2:
     Read_Vpf_Short (&uival, table.fp, 1 ) ;
     key.exid = (rspf_int32)uival;
     break;
   case 3:
...
{
   register rspf_int32 i,j;
   rspf_int32      status;
   char     *tptr;
   rspf_int32 size,count;
   row_type row;
   ...
   for (i=0;i<table.nfields;i++) {
	 ...
	 case 'T':
	    ...
	    break;
	 case 'I':
	    row[i].ptr = (rspf_int32 *)vpfmalloc(count*sizeof(rspf_int32));
		 ...
		 vpf_table_type table )
{
   ...
   for (i=0;i<table.nfields;i++) {
	 ...
	 case 'T':
	    ...
	    break;
	 case 'I':
	    size = count*sizeof(rspf_int32);
	    row[i].ptr = (rspf_int32 *)vpfmalloc(size);
 ...
 *   Parameters:
 *A
 *    row_number <input> == (rspf_int32) row number.
 *    table      <input> == (vpf_table_type) vpf table structure.
 ...
 *   Functions Called:
 *F
 *    rspf_int32 index_pos()                             VPFTABLE.C
 ...
 *E
 *************************************************************************/
row_type read_row( rspf_int32 row_number,
		   vpf_table_type table )
{
   rspf_int32 fpos;
 ...
 *   Parameters:
 *A
 *    row_number <input> == (rspf_int32) row number in range [1 .. table.nrows].
 *    table      <input> == (vpf_table_type) vpf table structure.
 ...
 *E
 *************************************************************************/
row_type get_row( rspf_int32 row_number,
		  vpf_table_type table )
{
   row_type row;
 ...
 *    table      <input> == (vpf_table_type) VPF table structure.
 *    table_pos  <output> == (rspf_int32) returned column number.
 ...
 *E
 *************************************************************************/
rspf_int32 table_pos( const char* field_name, vpf_table_type table )
{
   register rspf_int32 i;
   rspf_int32 col;
   char altfn[256];
   ...
   for (i = 0; i < table.nfields; i++) {
      ...
      if (rspf_strcasecmp(field_name,table.header[i].name) == 0
	  || rspf_strcasecmp(altfn,table.header[i].name) == 0) {
	 col = i;
	 break;
      }
   }
 ...
 *   Parameters:
 *A
 *    field_number <input> == (rspf_int32) field column number.
 *    row          <input> == (row_type) vpf table row.
 *    table        <input> == (vpf_table_type) VPF table structure.
 *    value       <output> == (void *) pointer to a single element value.
 *    count       <output> == (rspf_int32 *) pointer to the array size for a
			 ...
			 rspf_int32  *count )
{
   ...
   switch (table.header[col].type) {
	 ...
	 break;
      case 'I':
	 if (table.header[col].count == 1) {
	    memcpy(value,row[col].ptr,sizeof(rspf_int32));
	 } else {
	    retvalue = (rspf_int32 *)vpfmalloc(row[col].count*
					     sizeof(rspf_int32));
	    memcpy(retvalue,row[col].ptr,row[col].count*sizeof(rspf_int32));
	 }
	 break;
      case 'S':
	 if (table.header[col].count == 1) {
 ...
 *A
 *    field_name <input> == (char *) field name.
 *    row_number <input> == (rspf_int32) row_number.
 *    table      <input> == (vpf_table_type) VPF table structure.
 *    value     <output> == (void *) pointer to a single element value.
 *    count     <output> == (rspf_int32 *) pointer to the array size for a multiple
 *                                  element value.
 *    return    <output> == (void *) returned multiple element value.
 ...
 *   Parameters:
 *A
 *    field_number <input> == (rspf_int32) field number (offset from
 ...
 *E
 *************************************************************************/
void *table_element( rspf_int32         field_number,
		     rspf_int32         row_number,
		     vpf_table_type   table,
		     void           * value,
		     rspf_int32       * count )
