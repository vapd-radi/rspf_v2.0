...
typedef struct {
   ...
   rspf_int32 maplen;            /* Number of cells in the tree */
   rspf_int32 cellmap[1023][2];  /* The tree of cells */
   rspf_int32 buf[1023][2];      /* Buffer for one cell */
   rspf_int32 shift;             /* Size of data before real cell records */
 ...
 *   Parameters:
 *A
 *    num  <input>==(rspf_int32) record number.
 ...
 *E
 *************************************************************************/
static void get_record(rspf_int32 num, spx_type *spx, set_type set )
{
   long offset;
   int j,index;
   unsigned char box[4];
   ...
   read(spx->fd,spx->buf,2*spx->cellmap[index][NUMFEAT]*sizeof(rspf_int32));
 ...
 *   Parameters:
 *A
 *    record <input>==(rspf_int32) record number.
 ...
 *E
 *************************************************************************/
static void search_cell( rspf_int32 record, int level, unsigned char bnd[4],
...
/*                                 record = 2 integers (8 byte)   */
   read(spx.fd,head,6*sizeof(rspf_int32));
   spx.shift = 6*sizeof(rspf_int32);
...
/* read the cell tree */
   ...
   if( (size_t)read(spx.fd,spx.cellmap,2*spx.maplen*sizeof(rspf_int32)) !=
       (2*spx.maplen*sizeof(rspf_int32)) )
   {
      close(spx.fd);
      return set;
   }

   spx.shift += 2*spx.maplen*sizeof(rspf_int32);
