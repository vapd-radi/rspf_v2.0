 ...
 *    return <output> == (rspf_int32) size of the key.
 *E
 *::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
 *
 *   History:
 ...
 *E
 *************************************************************************/
rspf_int32 write_key( id_triplet_type key, FILE *fp )
{
  rspf_int32 size = 0 ;	/* to count size of key write */
  unsigned char tint ;
  rspf_int16 tshort ;
  ...
  size += sizeof ( char ) ;
   ...
   case 2:
     ...
     size += sizeof ( rspf_int16 ) ;
     break;
   case 3:
     Write_Vpf_Int (&(key.id), fp, 1 ) ;
     size += sizeof ( rspf_int32 ) ;
     break;
   }
 ...
 *E
 *************************************************************************/
rspf_int32 write_next_row(row_type row, vpf_table_type * table )
{
   register rspf_int32 i,
		     j;
   char            * tptr,
		   * output ;
   rspf_int32          recordsize = 0;
   rspf_int32	     count;
   id_triplet_type * keys;
   rspf_uint32 pos_for_ndx,
		     length;
   int               retn_val = 0;
   ...
   for (i = 0; i < table->nfields; i++) {   /* for each column */
     ...
     case 'I':
       ...
       recordsize += sizeof ( rspf_int32 ) * count ;
       break;

     case 'S':
       Write_Vpf_Short (row[i].ptr, table->fp, count ) ;
       recordsize += sizeof ( rspf_int16 ) * count ;
       break;
 ...
 *   Parameters:
 *A
 *     field <input>  == (rspf_int32) column offset.
 ...
 *E
 *************************************************************************/
void nullify_table_element( rspf_int32            field,
			    row_type       row,
 ...
 *     count <in>     == (rspf_int32) number of items in value.
 *     put_table_element <output> == (rspf_int32)
 ...
 *E
 *************************************************************************/
rspf_int32 put_table_element( rspf_int32              field,
		       row_type         row,
		       vpf_table_type   table,
		       void           * value,
		       rspf_int32         count )
{
   rspf_int32 i, len, stat;
   char *str;
   ...
   switch ( table.header[field].type ) {
      case 'T':
	len = (rspf_int32)max(count,table.header[field].count);
	...
	break;
      ...
      case 'I' :
	row[field].ptr = (rspf_int32 *) vpfmalloc (count*sizeof(rspf_int32));
	memcpy ( row[field].ptr, value, sizeof (rspf_int32) * count ) ;
	break;

      case 'S' :
	row[field].ptr = (rspf_int16 *) vpfmalloc (count*sizeof(rspf_int16));
	memcpy ( row[field].ptr, value, sizeof (rspf_int16) * count ) ;
	break;
...
rspf_int32 VpfWrite ( void *from, VpfDataType type, rspf_int32 count, FILE *to )
{
  ...
  case VpfInteger:
    {
      if (MACHINE_BYTE_ORDER != STORAGE_BYTE_ORDER) {
	 rspf_int32 itemp,
	   *iptr = (rspf_int32 *) from ;
	 for ( i=0; i < count; i++, iptr++ ) {
	   swap_four ( (char*)iptr, (char*)&itemp ) ;
	   retval = (long)fwrite ( &itemp, sizeof (rspf_int32), 1, to ) ;
	 }
      } else {
	 retval = (long)fwrite ( from, sizeof (rspf_int32), count, to ) ;
      }
    }
    break ;
  case VpfFloat:
