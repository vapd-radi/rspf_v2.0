//----------------------------------------------------------------------------
//
// File rspf-batch-test.cpp
// 
// License:  See top level LICENSE.txt file.
//
// Author:  David Burken, Oscar Kramer
//
// Description: Test code application rspf batch test.
//
//----------------------------------------------------------------------------
// $Id: rspf-batch-test.cpp 3112 2012-01-26 17:28:00Z david.burken $

#include <rspf/util/rspfBatchTest.h>
...
//**************************************************************************************************
bool rspfBatchTest::initialize(rspfArgumentParser& ap)
{
   ...
   {
      rspfFilename templateFile (tempString.c_str());
   ...
   while ( ap.read("-a", stringParam) || ap.read("--accept-test", stringParam) )
   {
      if ( tempString.size() )
      {
         ...
         rspfString os = tempString;
         rspfString separatorList = " ";
...
//**************************************************************************************************
void rspfBatchTest::writeTemplate(const rspfFilename& templateFile, bool write_long_form)
{
   if (templateFile.exists())
   {
      rspfString del_cmd ("$(RM_CMD) ");
   ...
   if (write_long_form)
   {
      out<< "//===================================================================================\n"
         ...
         << "// Generated by command: rspf-batch-test -W " << templateFile.c_str() << "\n"
         << "// Description: rspf-batch-test template config file.\n"
         << "// \n"
         << "// Control flags:\n"
         ...
         << "//     RSPF_BATCH_TEST_DATA     Top-level dir containing all test source data\n"
         << "//     RSPF_BATCH_TEST_RESULTS  Top-level dir containing all test results (exp, \n"
         << "//                               out and log),\n"
         << "// \n"
         ...
         << "test1.expected_results_command0: rspf-info --height -42.8508 147.2537 > $(OBT_EXP_DIR)\\height.txt\n"
         << "test1.expected_results_command1: $(COPY_CMD) $(OBT_OUT_DIR)\\height.txt $(OBT_EXP_DIR)\\height.txt\n"
         << "\n"
         << "//---------------------------------------\n"
         << "// The actual commands to test\n"
         << "//---------------------------------------\n"
         << "test1.test_command0: rspf-info --height -42.8508 147.2537 > $(OBT_OUT_DIR)\\height.txt\n"
         << "\n"
         << "//-------------------------------------------\n"
   ...
   else 
   {
      out<< "//===================================================================================\n"
         ...
         << "// Commands to generate expected results.\n"
         << "//---------------------------------------\n"
         << "expected_results_command0: rspf-info --height -42.8508 147.2537 > $(OBT_OUT_DIR)\\height.txt\n"
         << "expected_results_command1: $(COPY_CMD) $(OBT_OUT_DIR)\\height.txt $(OBT_EXP_DIR)\\height.txt\n"
         << "\n"
         << "//---------------------------------------\n"
         << "// The actual commands to test.\n"
         << "//---------------------------------------\n"
         << "test_command0: rspf-info --height -42.8508 147.2537 > $(OBT_OUT_DIR)\\height.txt\n"
         << "\n"
         << "//---------------------------------------\n"
   ...
   out.close();
   rspfNotify(rspfNotifyLevel_NOTICE) << "Wrote file: " << templateFile.c_str() << std::endl;
...
//**************************************************************************************************
rspf_uint8 rspfBatchTest::execute()
{
   if (m_templateModeActive)
      return (rspf_uint8) TEST_PASSED;

   // Establish the top-level test directory that will contain log, exp and out subdirs:
   rspfFilename base_output_dir = rspfEnvironmentUtility::instance()->getEnvironmentVariable(
         rspfString("RSPF_BATCH_TEST_RESULTS") );
   if (base_output_dir.empty())
   {
      cout<<"\nrspfBatchTest WARNING: The environment variable RSPF_BATCH_TEST_RESULTS is not "
...
#if defined(WIN32) || defined(_MSC_VER) && !defined(__CYGWIN__) && !defined(__MWERKS__)
   rspfString env_spec = rspfString("OBT_OUT_DIR=") + m_outDir;
   _putenv(env_spec.chars());
   env_spec = rspfString("OBT_EXP_DIR=") + m_expDir;
   _putenv(env_spec.chars());
#else
   ...
   rspf_uint8 status = processConfigList(kwl);
   if (status != TEST_TBD)
      return status;
   ...
   rspfFilename logDir = base_output_dir.dirCat("log");
   ...
   double startTime = rspfTimer::instance()->time_s();

   // Get the number of test:
   rspfString regExpStr = "test[0-9]+\\.";
   rspf_uint32 num_tests = kwl.getNumberOfSubstringKeys(regExpStr);
   const rspf_uint32 MAX_INDEX = num_tests + 1000;
   rspfString prefixBase = "test";
   rspf_uint32 index = 0;
   rspf_uint32 processedIndexes = 0;
   rspfString prefix;
   ...
   rspf_uint8 overall_test_status = (rspf_uint8) TEST_TBD;
   while ( processedIndexes < num_tests )
   {
      if (!is_single_test)
         prefix = prefixBase + rspfString::toString(index) + ".";

      rspf_uint8 individual_test_status = processTest( prefix, kwl);
      if ( individual_test_status != (rspf_uint8) TEST_TBD)
         ++processedIndexes;
   ...
   getDateString(date);
   m_logStr << "\nstop_time: " << date << "\n";
   double stopTime = rspfTimer::instance()->time_s();
...
//************************************************************************************************
rspf_uint8 rspfBatchTest::processConfigList(const rspfKeywordlist& kwl)
{
   rspf_uint8 overall_test_status = (rspf_uint8) TEST_TBD;
   rspfFilename config_list_path = m_configFileName.path();

   // Look for keywords for config file name:
   rspfString prefixBase = "test_config_file";
   rspfString regExpStr = prefixBase + "[0-9]+";
   const rspf_uint32 count = kwl.getNumberOfSubstringKeys(regExpStr);
   ...
   if (count != 0)
   {
      ...
      while (processedIndexes < count)
      {
         ...
         rspfString kw = prefixBase + rspfString::toString(index);
         m_configFileName = rspfFilename(kwl.find(kw.chars()));
...
//************************************************************************************************
void rspfBatchTest::getDateString(rspfString& date)
{
   time_t t;
   time(&t);
   tm* lt;
...
//**************************************************************************************************
void rspfBatchTest::getLogFilename(rspfFilename& logFile)
{
   logFile = "obt-log-";
   rspfString date;
   getDateString(date);
...
//**************************************************************************************************
rspf_uint8 rspfBatchTest::processTest(const rspfString& prefix, const rspfKeywordlist& kwl)
{
   rspf_uint8 testStatus = TEST_TBD;
   ...
   if ( lookup )
      enabled = rspfString(lookup).toBool();

   if ( !enabled )
   {
      testStatus = TEST_DISABLED;
      rspfString statusString;
      if ( prefix.size() )
      {
         statusString = prefix.trim(rspfString(".")) + rspfString(": disabled");
      }
      else
      {
...
//**************************************************************************************************
rspf_uint8 rspfBatchTest::processCommands(const rspfString& prefixBase,
                                            const rspfKeywordlist& kwl,
                                            const rspfString& testName,
                                            bool logTime,
                                            const rspfFilename& tempFileName)
{
   rspf_uint8 result = TEST_TBD;

   //---
   // We must do these in order so we will use the
   // rspfKeywordlist::getNumberOfSubstringKeys
   // instead of: rspfKeywordlist::getSubstringKeyList
   //
   ...
   //---
   rspfString regExpStr = prefixBase + "[0-9]+";
   const rspf_uint32 num_commands = kwl.getNumberOfSubstringKeys(regExpStr);
   const rspf_uint32 MAX_INDEX = num_commands + 1000;
   ...
   {
      rspfString cmd_kw =  prefixBase + rspfString::toString(index);
      lookup = kwl.find( cmd_kw );
      if ( lookup )
      {
         rspfString command_line = convertToNative( lookup ).c_str();
         ...
         if ( logTime )
         {
            ...
            // Start the clock:
            startTime = rspfTimer::instance()->time_s();
         }
         ...
         if ( logTime )
         {
            ...
            stopTime = rspfTimer::instance()->time_s();
            getDateString(date);
            m_logStr << "end: " << date << "\n"
...
//**************************************************************************************************
void rspfBatchTest::preprocessKwl(const std::vector<std::string>& testList,
                                   ...
                                   rspfKeywordlist& kwl)
{
   ...
   while ( testIter != testList.end() )
   {
      ...
      if (num_tests == 0)
      {
         ...
         const rspf_uint32 cmd_count = kwl.getNumberOfSubstringKeys(regExpStr);
         if (cmd_count != 0)
         {
            ++num_tests;
            is_single_test = true;
...
//**************************************************************************************************
void rspfBatchTest::disableAllKwl(rspfKeywordlist& kwl)
{
   if ( kwl.getSize() )
   {
      // Get the number of test:
      rspfString regExpStr = "test[0-9]+\\.";
      const rspf_uint32 COUNT = kwl.getNumberOfSubstringKeys(regExpStr);
      const rspf_uint32 MAX_INDEX = COUNT + 1000;
      rspfString prefixBase = "test";
      rspf_uint32 index = 0;
      rspf_uint32 processedIndexes = 0;
      ...
      while ( processedIndexes < COUNT )
      {
         rspfString prefix =  prefixBase + rspfString::toString(index) + ".";
         const char* lookup = kwl.find( prefix, "name" );
         if (lookup)
         {
...
} // End: disableAllKwl(rspfKeywordlist& kwl)
...
//! successful.
//**************************************************************************************************
bool rspfBatchTest::makeDefaultResultsDir()
{
...
//**************************************************************************************************
bool rspfBatchTest::doDefaultClean()
{
   bool result = true;

   rspfString del_cmd = rspfEnvironmentUtility::instance()->getEnvironmentVariable(
      rspfString("RMDIR_CMD") );
   rspfString command_line;
...
bool rspfBatchTest::getTempFileName( const rspfString& prefix,
                                      const rspfKeywordlist& kwl,
...
bool rspfBatchTest::getDefaultTempFileDir( rspfFilename& tempFile ) const
{
   // Create a default tmp directory under RSPF_BATCH_TEST_RESULTS.
   tempFile = rspfEnvironmentUtility::instance()->getEnvironmentVariable(
      rspfString("RSPF_BATCH_TEST_RESULTS") );
   if (!tempFile.empty())
      tempFile = tempFile.dirCat("tmp");
   else
      tempFile = rspfEnvironmentUtility::instance()->getEnvironmentVariable(rspfString("TEMP"));
   return ( tempFile.size() > 0 );
}

void rspfBatchTest::usage(rspfArgumentParser& ap)
{
   ...
   // Write usage.
   au->write(rspfNotify(rspfNotifyLevel_INFO));
}

std::string rspfBatchTest::convertToNative( const char* lookup ) const
{
   std::string s;
