...
//
//*****************************************************************************
//  $Id: rspfPolygon.cpp 19682 2011-05-31 14:21:20Z dburken $
...
rspfPolygon::rspfPolygon(const vector<rspfIpt>& polygon)
   :theOrderingType(RSPF_VERTEX_ORDER_UNKNOWN),
   ...
   
{
   ...
   for (std::vector<rspfIpt>::size_type i = 0; i < polygon.size(); ++i)
   {
      theVertexList[i] = polygon[i];
   }
}

rspfPolygon::rspfPolygon(const vector<rspfDpt>& polygon)
   :theOrderingType(RSPF_VERTEX_ORDER_UNKNOWN),
...
//*****************************************************************************
//  CONSTRUCTOR: rspfPolygon(int numVertices, const rspfDpt* vertex_array)
//  
//*****************************************************************************
rspfPolygon::rspfPolygon(int numVertices, const rspfDpt* v)
   : theOrderingType(RSPF_VERTEX_ORDER_UNKNOWN),
     theCurrentVertex(0)
...
//*****************************************************************************
//  COPY CONSTRUCTOR: rspfPolygon(rspfPolygon)
//  
//*****************************************************************************
rspfPolygon::rspfPolygon(const rspfPolygon& polygon)
   :theCurrentVertex(0)
{
   *this = polygon;
}
...
rspfPolygon::rspfPolygon(const rspfIrect& rect)
...
rspfPolygon::rspfPolygon(const rspfDrect& rect)
: theOrderingType(RSPF_CLOCKWISE_ORDER),
theVertexList(4),
theCurrentVertex(0)
{
...
void rspfPolygon::roundToIntegerBounds(bool compress)
{
   int i = 0;
...
rspfDpt rspfPolygon::midPoint()const
{
   int upper = (int)theVertexList.size();
   rspfDpt result(0.0, 0.0);
   int i = 0;
...
bool rspfPolygon::hasNans()const
{
   int upper = (int)theVertexList.size();
   int i = 0;
...
void rspfPolygon::getIntegerBounds(rspf_int32& minX,
   rspf_int32& minY,
   rspf_int32& maxX,
...
void rspfPolygon::getFloatBounds(rspf_float64& minX,
                                  rspf_float64& minY,
                                  rspf_float64& maxX,
...
bool rspfPolygon::clipToRect(vector<rspfPolygon>& result,
                              const rspfDrect& rect)const
{
   result.clear();
   rspfPolyArea2d p1(*this);
   rspfPolyArea2d p2(rect.ul(), rect.ur(), rect.lr(), rect.ll());
...
//*****************************************************************************
//  METHOD: rspfPolygon::clipLineSegment(p1, p2)
//  
...
//  
//*****************************************************************************
bool rspfPolygon::clipLineSegment(rspfDpt& P, rspfDpt& Q) const
{
   rspfDpt PQ (Q - P);
   double tE = 0.0;
   double tL = 1.0;
   rspfLine edge, edgeE, edgeL;
   bool intersected=false;
   double num, denom, t;
   rspf_uint32 npol = (rspf_uint32)theVertexList.size();
   ...
   for(i = 0, j = 1; i < npol;)
   {
      edge = rspfLine(theVertexList[i],
                       theVertexList[j]);
      
      rspfDpt normal = edge.normal();
...
*/
bool rspfPolygon::isRectWithin(const rspfIrect &rect) const 
{
...
*/
bool rspfPolygon::isPolyWithin(const rspfPolygon &poly) const 
{
   bool ret=false;
...
//*****************************************************************************
//  METHOD: rspfPolygon::pointWithin(const rspfDpt& point)
//  
//  Returns TRUE if point is inside polygon.
//  
//*****************************************************************************
bool rspfPolygon::isPointWithin(const rspfDpt& point) const
{
   ...
   if(!c) // check if on if not within
   {
      for (i = 0, j = npol-1; i < npol; j = i++)
      {
         if(rspfLine(theVertexList[i], theVertexList[j]).isPointWithin(point))
         {
            return true;
         }
      }
...
//*****************************************************************************
//  METHOD: rspfPolygon::vertex(int)
//  
//  Returns the rspfDpt vertex given the index. Also initializes the current
//  edge (theCurrentEdge) to the edge corresponding to the index.
//  
//*****************************************************************************
bool rspfPolygon::vertex(int index, rspfDpt& tbd_vertex) const 
{
...
//
//*****************************************************************************
bool rspfPolygon::nextVertex(rspfDpt& tbd_vertex) const 
{
   ++theCurrentVertex;
...
//  
//*****************************************************************************
const rspfPolygon& rspfPolygon::operator= (const rspfIrect& rect)
{
   theCurrentVertex = 0;
...
const rspfPolygon& rspfPolygon::operator= (const rspfDrect& rect)
{
   theCurrentVertex = 0;
   theVertexList.resize(4);
   theVertexList[0] = rect.ul();
...
const rspfPolygon&  rspfPolygon::operator=(const rspfPolygon& polygon)
{
...
const rspfPolygon& rspfPolygon::operator= (const vector<rspfDpt>& vertexList)
{
...
const rspfPolygon& rspfPolygon::operator=(const vector<rspfIpt>& vertexList)
{
...
//  
//*****************************************************************************
bool rspfPolygon::operator==(const rspfPolygon& polygon) const
{
...
const rspfPolygon& rspfPolygon::operator *=(const rspfDpt& scale)
{
...
rspfPolygon rspfPolygon::operator *(const rspfDpt& scale)const
{
   rspfPolygon result(*this);
...
//*****************************************************************************
//  METHOD: rspfPolygon::print(ostream)
//  
//*****************************************************************************
void rspfPolygon::print(ostream& os) const
{
...
rspfVertexOrdering rspfPolygon::checkOrdering()const
{
...
void rspfPolygon::intersectEdge(rspfDpt& result,
...
bool rspfPolygon::isInsideEdge(const rspfDpt& pt,
                                const rspfDrect& rect,
                                int edge)const
{
   switch(edge)
   ...
   case RECT_TOP_EDGE:
   {
      if(rect.orientMode() == RSPF_LEFT_HANDED)
      {
...
bool rspfPolygon::saveState(rspfKeywordlist& kwl,
                             const char* prefix)const
{
   ...
   {
   case RSPF_VERTEX_ORDER_UNKNOWN:
   {
      order = "unknown";
      break;
   }
   case RSPF_CLOCKWISE_ORDER:
   {
      order = "clockwise";
      break;
   }
   case RSPF_COUNTERCLOCKWISE_ORDER:
   {
...
bool rspfPolygon::loadState(const rspfKeywordlist& kwl,
                             const char* prefix)
{
   rspfString order = kwl.find(prefix, VERTEX_ORDER_KW);
...
void rspfPolygon::getMinimumBoundingRect(rspfPolygon& minRect) const
{
   ...
   rspfPolygon rotatedPolygon(*this);
   rspfDpt xlatedVertex;
   rspfDpt rotatedVertex(0.0, 0.0);
   double min_x, min_y, max_x, max_y;
   double area;
   double min_area = 1.0/DBL_EPSILON;
   rotatedPolygon.theVertexList[0] = rspfDpt(0, 0);  // first vertex always at origin
   ...
   while (angle_step > MIN_STEP)
   {
      ...
      for (int i=0; i<5; i++)
      {
         ...
         if ((i != 2) || (first_time)) 
         {
            ...
            if (TESTING)
            {
               rspfDpt v1 (cos_theta*min_x - sin_theta*max_y + theVertexList[0].x,
                            cos_theta*max_y + sin_theta*min_x + theVertexList[0].y);
               rspfDpt v2 (cos_theta*max_x - sin_theta*max_y + theVertexList[0].x,
                            cos_theta*max_y + sin_theta*max_x + theVertexList[0].y);
               rspfDpt v3 (cos_theta*max_x - sin_theta*min_y + theVertexList[0].x,
                            cos_theta*min_y + sin_theta*max_x + theVertexList[0].y);
               rspfDpt v4 (cos_theta*min_x - sin_theta*min_y + theVertexList[0].x,
            ...
            if (area < min_area)
            {
               best_theta = theta;
               min_area = area;
               best_rect = rspfDrect(min_x, max_y, max_x, min_y, RSPF_RIGHT_HANDED);
            }
   ...
   rspfDpt v1 (cos_theta*best_rect.ul().x - sin_theta*best_rect.ul().y + theVertexList[0].x,
                cos_theta*best_rect.ul().y + sin_theta*best_rect.ul().x + theVertexList[0].y);
   rspfDpt v2 (cos_theta*best_rect.ur().x - sin_theta*best_rect.ur().y + theVertexList[0].x,
                cos_theta*best_rect.ur().y + sin_theta*best_rect.ur().x + theVertexList[0].y);
   rspfDpt v3 (cos_theta*best_rect.lr().x - sin_theta*best_rect.lr().y + theVertexList[0].x,
                cos_theta*best_rect.lr().y + sin_theta*best_rect.lr().x + theVertexList[0].y);
   rspfDpt v4 (cos_theta*best_rect.ll().x - sin_theta*best_rect.ll().y + theVertexList[0].x,
   ...
   if(minRect.getOrdering()==RSPF_COUNTERCLOCKWISE_ORDER)
...
*/
void rspfPolygon::removeVertex(int vertex)
{
   int numvertices=getNumberOfVertices();
   if(vertex>numvertices) {
      return;
   } else {
      vector<rspfDpt>::iterator it;
      int v=0;
...
*/
void rspfPolygon::removeSmallestContributingVertex()
{
...
rspfDpt& rspfPolygon::operator[](int index)
{
   return theVertexList[index];
}

const rspfDpt& rspfPolygon::operator[](int index)const
{
   return theVertexList[index];
}

rspf_uint32 rspfPolygon::getVertexCount()const
{
   return getNumberOfVertices();
}

rspf_uint32 rspfPolygon::getNumberOfVertices()const
{
   return (rspf_uint32)theVertexList.size();
}

void rspfPolygon::getBoundingRect(rspfIrect& rect)const
{
   rspf_int32 minX;
   rspf_int32 minY;
   rspf_int32 maxX;
   ...
   rect = rspfIrect(minX, minY, maxX, maxY);
}

void rspfPolygon::getBoundingRect(rspfDrect& rect)const
{
   rspf_float64 minX;
   rspf_float64 minY;
   rspf_float64 maxX;
...
void rspfPolygon::clear()
{
   theVertexList.clear();
   theOrderingType = RSPF_VERTEX_ORDER_UNKNOWN;
}

void rspfPolygon::addPoint(const rspfDpt& pt)
{
   theVertexList.push_back(pt);
   theOrderingType = RSPF_VERTEX_ORDER_UNKNOWN;
}

void rspfPolygon::addPoint(double x, double y)
{
   theVertexList.push_back(rspfDpt(x, y));
   theOrderingType = RSPF_VERTEX_ORDER_UNKNOWN;
}

const vector<rspfDpt>& rspfPolygon::getVertexList()const
{
   return theVertexList;
}

 bool rspfPolygon::pointWithin(const rspfDpt& point) const
{
   return isPointWithin(point);
}

const rspfPolygon& rspfPolygon::operator *=(double scale)
{
   return ((*this)*=rspfDpt(scale, scale));
}

rspfPolygon rspfPolygon::operator *(double scale)const
{
   return ((*this)*rspfDpt(scale, scale));
}

void rspfPolygon::resize(rspf_uint32 newSize)
{
...
rspfVertexOrdering rspfPolygon::getOrdering()const
{
   return theOrderingType;
}

bool rspfPolygon::operator!=(const rspfPolygon& compare_this) const
{
   return !(*this == compare_this);
}

ostream& operator<<(ostream& os, const rspfPolygon& polygon)
{
   polygon.print(os);
   return os;
}
...
*/
void rspfPolygon::getCentroid(rspfDpt &centroid) const
{
   int numpts = (int)theVertexList.size();
   unsigned int next;
   double area=0,parea;

   centroid=rspfDpt(0,0);
...
*/
void rspfPolygon::fitCircleInsideVertex(rspfDpt &destPt, unsigned int vertex, double radius) const
{
   rspf_uint32 num_vertices=(int)theVertexList.size(),n1,n2;
   rspfDpt side1,side2,bisection,currpt;
   ...
   checkOrdering();
   if(getOrdering()==RSPF_COUNTERCLOCKWISE_ORDER) {
   ...
   destPt=rspfDpt(currpt+bisection*length_out);
   return;
}
...
*/
bool rspfPolygon::shrink(rspfPolygon &dest, double inset) const 
{
   int numpts = (int) theVertexList.size();
   rspfDpt pt;
