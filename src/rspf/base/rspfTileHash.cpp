...
//              
//*******************************************************************
//  $Id: rspfTileHash.cpp 9094 2006-06-13 19:12:40Z dburken $

#include <cfloat> // for FLT_EPSILON

#include <rspf/base/rspfTileHash.h>

rspfTileHash::rspfTileHash(const rspfDrect& imageRect,
                             double tileWidth,
                             double tileHeight)
   :
      rspfPointHash(),
...
rspfTileHash::~rspfTileHash()
{
}

long rspfTileHash::operator()(const rspfFpt& aPoint)
{
...
long rspfTileHash::operator()(const rspfDpt& aPoint)
{
   if ( (aPoint.x >= theImageRect.ul().x) &&
        (aPoint.x <= theImageRect.lr().x) &&
        (aPoint.y <= theImageRect.ul().y) &&
