...
// Description: 
//      A rspfHistogram contains an array of "buckets", which represent finite
// segments of some value axis, along with a corresponding array of
// frequency counts for each of these buckets.
//
//********************************************************************
// $Id: rspfHistogram.cpp 22124 2013-01-27 16:03:52Z dburken $

#include <rspf/base/rspfCommon.h>
#include <rspf/base/rspfHistogram.h>
#include <rspf/base/rspfNotifyContext.h>
#include <rspf/base/rspfThinPlateSpline.h>
...
RTTI_DEF1(rspfHistogram, "rspfHistogram", rspfObject);
rspfHistogram::rspfHistogram()
   :
...
rspfHistogram::rspfHistogram(int xres, float val1, float val2)
   :
   stats_consistent(MEAN_FLAG | SD_FLAG),
   vals(new float [xres]),
...
rspfHistogram::rspfHistogram(float* uvals, float* ucounts, int xres)
   :
   stats_consistent(MEAN_FLAG | SD_FLAG),
   vals(uvals),
   counts(ucounts),
...
//-----------------------------------------------------------
// -- Copy constructor
rspfHistogram::rspfHistogram(const rspfHistogram& his)
{
...
rspfHistogram::rspfHistogram(const rspfHistogram* his, float width)
{
...
void rspfHistogram::create(int xres, float val1, float val2)
{
   // clear all the data
   deleteAll();
   ...
   if (vals == NULL || counts == NULL)
   {
      rspfNotify(rspfNotifyLevel_FATAL) << "Histogram : Ran out of memory for arrays.\n";
      vals = NULL;
      counts = NULL;
      num = 0;
      vmin = 0;
...
rspfHistogram* rspfHistogram::fillInteriorEmptyBins(int type)const
{
   rspfHistogram* result = new rspfHistogram(*this);
   if(num < 1) return 0;
   switch(type)
   {
...
rspfHistogram* rspfHistogram::Scale(float scale_factor)
{
...
// Construct a new histogram
   ...
   rspfHistogram* scaled_his = new rspfHistogram(this, delta);
...
//            {x | x <= (xi + .5*delta)}
rspfHistogram* rspfHistogram::CumulativeGreaterThanEqual()const
{
   rspfHistogram* cum_his = new rspfHistogram(*this);
...
rspfHistogram* rspfHistogram::CumulativeLessThanEqual()const
{
   rspfHistogram* cum_his = new rspfHistogram(*this);
   const float* density_counts = this->GetCounts();
   int res = this->GetRes();
...
rspfHistogram* rspfHistogram::NonMaximumSupress(int radius, bool cyclic)
{
   if((2*radius +1)> num/2)
   {
      rspfNotify(rspfNotifyLevel_WARN)<<"rspfHistogram::NonMaximumSupress the radius is too large \n";
      return NULL;
   }
   //Get the counts array of "this"
   rspfHistogram* h_new = new rspfHistogram(*this);
...
float rspfHistogram::GetMean()const
{
   float xsum = 0.0;
...
float rspfHistogram::GetStandardDev()const
{
   float sum = 0.0;
...
int rspfHistogram::GetIndex(float pixelval)const
{
   if ((pixelval > vmax) || (pixelval < vmin)||(num==0))
   {
      return -1;
   }
//   rspf_float32 d = vmax-vmin;
   int bandIdx = (rspf_int32)((pixelval-vmin)/delta);
...
int rspfHistogram::GetValIndex(float pixelval)const
{
   if ((pixelval > vmax) || (pixelval < vmin))
      return -1;
...
float rspfHistogram::GetCount(float pixelval)const
{
...
float rspfHistogram::GetMinVal()const
{
   register int i=0;
...
float rspfHistogram::GetMaxVal()const
{
...
float rspfHistogram::GetMaxCount()const
{
   register int i=0;
   float max;
   max = 0.0;
...
float rspfHistogram::SetCount(float pixelval, float count)
{
   stats_consistent = 0;
...
void rspfHistogram::UpCount(float pixelval)
{
...
float rspfHistogram::ComputeArea(float low, float high)const
{
...
//
float rspfHistogram::ComputeArea()const
{
...
float rspfHistogram::getLowFractionFromValue(float val) const
{
...
float rspfHistogram::getHighFractionFromValue(float val) const
{
   float min = floor(GetMinVal());
   float max = ceil(GetMaxVal());
   if (val < min || val > max)
...
//     histogram area.
//
float rspfHistogram::LowClipVal(float clip_fraction)const
{
...
//     histogram area.
//
float rspfHistogram::HighClipVal(float clip_fraction)const
{
   if(clip_fraction<0) clip_fraction=0.0;
   if(clip_fraction>1.0) clip_fraction=1.0;
   float area = this->ComputeArea();
...
void rspfHistogram::Print()const
{
   ostream& out = rspfNotify(rspfNotifyLevel_INFO);
   const float* VALS = this->GetVals();
   const float* COUNTS = this->GetCounts();
   int res = this->GetRes();
   int width = 0;
...
void rspfHistogram::Dump(char *dumpfile)const
{
...
int rspfHistogram::WritePlot(const char *fname)const
{
...
bool rspfHistogram::importHistogram(istream& in)
{
...
bool rspfHistogram::importHistogram(const rspfFilename& inputFile)
{
   if(inputFile.exists())
   {
...
bool rspfHistogram::rspfProprietaryHeaderInformation::parseStream(istream& in)
{
...
bool rspfHistogram::saveState(rspfKeywordlist& kwl,
                               const char* prefix)const
{
   ...
   rspfString binArrayList = "(";
   bool firstValue = true;

   for(rspf_int32 index = 0; index < num; ++index)
   {
      if(fabs(counts[index]) > FLT_EPSILON)
      {
         ...
         binArrayList += "("+rspfString::toString(index)+","+rspfString::toString(counts[index])+")";
     }
   }
   ...
   for(rspf_int32 index = 0; index < num; ++index)
   {
      if(fabs(counts[index]) > FLT_EPSILON)
      {
         //     binValue = prefix;
         binValue = "bin";
         binValue += rspfString::toString(index);
...
bool rspfHistogram::loadState(const rspfKeywordlist& kwl,
                               const char* prefix)
{
   ...
   if(number_of_bins)
   {
      rspf_uint32 bins = rspfString(number_of_bins).toUInt32();
      ...
      if(bins > 0)
      {
         ...
         if(min_value)
         {
            minValue = (rspf_float32)rspfString(min_value).toDouble();
         }
         if(max_value)
         {
            maxValue = (rspf_float32)rspfString(max_value).toDouble();
         }
         ...
         //
         rspfString binsString = kwl.find(prefix, "bins");
         if(!binsString.empty())
         {
            std::vector<rspfDpt> result;
            rspf::toVector(result, binsString);
            if(!result.empty())
            {
               rspf_uint32 idx = 0;
               for(idx = 0; idx < result.size();++idx)
               {
                  rspf_uint32 binIdx = static_cast<rspf_uint32>(result[idx].x);
                  if(binIdx < bins)
                  {
         ...
         {
            rspfKeywordlist binsKwl;
            rspf_uint32 offset = (rspf_uint32)(rspfString(prefix)+"bin").size();
            rspfString regExpression =  rspfString("^(") + rspfString(prefix) + "bin[0-9]+)";
            kwl.extractKeysThatMatch(binsKwl,regExpression);
            const rspfKeywordlist::KeywordMap& kwlMap = binsKwl.getMap();
            rspfKeywordlist::KeywordMap::const_iterator iter = kwlMap.begin();
            while(iter != kwlMap.end())
            {
               rspfString numberStr(iter->first.begin() + offset,
                                     iter->first.end());
               countsPtr[numberStr.toUInt32()] = rspfString(iter->second).toDouble();
               ++iter;
            }
         }
         ...
         rspf_uint32 numberOfBins = (rspf_uint32)keys.size();
         rspf_uint32 offset = (rspf_uint32)(rspfString(prefix)+"bin").size();

         std::vector<rspf_uint32> theNumberList(numberOfBins);
         rspf_uint32 idx = 0;
         ...
         {
            const char* binCount = kwl.find(prefix, rspfString("bin") + rspfString::toString(theNumberList[idx]));
            countsPtr[theNumberList[idx]] = (float)rspfString(binCount).toDouble();
         }
#endif
      }
   }
...
bool rspfHistogram::loadState(const rspfRefPtr<rspfXmlNode> xmlNode)
{
   rspfRefPtr<rspfXmlNode> binValues =  xmlNode->findFirstNode("binValues");
   rspfRefPtr<rspfXmlNode> minValueNode  =  xmlNode->findFirstNode("minValue");
   rspfRefPtr<rspfXmlNode> maxValueNode  =  xmlNode->findFirstNode("maxValue");
   ...
   if(binValues.valid())
   {
      ...
      count = (rspf_uint32)floatValues.size();
...
bool rspfHistogram::saveState(rspfRefPtr<rspfXmlNode> xmlNode)const
{
   rspfRefPtr<rspfXmlNode> binValues = new rspfXmlNode;
   xmlNode->setTag("rspfHistogram");
   xmlNode->addChildNode("minValue", rspfString::toString(vmin));
   xmlNode->addChildNode("maxValue", rspfString::toString(vmax));
   xmlNode->addChildNode("standardDeviation", rspfString::toString(standard_dev));
   xmlNode->addChildNode("mean", rspfString::toString(mean));
   ...
   if(num > 0)
   {
      ...
      {
         out << rspfString::toString(counts[idx], 8) << " ";
      }
