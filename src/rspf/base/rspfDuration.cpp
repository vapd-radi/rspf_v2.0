...
//*************************************************************************
// $Id$
#include <rspf/base/rspfDuration.h>
#include <rspf/base/rspfCommon.h>
#include <sstream>
#include <cctype>
rspfDuration::rspfDuration(const rspfString& iso8601Duration)
:theSign(1),
theYears(0),
theMonths(0),
theWeeks(0),
...
bool rspfDuration::readIso8601Encoding(std::istream& in)
{
   clearFields();
...
bool rspfDuration::setByIso8601DurationString(const rspfString& iso8601Duration)
{
   if(iso8601Duration.empty())
   {
      clearFields();
...
void rspfDuration::toIso8601DurationString(rspfString& result)
{
   result = "";
   ...
   bool hasTimePeriod = ((theHours!=0)||
                         (theMinutes!=0)||
                         (!rspf::almostEqual(theSeconds, 0.0, .00000000001)));
   ...
   if(hasDatePeriod)
   {
      if(theYears > 0)
      {
         result+=rspfString::toString(theYears);
         result+="Y";
      }
      if(theMonths>0)
      {
         result+=rspfString::toString(theMonths);
         result+="M";
      }
      if(theWeeks>0)
      {
         result+=rspfString::toString(theWeeks);
         result+="W";
      }
      if(theDays>0)
      {
         result+=rspfString::toString(theDays);
         result+="D";
      }
   }
   if(hasTimePeriod)
   {
      ...
      if(theHours>0)
      {
         result+=rspfString::toString(theHours);
         result+="H";
      }
      if(theMinutes>0)
      {
         result+=rspfString::toString(theMinutes);
         result+="M";
      }
      if(theSeconds>0)
      {
         result+=rspfString::toString(theSeconds, 15);
         result+="S";
      }
   }
}

rspf_float64 rspfDuration::toSeconds()const
{
   rspf_float64 result = theSeconds;
