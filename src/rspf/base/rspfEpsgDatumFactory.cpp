...
rspfEpsgDatumFactory* rspfEpsgDatumFactory::m_instance = 0;

//*************************************************************************************************
//! Singleton implementation.
//*************************************************************************************************
rspfEpsgDatumFactory* rspfEpsgDatumFactory::instance()
{
   if(!m_instance) 
      m_instance = new rspfEpsgDatumFactory;
   return m_instance; 
} 

//*************************************************************************************************
// Hidden constructor part of singleton implementation.
// Intializes the map from EPSG numeric to RSPF/Geotrans alpha codes
...
//*************************************************************************************************
rspfEpsgDatumFactory::rspfEpsgDatumFactory()
{
...
//*************************************************************************************************
const rspfDatum* rspfEpsgDatumFactory::create(const rspfString &epsg_spec) const
{
   if ( epsg_spec.size() )
   {
      rspf_uint32 code;
      rspfString group; // assume we are working with EPSG codes
...
//*************************************************************************************************
const rspfDatum* 
rspfEpsgDatumFactory::create(const rspfKeywordlist& kwl, const char *prefix) const
{
   rspfString lookup = kwl.find(prefix, rspfKeywordNames::DATUM_KW);
   if(!lookup.empty())
...
//! Creates given an EPSG spec in the form "EPSG:<datum_code>".
//*************************************************************************************************
const rspfDatum* rspfEpsgDatumFactory::create(rspf_uint32 datum_code) const
{
   ...
   // Find the datum code in the map in order to determine the corresponding RSPF/Geotrans alpha
   // code. Then use the datum factory to create the datum.
   rspfString alphaCode = findAlphaCode(datum_code);
   if (!alphaCode.empty())
      return rspfDatumFactory::instance()->create(alphaCode);
...
//*************************************************************************************************
void rspfEpsgDatumFactory::getList(std::vector<rspfString>& list) const
{
   rspfDatumFactory* df = rspfDatumFactory::instance();
   const rspfDatum* datum;
   std::map<rspf_uint32, std::string>::const_iterator iter = m_epsgToAlphaMap.begin();

   // Loop over all handled datum codes and instantiate the corresponding datum in order to
   // get the corresponding RSPF/Geotrans name. Eventually the datum info (including its name as
   ...
   while(iter != m_epsgToAlphaMap.end())
   {
      ...
      if (datum)
      {
         rspfString entry ("EPSG:");
         entry += rspfString::toString(iter->first);
         entry += ",";
         entry += datum->name();
...
//! 0 if not found.
//*************************************************************************************************
rspf_uint32 rspfEpsgDatumFactory::findEpsgCode(const rspfString& alpha_code) const
{
   std::map<rspf_uint32, std::string>::const_iterator iter = m_epsgToAlphaMap.begin();
   while (iter != m_epsgToAlphaMap.end())
   {
...
//*************************************************************************************************
rspfString rspfEpsgDatumFactory::findAlphaCode(rspf_uint32 epsg_code) const
{
   rspfString result("");
   std::map<rspf_uint32, std::string>::const_iterator iter = m_epsgToAlphaMap.find(epsg_code);
