...
//
// DESCRIPTION:
//   Contains implementation of class rspfDblGrid. This class maintains
...
//
//*****************************************************************************
//  $Id: rspfDblGrid.cpp 20204 2011-11-04 15:12:28Z dburken $
...
static rspfTrace traceExec  ("rspfDblGrid:exec");
static rspfTrace traceDebug ("rspfDblGrid:debug");

static const rspfString MAGIC_NUMBER ("RSPF_DBL_GRID");
static const int   MAX_LENGTH_DESCR = 80;

/*!****************************************************************************
*  DEFAULT CONSTRUCTOR: rspfDblGrid
*  
*****************************************************************************/
rspfDblGrid::rspfDblGrid()
:
theGridData       (0),
theSize           (0, 0),
theOrigin         (0.0, 0.0),
theSpacing        (0.0, 0.0),
theMinValue       (RSPF_DEFAULT_MIN_PIX_DOUBLE),
theMaxValue       (RSPF_DEFAULT_MAX_PIX_DOUBLE),
theNullValue      (RSPF_DEFAULT_NULL_PIX_DOUBLE),
...
/*!****************************************************************************
*  COPY CONSTRUCTOR: rspfDblGrid
*  
*****************************************************************************/
rspfDblGrid::rspfDblGrid(const rspfDblGrid&  source)
:
theGridData   (0),
theMinValue   (RSPF_DEFAULT_MIN_PIX_DOUBLE),
theMaxValue   (RSPF_DEFAULT_MAX_PIX_DOUBLE),
...
theDomainType (CONTINUOUS)
{
   static const char MODULE[] = "rspfDblGrid Constructor";
   if (traceExec())  rspfNotify(rspfNotifyLevel_DEBUG) << MODULE << " entering...\n";
   ...
   */
   ...
   if (traceExec())  rspfNotify(rspfNotifyLevel_DEBUG) << MODULE << " returning...\n";
}

/*!****************************************************************************
*  CONSTRUCTOR: rspfDblGrid
*  
*****************************************************************************/
rspfDblGrid::rspfDblGrid(const rspfIpt&  size, 
...
*  
*****************************************************************************/
rspfDblGrid::rspfDblGrid(const rspfDrect&  rect, 
                           const rspfDpt&    spacing,
                           double             null_value)
                           :
theGridData   (0),
...
theDomainType (CONTINUOUS)
{
   ...
   rspfIpt size ((int) (rect.width()/spacing.x)  + 1,
      (int) (rect.height()/spacing.y) + 1);

   initialize(size, rect.ul(), spacing, null_value);

   if (traceExec())  rspfNotify(rspfNotifyLevel_WARN) << MODULE << " returning...\n";
}

//*****************************************************************************
//  DESTRUCTOR: ~rspfDblGrid()
//  
//*****************************************************************************
rspfDblGrid::~rspfDblGrid()
{
...
//*****************************************************************************
//  METHOD: rspfDblGrid::initialize()
//  
//  Permits initialization after construction
//  
//*****************************************************************************
void rspfDblGrid::initialize(const rspfIpt&  size, 
                              const rspfDpt&  origin,
                              const rspfDpt&  spacing,
                              double           null_value)
{
   static const char MODULE[] = "rspfDblGrid::initialize()";
   if (traceExec())  rspfNotify(rspfNotifyLevel_WARN) << MODULE << " entering...\n";
   ...
   * Allocate mem for the grid, and initialize:
   */
   rspf_uint32 buflen = theSize.x * theSize.y;
   if(buflen > 0)
   {
      theGridData = new double [buflen];

      for (rspf_uint32 i=0; i<buflen; i++)
...
//  
//*****************************************************************************
void rspfDblGrid::initialize(const rspfDrect&  uv_rect, 
                              const rspfDpt&    spacing,
                              double null_value)
{
   static const char MODULE[] = "rspfDblGrid::initialize()";
   ...
   rspfIpt size ((int) (uv_rect.width()/spacing.x)  + 1,
...
/*!****************************************************************************
* METHOD: rspfDblGrid::setNode(x, y) NON-CONST
*
*  This method is used to assign the grid data values.
*  
*****************************************************************************/
void rspfDblGrid::setNode (int x, int y, const double& input) 
{
...
/*!****************************************************************************
* METHOD: rspfDblGrid::setNearestNode(uv)
*
...
*  
*****************************************************************************/
void rspfDblGrid::setNearestNode (const rspfDpt& uv_point,
                                   const double&   input) 
{
   if(!theGridData) return;
   ...
   //***
   int xi = rspf::round<int>((uv_point.u - theOrigin.u)/theSpacing.x);
   int yi = rspf::round<int>((uv_point.v - theOrigin.v)/theSpacing.y);
...
/*!****************************************************************************
* METHOD: rspfDblGrid::getNode(x, y) CONST
*
*  This method is used to return the grid data values.
*  
*****************************************************************************/
double rspfDblGrid::getNode (int x, int y) const
{
   if(!theGridData) return theNullValue;
   if ((x>=0)&&(x<theSize.x)&&(y>=0)&&(y<theSize.y))
   {
      rspf_uint32 i = index(x, y);
...
/*!****************************************************************************
* METHOD: rspfDblGrid::operator(double, double)
*
*  This method interpolates between grid points given a fractional location
*  in UV (external) world space.
*  
*****************************************************************************/
double rspfDblGrid::operator() (const double& u, const double& v) const
{
   if(!theGridData) return theNullValue;
...
//*************************************************************************************************
double rspfDblGrid::interpolate(double xi, double yi) const
{
   if(!theGridData) 
      return theNullValue;
...
//**************************************************************************************************
//  METHOD: rspfDblGrid::extrapolate()
//  
//  Establishes bilinear extrapolation value for point outside of the grid. 
//
//**************************************************************************************************
double rspfDblGrid::extrapolate(double x, double y) const
{
   if(!theGridData) 
      return theNullValue;
...
//  
//*****************************************************************************
const rspfDblGrid& rspfDblGrid::operator = (const rspfDblGrid& source)
{
...
/*!****************************************************************************
* METHOD: rspfDblGrid::meanValue()
*  
*****************************************************************************/
double  rspfDblGrid::meanValue()
{
   static const char MODULE[] = "rspfDblGrid::meanValue()";
   if (traceExec())  rspfNotify(rspfNotifyLevel_DEBUG) << MODULE << " entering...\n";
...
/*!****************************************************************************
* METHOD: rspfDblGrid::meanStdDev()
*  
*****************************************************************************/
double  rspfDblGrid::meanStdDev()
{
   static const char MODULE[] = "rspfDblGrid::meanStdDev()";
   if (traceExec())  rspfNotify(rspfNotifyLevel_DEBUG) << MODULE << " entering...\n";
...
/*!****************************************************************************
* METHOD: rspfDblGrid::computeMean()
*  
*****************************************************************************/
void rspfDblGrid::computeMean()
{
   static const char MODULE[] = "rspfDblGrid::meanStdDev()";
   if(!theGridData) return;
   if (traceExec())  rspfNotify(rspfNotifyLevel_DEBUG) << "entering...\n";
...
/*!****************************************************************************
* INLINE METHOD: rspfDblGrid::isInside(const rspfDpt& pt) const
*****************************************************************************/
bool rspfDblGrid::isInside(const double& u, const double& v) const
{
   double xi = (u - theOrigin.u)/theSpacing.x;
   double yi = (v - theOrigin.v)/theSpacing.y;
...
//*****************************************************************************
//  METHOD: rspfDblGrid::save()
//  
//  Saves the grid to the stream in compact ASCII format (not necessarily
//  human readable).
//  
//*****************************************************************************
bool rspfDblGrid::save(ostream& os, const char* descr) const
{
   static const char MODULE[] = "rspfDblGrid::save()";
   if (traceExec())  rspfNotify(rspfNotifyLevel_DEBUG) << MODULE << " entering...\n";
...
//*****************************************************************************
//  METHOD: rspfDblGrid::load()
//  
//  Loads the grid from the stream in compact ASCII format (not necessarily
//  human readable).
//  
//*****************************************************************************
bool rspfDblGrid::load(istream& is)
{
   static const char MODULE[] = "rspfDblGrid::load()";
   if (traceExec())  rspfNotify(rspfNotifyLevel_DEBUG) << MODULE << " entering...\n";

   char strbuf[128];

   //***
   // Read magic number tag to insure it is an rspfDblGrid record:
   //***
   is >> strbuf;
   if (std::strncmp(strbuf, MAGIC_NUMBER.c_str(), MAGIC_NUMBER.length()))
   {
      rspfNotify(rspfNotifyLevel_FATAL) << MODULE << "Error reading RSPF_DBL_GRID magic number from stream. "
         << "Aborting...\n";
      return false;
   }
   is.getline(strbuf, 128, '\n');
   theSize           = rspfDpt(0,0);
   theOrigin         = rspfDpt(0,0);
   theSpacing        = rspfDpt(0,0);
   theMinValue       = RSPF_DEFAULT_MAX_PIX_DOUBLE;
   theMaxValue       =  RSPF_DEFAULT_MIN_PIX_DOUBLE;
...
//*****************************************************************************
//   METHOD: rspfDblGrid::interpolateNullValuedNodes(decay_rate)
//
...
//  
//*****************************************************************************
void rspfDblGrid::interpolateNullValuedNodes(const double& decay_rate)
{
   static const char MODULE[] = "rspfDblGrid::interpolateNullValuedNodes()";
   if (traceExec())  rspfNotify(rspfNotifyLevel_DEBUG) << MODULE << " entering...\n";
   if(!theGridData) return;
...
//*****************************************************************************
//  METHOD: rspfDblGrid::filter(size_x, size_y, kernel)
//  
...
//  
//*****************************************************************************
void rspfDblGrid::filter(int size_x, int size_y, double* kernel)
{
   static const char MODULE[] = "rspfDblGrid::filter()";
   if (traceExec())  rspfNotify(rspfNotifyLevel_DEBUG) << MODULE << "entering...\n";
   if(!theGridData) return;
   ...
   // The resampled data is accumulated and stored in a temporary rspfDblGrid object so that we
   // can take advantage of the extrapolation feature later in this method.
   rspfIpt resample_grid_size(end_x-start_x, end_y-start_y);
   rspfDpt resample_grid_origin(start_x, start_y);
   rspfDpt resample_grid_spacing(1,1);
   rspfDblGrid resample_grid(resample_grid_size, resample_grid_origin, resample_grid_spacing);
...
//*****************************************************************************
//  METHOD: rspfDblGrid::fill
//  
//  Fills the current grid with constant value provided.
//  
//*****************************************************************************
void rspfDblGrid::fill(double fill_value)
{
   if (!theGridData)
   {
      return;
...
//*************************************************************************************************
void rspfDblGrid::constrain(double& value) const
{
...
//  
//*****************************************************************************
ostream& operator<<(ostream& os, const rspfDblGrid& grid) 
{
   os << "\nDump of rspfDblGrid at " << (void*) &grid
