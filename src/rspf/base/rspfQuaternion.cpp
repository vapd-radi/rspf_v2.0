...
void rspf::Quaternion::set(const rspfMatrix4x4& matrix)
{
   matrix.getRotate(*this);
}

void rspf::Quaternion::get(rspfMatrix4x4& matrix) const
{
...
void rspf::Quaternion::makeRotate( value_type angle, value_type x, value_type y, value_type z )
{
   ...
   if (length < epsilon)
   {
      ...
      *this = rspf::Quaternion();
        return;
    }
   double tempAngle = rspf::degreesToRadians(angle);
...
void rspf::Quaternion::makeRotate ( value_type angle1, const rspfColumnVector3d& axis1, 
                                     value_type angle2, const rspfColumnVector3d& axis2,
                                     value_type angle3, const rspfColumnVector3d& axis3)
{
    rspf::Quaternion q1; q1.makeRotate(angle1,axis1);
    rspf::Quaternion q2; q2.makeRotate(angle2,axis2);
    rspf::Quaternion q3; q3.makeRotate(angle3,axis3);
    *this = q1*q2*q3;
}                        

void rspf::Quaternion::makeRotate ( value_type angle1, const rspfDpt3d& axis1, 
...
void rspf::Quaternion::makeRotate( const rspfColumnVector3d& vec1, const rspfColumnVector3d& vec2 )
{
   makeRotate(rspfDpt3d(vec1[0], vec1[1], vec1[2]),
              rspfDpt3d(vec2[0], vec2[1], vec2[2]));
}
...
// */
void rspf::Quaternion::makeRotate( const rspfDpt3d& from, const rspfDpt3d& to )
{
    ...
    rspfDpt3d sourceVector = from;
    rspfDpt3d targetVector = to;
    ...
    if (dotProdPlus1 < 1e-7) {
        ...
        const rspfDpt3d tmp = sourceVector ^ targetVector / (2.0*s);
        theVector[0] = tmp.x;
        theVector[1] = tmp.y;
        theVector[2] = tmp.z;
        theVector[3] = s;
...
// void rspf::Quaternion::makeRotate_original( const Vec3d& from, const Vec3d& to )
// {
//     const value_type epsilon = 0.0000001;
...
//     {
//         osg::notify(osg::INFO)<<"*** rspf::Quaternion::makeRotate(from,to) with near co-linear vectors, epsilon= "<<fabs(cosangle-1)<<std::endl;
...
// void rspf::Quaternion::getRotate( value_type& angle, Vec3f& vec ) const
// {
//     value_type x,y,z;
...
//     vec[2]=z;
// }
// void rspf::Quaternion::getRotate( value_type& angle, Vec3d& vec ) const
// {
//     value_type x,y,z;
//     getRotate(angle,x,y,z);
//     vec[0]=x;
...
// with a rotation!
void rspf::Quaternion::getRotate( value_type& angle, value_type& x, value_type& y, value_type& z ) const
{
    ...
    angle = rspf::radiansToDegrees(angle);
}
...
/// See also
/// http://www.gamasutra.com/features/programming/19980703/quaternions_01.htm
void rspf::Quaternion::slerp( value_type t, const rspf::Quaternion& from, const rspf::Quaternion& to )
{
