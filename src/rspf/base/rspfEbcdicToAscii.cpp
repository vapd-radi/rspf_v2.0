//----------------------------------------------------------------------------
//
// License:  See top level LICENSE.txt file.
//
// Author:  David Burken
//
// Description: Utility class to convert to/from EBCDIC/ASCII
//
// ASCII = American National Standard Code for Information Interchange
//
// EBCDIC = Extended Binary Coded Decimal Interchange Code
//
// $Id: rspfEbcdicToAscii.cpp 9094 2006-06-13 19:12:40Z dburken $
//----------------------------------------------------------------------------
#include <rspf/base/rspfEbcdicToAscii.h>

static const rspf_uint8 a2e[] =
{
   
};

static const rspf_uint8 e2a[] =
{
   
};

rspfEbcdicToAscii::rspfEbcdicToAscii()
{
}

rspfEbcdicToAscii::~rspfEbcdicToAscii()
{
}

rspf_uint8 rspfEbcdicToAscii::asciiToEbcdic(rspf_uint8 c) const
{
   return a2e[c];
   
}

rspf_uint8 rspfEbcdicToAscii::ebcdicToAscii(rspf_uint8 c) const
{
   return e2a[c];
}

void rspfEbcdicToAscii::ebcdicToAscii(char* str, rspf_uint32 size) const
{
   if (!str) return;

   for (rspf_uint32 i = 0; i < size; ++i)
   {
      str[i] =
         static_cast<char>(ebcdicToAscii(static_cast<rspf_uint8>(str[i])));
   }
}
