...
// Description:
//
// Contains class definition for Degrees Minutes Seconds (rspfDms)
//*******************************************************************
//  $Id: rspfDms.cpp 19682 2011-05-31 14:21:20Z dburken $
...
const rspf_uint8 rspfDms::theDegreeSign = 176;
const char* rspfDms::DEFAULT_FORMAT = "ddd mm.mmC";
const char* rspfDms::SPACES = "                   ";
...
rspfDms::rspfDms(double someDegrees, bool latFlag)
...
rspfDms::rspfDms(const std::string& value)
   : theDegrees(0.0),
     theLatFlag(false),
     theDecDegs(0.0),
     theAfterDot(false),
...
rspfDms& rspfDms::setDegrees(double degrees)
{
   theDegrees = degrees;
...
bool rspfDms::setDegrees(const std::string& cdegrees)
{
   bool status = true;
   
   rspfString copy = cdegrees;
...
rspfString rspfDms::toString(const rspfString& formatString)const
{
   rspfString result="";
   std::string::const_iterator stringIter;
   rspfString tempFormatString = formatString;
   if(formatString == rspfString(""))
   {
      tempFormatString = rspfDms::DEFAULT_FORMAT;
   }
         ...
         {			/* how many 'd's until not 'd' */
            ...
            {		/* beyond the decimal point */
               ...
               while (i > 0)
               {
                  ...
                  result += rspfString::toString((int)fractionalDegrees);
            ...
            else
            {
               ...
               rspfString temp = rspfString::toString(theIntDegs);
               rspfString prefix;
               d_s -= (int)temp.length();
               while(d_s > 0)
               {
...
rspfString rspfDms::degree_to_string(double degrees,
                                       char format[],
                                       bool lat_flag)const
{
   char cdegrees[64];
...
void rspfDms::calc_mins_or_secs(double *dd,
...
//        result += rspfString::toString(ires);
}
...
int rspfDms::calc_mins_or_secs(double *dd,
                                const char *format,
                                char *res)const
{
	double du;
...
void rspfDms::setup_printf(int ival, char *fmt)const
{
	char precis[3];
...
void rspfDms::set_default(char *fp, char *rp)const
{
...
void rspfDms::init_values(double d)const
{
...
double rspfDms::string_to_degree(const std::string& cdegrees)
{
