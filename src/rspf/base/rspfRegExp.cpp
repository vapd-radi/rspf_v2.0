...
#include <iostream>
// rspfRegExp -- Copies the given regular expression.

rspfRegExp::rspfRegExp (const rspfRegExp& rxp) {
   if(!rxp.program)
   {
      program = 0;
      progsize = 0;
...
bool rspfRegExp::operator== (const rspfRegExp& rxp) const {
  if (this != &rxp) {				// Same address?
    rspf_uint32 ind = this->progsize;		// Get regular expression size
...
bool rspfRegExp::deep_equal (const rspfRegExp& rxp) const {
  rspf_uint32 ind = this->progsize;		// Get regular expression size
  if (ind != rxp.progsize)			// If different size regexp
    return false;				// Return failure
  while(ind-- != 0)		 		// Else while still characters
...
void rspfRegExp::compile (const char* exp) {
    ...
    if (exp == NULL) {
      //RAISE Error, SYM(rspfRegExp), SYM(No_Expr),
      printf ("rspfRegExp::compile(): No expression supplied.\n");
      return;
    }
    ...
    if(!reg(0, &flags))
      {
	printf ("rspfRegExp::compile(): Error in compile.\n");
	return;
      }
    ...
    if (regsize >= 32767L) {	// Probably could be 65535L. 
      //RAISE Error, SYM(rspfRegExp), SYM(Expr_Too_Big),
      printf ("rspfRegExp::compile(): Expression too big.\n");
      return;
    }
...
//#endif
    ...
    if (this->program == NULL) {
      //RAISE Error, SYM(rspfRegExp), SYM(Out_Of_Memory),
      printf ("rspfRegExp::compile(): Out of memory.\n"); 
      return;
    }
 ...
 */
char* rspfRegExp::reg (int paren, int *flagp) {
    register char* ret;
    register char* br;
    register char* ender;
    ...
    *flagp = HASWIDTH;		// Tentatively.
    ...
    if (paren) {
	if (regnpar >= NSUBEXP) {
	  //RAISE Error, SYM(rspfRegExp), SYM(Too_Many_Parens),
	  printf ("rspfRegExp::compile(): Too many parentheses.\n");
	  return 0;
        }
	parno = regnpar;
	regnpar++;
	...
	*flagp |= flags & SPSTART;
    ...
    if (paren && *regparse++ != ')') {
        //RAISE Error, SYM(rspfRegExp), SYM(Unmatched_Parens),
        printf ("rspfRegExp::compile(): Unmatched parentheses.\n");
	return 0;
    }
	    ...
	    //RAISE Error, SYM(rspfRegExp), SYM(Internal_Error),
	    printf ("rspfRegExp::compile(): Internal error.\n");
	    return 0;
        }
	// NOTREACHED
    }
 ...
 */
char* rspfRegExp::regbranch (int *flagp) {
    register char* ret;
    register char* chain;
 ...
 */
char* rspfRegExp::regpiece (int *flagp) {
    register char* ret;
    register char  op;
    register char* next;
    int            flags;
	...
	*flagp = flags;
    ...
    if (!(flags & HASWIDTH) && op != '?') {
        //RAISE Error, SYM(rspfRegExp), SYM(Empty_Operand),
        printf ("rspfRegExp::compile() : *+ operand could be empty.\n");
	return 0;
    }
    *flagp = (op != '+') ? (WORST | SPSTART) : (WORST | HASWIDTH);
    ...
    regparse++;
    if (ISMULT(*regparse)) {
        //RAISE Error, SYM(rspfRegExp), SYM(Nested_Operand),
        printf ("rspfRegExp::compile(): Nested *?+.\n");
	return 0;
    }
    return (ret);
}
 ...
 */
char* rspfRegExp::regatom (int *flagp) {
    register char* ret;
             int   flags;
    ...
    *flagp = WORST;		// Tentatively.
	...
	case '[':{
		...
		while (*regparse != '\0' && *regparse != ']') {
			...
			if (*regparse == ']' || *regparse == '\0')
			    ...
			    if (rxpclass > rxpclassend + 1) {
			       //RAISE Error, SYM(rspfRegExp), SYM(Invalid_Range),
			       printf ("rspfRegExp::compile(): Invalid range in [].\n");
			       return 0;
                            }
		...
		regc('\0');
		if (*regparse != ']') {
                    //RAISE Error, SYM(rspfRegExp), SYM(Unmatched_Bracket),
                    printf ("rspfRegExp::compile(): Unmatched [].\n");
		    return 0;
	        }
		regparse++;
		*flagp |= HASWIDTH | SIMPLE;
	...
	case '+':
	case '*':
	    //RAISE Error, SYM(rspfRegExp), SYM(No_Operand),
            printf ("rspfRegExp::compile(): ?+* follows nothing.\n");
	    return 0;
	case '\\':
	    if (*regparse == '\0') {
	        //RAISE Error, SYM(rspfRegExp), SYM(Trailing_Backslash),
                printf ("rspfRegExp::compile(): Trailing backslash.\n");
		return 0;
            }
	    ret = regnode(EXACTLY);
	    regc(*regparse++);
...
/*
 - regnode - emit a node
   Location.
 */
char* rspfRegExp::regnode (char op) {
    register char* ret;
    register char* ptr;
 ...
 */
void rspfRegExp::regc (unsigned char b) {
    if (regcode != &regdummy)
	*regcode++ = b;
    else
	regsize++;
 ...
 */
void rspfRegExp::reginsert (char op, char* opnd) {
    register char* src;
    register char* dst;
    register char* place;
 ...
 */
void rspfRegExp::regtail (char* p, const char* val) {
    register char* scan;
    register char* temp;
    register int   offset;
 ...
 */
void rspfRegExp::regoptail (char* p, const char* val) {
...
bool rspfRegExp::find (const char* string) {
    ...
    if (!this->program || UCHARAT(this->program) != MAGIC) {
        //RAISE Error, SYM(rspfRegExp), SYM(Internal_Error),
        printf ("rspfRegExp::find(): Compiled regular expression corrupted.\n");
        return 0;
    }
...
/*
 - regtry - try match at specific point
   0 failure, 1 success
 */
int rspfRegExp::regtry (const char* string, const char* *start,
 ...
 * 0 failure, 1 success
 */
int rspfRegExp::regmatch (const char* prog) {
...
//		break;
	    ...
	    default:
	        //RAISE Error, SYM(rspfRegExp), SYM(Internal_Error),
                printf ("rspfRegExp::find(): Internal error -- memory corrupted.\n");
		return 0;
	}
	scan = next;
    }
    ...
    // 
    //RAISE Error, SYM(rspfRegExp), SYM(Internal_Error),
    printf ("rspfRegExp::find(): Internal error -- corrupted pointers.\n");
    return (0);
}
 ...
 */
int rspfRegExp::regrepeat (const char* p) {
	...
	default:		// Oh dear.  Called inappropriately.
	    //RAISE Error, SYM(rspfRegExp), SYM(Internal_Error),
	    printf ("rspfRegExp::find(): Internal error.\n");
	    return 0;
    }
    reginput = scan;
    return (count);
 ...
 */
const char* rspfRegExp::regnext (register const char* p) {
    register int offset;
...
char* rspfRegExp::regnext (register char* p) {
    register int offset;
