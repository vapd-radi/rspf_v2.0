...
//*******************************************************************
//  $Id: rspfLine.cpp 9094 2006-06-13 19:12:40Z dburken $
...
std::ostream& operator <<(std::ostream& out, const rspfLine& rhs)
{
   return out << "line: " << rhs.theP1 << "," << rhs.theP2 << endl;
}

rspfDpt rspfLine::intersectInfinite(const rspfLine& line)const
{
   rspfDpt result;
   rspfDpt p3 = line.theP1;
   rspfDpt p4 = line.theP2;
   ...
   if(fabs(denominator) > FLT_EPSILON)
   {
      ...
      result = rspfDpt(theP1.x + ua*(theP2.x-theP1.x),
...
rspfDpt rspfLine::intersectSegment(const rspfLine& line)const
{
   rspfDpt result;
   rspfDpt p3 = line.theP1;
   rspfDpt p4 = line.theP2;
   ...
   if(fabs(denominator) > FLT_EPSILON)
   {
      ...
      {
         rspfDpt test = rspfDpt(theP1.x + ua*(theP2.x-theP1.x),
                                  theP1.y + ua*(theP2.y-theP1.y));
         rspfDpt midPoint = line.theP1 + (line.theP2-line.theP1)*.5;
...
double rspfLine::length()const
{
   return (theP2-theP1).length();
}

rspfDpt rspfLine::midPoint()const
{
   return (theP1 + (theP2-theP1)*.5);
}

bool rspfLine::isPointWithin(const rspfDpt& point, double delta)const
{   
...
bool rspfLine::isPointOnInfiniteLine(const rspfDpt& point, double delta)const
{
   ...
   {
      rspfDpt v1  = getVector();
      v1 = v1 * (1.0/v1.length());
      rspfDpt v2  = (point - theP1);
      double s     = v1.x*v2.x + v1.y*v2.y;
      rspfDpt p   = theP1 + v1*s;
...
rspfDpt rspfLine::normal()const
{
   rspfDpt delta = getVector();
   rspfDpt result;
