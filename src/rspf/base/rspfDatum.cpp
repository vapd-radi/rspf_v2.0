...
//
//*******************************************************************
//  $Id: rspfDatum.cpp 19795 2011-06-30 15:04:48Z gpotts $
#include <rspf/base/rspfDatum.h>
#include <rspf/base/rspfGpt.h>
#include <rspf/base/rspfEllipsoid.h>
#include <rspf/base/rspfEpsgDatumFactory.h> // For accessing the EPSG codes

RTTI_DEF1(rspfDatum, "rspfDatum", rspfObject);

rspfDatum::rspfDatum(const rspfString &alpha_code, const rspfString &name,
...
{
   theEpsgCode = rspfEpsgDatumFactory::instance()->findEpsgCode(alpha_code);
};

void rspfDatum::molodenskyShift(double a,
                                 double da,
                                 double f,
                                 double df,
                                 double dx,
...
bool rspfDatum::operator==(const rspfDatum& rhs) const
{
...
bool rspfDatum::isEqualTo(const rspfObject& obj, rspfCompareType compareType)const
{
   const rspfDatum* rhs = dynamic_cast<const rspfDatum*> (&obj);
   bool result = rhs&&rspfObject::isEqualTo(obj, compareType);
   if(result)
   {
      result = ((theCode == rhs->theCode)&&
                (theEpsgCode == rhs->theEpsgCode)&&
                (theName == rhs->theName)&&
                (rspf::almostEqual(theSigmaX, rhs->theSigmaX))&&
                (rspf::almostEqual(theSigmaY, rhs->theSigmaY))&&
                (rspf::almostEqual(theSigmaZ, rhs->theSigmaZ))&&
                (rspf::almostEqual(theWestLongitude, rhs->theWestLongitude))&&
                (rspf::almostEqual(theEastLongitude, rhs->theEastLongitude))&&
                (rspf::almostEqual(theSouthLatitude, rhs->theSouthLatitude))&&
                (rspf::almostEqual(theNorthLatitude, rhs->theNorthLatitude)));
      ...
      if(result)
      {
         ...
         else if(reinterpret_cast<rspf_uint64>(theEllipsoid) | reinterpret_cast<rspf_uint64>(rhs->theEllipsoid)) // one is null
         {
            result = false;
         }
      }
