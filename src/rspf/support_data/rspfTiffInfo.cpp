...
// 
//----------------------------------------------------------------------------
// $Id: rspfTiffInfo.cpp 2673 2011-06-06 14:57:24Z david.burken $
...
static rspfTrace traceDebug("rspfTiffInfo:debug");
static rspfTrace traceDump("rspfTiffInfo:dump"); // This will dump offsets.
...
bool rspfTiffInfo::open(const rspfFilename& file)
{
   bool result = false;
   ...
   if (str.good()) 
   {
      ...
      rspfByteOrder sysByteOrder = rspf::byteOrder();
      rspfByteOrder tifByteOrder = RSPF_LITTLE_ENDIAN;
      
      if (byteOrder[0] == 'M')
      {
         tifByteOrder = RSPF_BIG_ENDIAN;
      }
...
std::ostream& rspfTiffInfo::print(std::ostream& out) const
{
   static const char MODULE[] = "rspfTiffInfo::print";
   ...
   if (traceDebug())
   {
      rspfNotify(rspfNotifyLevel_DEBUG)<< "system_byte_order: ";
      if (rspf::byteOrder() == RSPF_BIG_ENDIAN)
      {
         rspfNotify(rspfNotifyLevel_DEBUG)<< "big_endian\n";
      }
      else // RSPF_LITTLE_ENDIAN
      {
         rspfNotify(rspfNotifyLevel_DEBUG)<< "little_endian\n";
      }
      rspfNotify(rspfNotifyLevel_DEBUG)
   ...
   while(seekOffset)
   {
      ...
      //---
      rspf_uint16*  geoKeyBlock     = 0;
      rspf_uint64   geoKeyLength    = 0;
      rspf_float64* geoDoubleBlock  = 0;
      rspf_uint64   geoDoubleLength = 0;
      rspf_int8*    geoAsciiBlock   = 0;
      rspf_uint64   geoAsciiLength  = 0;
      
      //---
      // Get the number of directories within the IFD.
      //---
      rspf_uint64 nTags; // Number of tags in an IFD.
      ...
      // Tag loop:
      for (rspf_uint64 tagIdx = 0; tagIdx < nTags; ++tagIdx)
      {
         // Variables used within the loop.
         rspf_uint16   tag              = 0; // Tag number
         rspf_uint16   type             = 0; // Type(short, long...)
         rspf_uint64   count            = 0;
         rspf_uint64   arraySizeInBytes = 0; // 
         rspf_uint8*   valueArray       = 0; // To hold value.
         ...
         else
         {
            ...
            valueArray = new rspf_uint8[arraySizeInBytes];
         ...
         if (tag == OGEO_KEY_DIRECTORY_TAG)
         {
            ...
            geoKeyBlock = reinterpret_cast<rspf_uint16*>(valueArray);
            geoKeyLength = count;
         }
         else if (tag == OGEO_DOUBLE_PARAMS_TAG)
         {
            // tag 34736 save for printGeoKeys
            geoDoubleBlock  = reinterpret_cast<rspf_float64*>(valueArray);
            geoDoubleLength = count;
         }
         else if (tag == OGEO_ASCII_PARAMS_TAG)
         {
            // tag 34737 save for printGeoKeys
            geoAsciiBlock   = reinterpret_cast<rspf_int8*>(valueArray);
...
std::ostream& rspfTiffInfo::print(std::istream& inStr,
                                   std::ostream& outStr) const
{
   static const char MODULE[] = "rspfTiffInfo::print(std::istream&, std::ostream&)";
   if (traceDebug())
   {
      rspfNotify(rspfNotifyLevel_DEBUG) << MODULE << " DEBUG Entered...\n";
   }
...
bool rspfTiffInfo::getImageGeometry(rspfKeywordlist& geomKwl,
                                     rspf_uint32 entryIndex) const
{
   static const char MODULE[] = "rspfTiffInfo::getImageGeometry #1";
   if (traceDebug())
   {
      rspfNotify(rspfNotifyLevel_DEBUG) << " entered...\n";
   }
...
bool rspfTiffInfo::getImageGeometry(std::istream& inStr,
                                     rspfKeywordlist& geomKwl,
                                     rspf_uint32 entryIndex) const
{
   static const char M[] = "rspfTiffInfo::getImageGeometry #2";
   if (traceDebug())
   {
      rspfNotify(rspfNotifyLevel_DEBUG) << " entered...\n";
   }
...
void rspfTiffInfo::getImageGeometry(rspf_uint64   geoKeyLength,
                                     rspf_uint16*  geoKeyBlock,
                                     rspf_uint64   geoDoubleLength,
                                     rspf_float64* geoDoubleBlock,
                                     rspf_uint64   geoAsciiLength,
                                     rspf_int8*    geoAsciiBlock,
                                     rspfKeywordlist& geomKwl) const
{
   static const char M[] = "rspfTiffInfo::getImageGeometry #3";
   if (traceDebug())
   {
...
// Private method:
bool rspfTiffInfo::getImageGeometry(const rspfKeywordlist& gtiffKwl,
                                     rspfKeywordlist& geomKwl,
                                     rspf_uint32 entryIndex) const
{
   static const char M[] = "rspfTiffInfo::getImageGeometry #4";
   if (traceDebug())
   {
      rspfNotify(rspfNotifyLevel_DEBUG) << M << " entered...\n";
   }
   ...
   if ( traceDebug() )
   {
      rspfNotify(rspfNotifyLevel_DEBUG) << "tiffinfo dump to kwl:\n" << gtiffKwl << "\n";
   }
   
   rspfString gtiffPrefix = "tiff.image";
   gtiffPrefix += rspfString::toString(entryIndex);
   gtiffPrefix += ".";
   rspfString geomPrefix = "image";
   geomPrefix += rspfString::toString(entryIndex);
   geomPrefix += ".";
   ...
   // Get the lines.
   rspf_uint32 height = getLines(gtiffPrefix, gtiffKwl);
   if (height)
   {
      geomKwl.add(geomPrefix.c_str(),
   ...
   // Get the samples.
   rspf_uint32 width = getSamples(gtiffPrefix, gtiffKwl);
   if (width)
   {
      geomKwl.add(geomPrefix.c_str(),
   ...
   rspfString pcsCode;
   rspfString rspfProjectionName = "";
   bool hasPcsCode = getPcsCode(gtiffPrefix, gtiffKwl, pcsCode);

   //---
   // The rspfEpsgProjectionFactory will not pick up the origin latitude if code is
   ...
   {
      if ( getOssimProjectionName(gtiffPrefix, gtiffKwl, rspfProjectionName) == false )
      {
         rspfProjectionName = "rspfEquDistCylProjection";
      }
      geomKwl.add(geomPrefix.c_str(), rspfKeywordNames::TYPE_KW, rspfProjectionName);

      if ( rspfProjectionName == "rspfEquDistCylProjection" )
      {
         isGeographic = true;
      }
   }
   ...
   //--- 
   rspf_uint32 tieCount = (rspf_uint32)ties.size()/6;
   
   // Get the model transform.
   std::vector<rspf_float64> xfrm;
   ...
   if ( getOriginLat(gtiffPrefix, gtiffKwl, tmpDbl) == false )
   {
      if ( isGeographic && hasScale && scale.x )
      {
         ...
         // horizontal scaling.
         //---
         tmpDbl = rspf::acosd(scale.y/scale.x);
      }
   }
   if ( !rspf::isnan(tmpDbl) )
   {
      geomKwl.add(geomPrefix.c_str(),
...
void rspfTiffInfo::readShort(rspf_uint16& s, std::istream& str) const
{
...
void rspfTiffInfo::readLong(rspf_uint32& l, std::istream& str) const
{
...
void rspfTiffInfo::readLongLong(rspf_uint64& l, std::istream& str) const
{
   str.read((char*)&l, sizeof(l));
   if (theEndian)
   {
...
bool rspfTiffInfo::getOffset(
   std::streamoff& offset, std::istream& str, rspf_uint16 version) const
{
   bool status = true;
   if  (version == 42)
   {
      rspf_uint32   littleOffset;
      readLong(littleOffset, str);
      offset = littleOffset;
   }
   else
   {
      rspf_uint64 bigOffset;
...
bool rspfTiffInfo::getValue(rspf_uint64& value,
                             std::istream& str,
                             WordType type,
                             rspf_uint16 version) const
{
   bool status = true;
   if  (version == 42)
   {
...
rspf_uint64 rspfTiffInfo::getArraySizeInBytes(rspf_uint64 length,
                                                rspf_uint16 type) const
{
   return length * getTypeByteSize(type);
}

rspf_uint16 rspfTiffInfo::getTypeByteSize(rspf_uint16 type) const
{
   ...
   switch (type)
   {
      ...
      default:
      {
         ...
         {
            rspfNotify(rspfNotifyLevel_DEBUG)
               << "rspfTiffInfo::getTypeByteSize DEBUG:"
...
void rspfTiffInfo::eatValue(std::istream& str, rspf_uint16 version) const
{
   if (version == 42)
   {
      rspf_uint32 dummy;
...
void rspfTiffInfo::swapBytes(rspf_uint8* v,
                              rspf_uint16 type,
                              rspf_uint64 count) const
{
   if (theEndian)
   {
      rspf_uint32 byteSize = getTypeByteSize(type);
      switch(byteSize)
      {
         case 2:
...
template <class T> void rspfTiffInfo::getArrayValue(
   T& v,
   rspf_uint8* array,
   rspf_uint64 position) const
{
   T* p = (T*)array;
   v = p[position];
}

std::ostream& rspfTiffInfo::print(std::ostream& out,
                                   const std::string& prefix,
                                   rspf_uint64 tagIdx,
                                   rspf_uint16 tag,
                                   rspf_uint16 type,
                                   rspf_uint64 count,
                                   rspf_uint64 arraySizeInBytes,
                                   rspf_uint8* valueArray) const
{
   switch(tag)
   {
...
std::ostream& rspfTiffInfo::printValue(std::ostream& out,
                                        rspf_uint16 type,
                                        rspf_uint8* valueArray) const
{
   switch (type)
...
std::ostream& rspfTiffInfo::printArray(std::ostream& out,
                                        rspf_uint16 type,
                                        rspf_uint64 arraySizeInBytes,
                                        rspf_uint8* valueArray) const
{
   if (valueArray)
   {
      switch (type)
      {
         case OTIFF_BYTE:
         {
            for (rspf_uint64 i = 0; i < arraySizeInBytes; ++i)
            {
               out << ((rspf_uint8)valueArray[i]);
            }
            out << "\n";
            break;
         }
         case OTIFF_ASCII:
         {
            for (rspf_uint64 i = 0; i < arraySizeInBytes; ++i)
            {
               ...
               // rspfKeywordlist::parseStream returning false on trailing null Where array was
         ...
         case OTIFF_SHORT:
         {
            rspf_uint16* p = (rspf_uint16*)valueArray;
            for (rspf_uint64 i = 0; i < arraySizeInBytes; ++i)
            {
               out << p[i] << " ";
            }
         ...
         case OTIFF_LONG:
         {
            rspf_uint32* p = (rspf_uint32*)valueArray;
            for (rspf_uint64 i = 0; i < arraySizeInBytes; ++i)
            {
               out << p[i] << " ";
            }
         ...
         case OTIFF_DOUBLE:
         {
            rspf_float64* p = (rspf_float64*)valueArray;
            for (rspf_uint64 i = 0; i < arraySizeInBytes; ++i)
            {
               out << p[i] << " ";
            }
   ...
   {
      out << "null array passed to rspfTiffInfo::printArray method." << "\n";
   }
   return out;
}

std::ostream& rspfTiffInfo::printGdalMetadata(std::ostream& out,
                                               const std::string& prefix,
                                               rspf_uint64 count,
                                               rspf_uint8* valueArray) const
{
   rspfString xmlString(valueArray, valueArray+count);
   rspfRefPtr<rspfXmlNode> xmlNode = new rspfXmlNode();
   std::istringstream in(xmlString);
   if(xmlNode->read(in))
   {
      const rspfXmlNode::ChildListType& children = xmlNode->getChildNodes();
      
      rspf_uint32 idx = 0;
      for(idx = 0; idx < children.size(); ++idx)
      {
         out << prefix << "gdalmetadata."
             << rspfString(children[idx]->getAttributeValue("name")).downcase()
...
std::ostream& rspfTiffInfo::printGeoKeys(
   std::ostream& out,
   const std::string&  prefix,
   rspf_uint64   geoKeyLength,
   rspf_uint16*  geoKeyBlock,
   rspf_uint64   geoDoubleLength,
   rspf_float64* geoDoubleBlock,
   rspf_uint64   geoAsciiLength,
   rspf_int8*    geoAsciiBlock) const
{
   if (geoKeyLength && geoKeyBlock)
   {
      ...
      //---
      rspf_int32 index = 0;
      rspf_int32 tagCount = static_cast<rspf_int32>(geoKeyLength/4);
      for (rspf_int32 i = 0; i < tagCount; ++i)
      {
         //---
         ...
         // value or tag offset
         //---
         rspf_uint16 key   = geoKeyBlock[index++];
         rspf_uint16 tag   = geoKeyBlock[index++];
         rspf_uint16 count = geoKeyBlock[index++];
         rspf_uint16 code  = geoKeyBlock[index++];

         if (traceDebug())
         {
            rspfNotify(rspfNotifyLevel_DEBUG)
               << "DEBUG  rspfTiffInfo::printGeoKeys"
               << "\nKey index:  " << i
               << "\ngeo key:  " << key
               << "\ntag:      " << tag
               << "\ncount:    " << count
         ...
         switch (key)
         {
            ...
            {
               out << prefix << rspfKeywordNames::GCS_CODE_KW << ": "
                   << code << "\n";
               break;
            }
            ...
            case OPROJ_FALSE_EASTING_GEO_KEY:  // key 3082
            {
               if (tag == 34736) // using double array
               {
                  ...
                  if ( geoDoubleBlock && ( code < geoDoubleLength) )
                  {
                     // Always count of one.
                     out << prefix
                         << rspfKeywordNames::FALSE_EASTING_KW<< ": "
                         << geoDoubleBlock[code] << "\n";
                  }
               }
               break;
            ...
            case OPROJ_FALSE_NORTHING_GEO_KEY:  // key 3083
            {
               if (tag == 34736) // using double array
               {
                  ...
                  if ( geoDoubleBlock && ( code < geoDoubleLength) )
                  {
                     // Always count of one.
                     out << prefix
                         << rspfKeywordNames::FALSE_NORTHING_KW
                         << ": "
                         << geoDoubleBlock[code] << "\n";
                  }
               }
            ...
            case OPROJ_SCALE_AT_NAT_ORIGIN_GEO_KEY:  // key 3092
            {
               if (tag == 34736) // using double array
               {
                  ...
                  {
                     // Always count of one.
                     out << prefix << rspfKeywordNames::SCALE_FACTOR_KW
                         << ": "
                         << geoDoubleBlock[code] << "\n";
                  }
               }
      ...
      if (traceDebug())
      {
         ...
         {
            rspfNotify(rspfNotifyLevel_DEBUG)
               << "rspfTiffInfo::printGeoKeys DEBUG: geo ascii block: ";
            std::string s(geoAsciiBlock, static_cast<std::string::size_type>(geoAsciiLength));
            rspfNotify(rspfNotifyLevel_DEBUG)<< s << "\n";
         }
      } 
...
std::ostream& rspfTiffInfo::printModelType(std::ostream& out,
                                            const std::string& prefix,
                                            rspf_uint16 code) const
{
...
std::ostream& rspfTiffInfo::printRasterType(std::ostream& out,
                                             const std::string& prefix,
                                             rspf_uint16 code) const
{
...
std::ostream& rspfTiffInfo::printAngularUnits(std::ostream& out,
                                               const std::string& prefix,
                                               rspf_uint16 code) const
{
...
std::ostream& rspfTiffInfo::printCoordTrans(std::ostream& out,
                                             const std::string& prefix,
                                             rspf_uint16 code) const
{
...
std::ostream& rspfTiffInfo::printLinearUnits(std::ostream& out,
                                              const std::string& prefix,
...
std::ostream& rspfTiffInfo::printOrientation(std::ostream& out,
                                              const std::string& prefix,
                                              rspf_uint16 code) const
{
   // Tag 274:
...
void rspfTiffInfo::getDirPrefix(rspf_int32 index,
...
bool rspfTiffInfo::getPixelScale(const rspfString& gtiffPrefix,
                                  const rspfKeywordlist& gtiffKwl,
                                  rspfDpt& scale) const
{
   bool result = false;

   std::vector<rspf_float64> floats;
...
bool rspfTiffInfo::getTiePoint(const rspfString& gtiffPrefix,
                                const rspfKeywordlist& gtiffKwl,
                                std::vector<rspf_float64>& ties) const
{
   return getFloats(gtiffPrefix, MODEL_TIE_POINT_KW, gtiffKwl, ties);
}

bool rspfTiffInfo::getModelTransform(const rspfString& gtiffPrefix,
                                      const rspfKeywordlist& gtiffKwl,
                                      std::vector<rspf_float64>& xfrm) const
{
   return getFloats(gtiffPrefix, MODEL_TRANSFORM_KW, gtiffKwl, xfrm);
}

bool rspfTiffInfo::getFloats(const rspfString& gtiffPrefix,
                              const rspfString& key,
                              const rspfKeywordlist& gtiffKwl,
                              std::vector<rspf_float64>& floats) const
{
   bool result = false;
...
bool rspfTiffInfo::getFloats(const rspfString& line,
                              std::vector<rspf_float64>& floats) const
{
   bool result = false;
...
bool rspfTiffInfo::getDatumCode(const rspfString& gtiffPrefix,
                                 const rspfKeywordlist& gtiffKwl,
                                 rspfString& datumCode) const
{
   bool result = false;
   ...
   if (lookup)
   {
      rspf_int32 code = rspfString(lookup).toInt32();
   ...
   {
      // Try GCS_CODE_KW:
      lookup = gtiffKwl.find(gtiffPrefix.c_str(), rspfKeywordNames::GCS_CODE_KW);
      if ( lookup )
      {
         rspf_int32 code = rspfString(lookup).toInt32();
         result = getDatumCode( code, datumCode );
...
bool rspfTiffInfo::getDatumCode(rspf_int32 code, rspfString& datumCode) const
{
   bool result = false;
   datumCode.clear();
   switch(code)
...
bool rspfTiffInfo::getPcsCode(const rspfString& gtiffPrefix,
                               const rspfKeywordlist& gtiffKwl,
                               rspfString& pcsCode) const
{
   bool result = false;

   // Check for key "pcs_code":
   const char* lookup = gtiffKwl.find(gtiffPrefix.c_str(), rspfKeywordNames::PCS_CODE_KW);
   if (lookup)
   {
      pcsCode = lookup;
      
      rspf_uint32 code = pcsCode.toUInt32();
      if ( code != 32767 )
      {
         // See if we handle this code in our projection factories.
         rspfRefPtr<rspfProjection>  proj = 
            rspfEpsgProjectionFactory::instance()->createProjection(pcsCode);
         if (proj.valid())
         {
            proj = 0;
            result = true;
   ...
   if (result == false)
   {
      ...
      if ( lookup )
      {
         rspfString spec = lookup;

         // Strip commonly found or bar '|' from end if present.
         spec.trim(rspfString("|"));

         // See if we handle this code in our projection factories.
         rspfRefPtr<rspfProjection>  proj =
            rspfEpsgProjectionFactory::instance()->createProjection(spec);
         if (proj.valid())
         {
            rspfMapProjection* mapProj = dynamic_cast<rspfMapProjection*>(proj.get());
            if ( mapProj )
            {
               rspf_uint32 intCode = mapProj->getPcsCode();
               if ( intCode != 32767 )
               {
                  proj = 0;
                  pcsCode = rspfString::toString(intCode);
                  result = true;
               }
            }
         }
...
bool rspfTiffInfo::getUnits(const rspfString& gtiffPrefix,
                             const rspfKeywordlist& gtiffKwl,
                             rspfString& units) const
{
   bool result = true;
   
   rspfString linearUnits = "";
   getLinearUnits(gtiffPrefix, gtiffKwl, linearUnits);
   
   rspfString angularUnits = "";
   getAngularUnits(gtiffPrefix, gtiffKwl, angularUnits);

   rspfString modelType;
...
bool rspfTiffInfo::getLinearUnits(const rspfString& gtiffPrefix,
                                   const rspfKeywordlist& gtiffKwl,
                                   rspfString& linearUnits) const
{
   bool result = false;
...
bool rspfTiffInfo::getVerticalUnits(const rspfString& gtiffPrefix,
                                     const rspfKeywordlist& gtiffKwl,
                                     rspfString& verticalUnits) const
{
   bool result = false;
...
bool rspfTiffInfo::getAngularUnits(const rspfString& gtiffPrefix,
                                    const rspfKeywordlist& gtiffKwl,
                                    rspfString& angularUnits) const
{
   bool result = false;
...
bool rspfTiffInfo::getPixelType(const rspfString& gtiffPrefix,
                                 const rspfKeywordlist& gtiffKwl,
                                 rspfString& pixelType) const
{
   bool result = false;
...
bool rspfTiffInfo::getModelType(const rspfString& gtiffPrefix,
                                 const rspfKeywordlist& gtiffKwl,
                                 rspfString& modelType) const
{
   bool result = false;
...
bool rspfTiffInfo::getOssimProjectionName(const rspfString& gtiffPrefix,
                                           const rspfKeywordlist& gtiffKwl,
                                           rspfString& rspfProj) const
{
   bool result = false;
   rspfProj.clear();
   ...
   if (lookup)
   {
      rspf_uint32 code = rspfString(lookup).toUInt32();

      rspfGeoTiffCoordTransformsLut lut;
      
      rspfProj = lut.getEntryString(code);
...
rspf_uint32 rspfTiffInfo::getLines(const rspfString& gtiffPrefix,
                                     const rspfKeywordlist& gtiffKwl) const
{
...
rspf_uint32 rspfTiffInfo::getSamples(const rspfString& gtiffPrefix,
                                       const rspfKeywordlist& gtiffKwl) const
{
   rspf_uint32 result = 0;
...
bool rspfTiffInfo::getStdParallelOne(const rspfString& gtiffPrefix,
                                      const rspfKeywordlist& gtiffKwl,
                                      rspfString& value) const
{
   bool result = false;
   const char* lookup = gtiffKwl.find(gtiffPrefix.c_str(), rspfKeywordNames::STD_PARALLEL_1_KW);
   if (lookup)
   {
      value = lookup;
      result = true;
...
bool rspfTiffInfo::getStdParallelTwo(const rspfString& gtiffPrefix,
                                      const rspfKeywordlist& gtiffKwl,
                                      rspfString& value) const
{
   bool result = false;
   const char* lookup = gtiffKwl.find(gtiffPrefix.c_str(), rspfKeywordNames::STD_PARALLEL_2_KW);
   if (lookup)
   {
      value = lookup;
      result = true;
...
bool rspfTiffInfo::getFalseEasting(const rspfString& gtiffPrefix,
                                    const rspfKeywordlist& gtiffKwl,
                                    rspfString& value) const
{
   bool result = false;
   const char* lookup = gtiffKwl.find(gtiffPrefix.c_str(), rspfKeywordNames::FALSE_EASTING_KW);
   if (lookup)
   {
      value = lookup;
      result = true;
...
bool rspfTiffInfo::getFalseNorthing(const rspfString& gtiffPrefix,
                                     const rspfKeywordlist& gtiffKwl,
                                     rspfString& value) const
{
   bool result = false;
   const char* lookup = gtiffKwl.find(gtiffPrefix.c_str(), rspfKeywordNames::FALSE_NORTHING_KW);
   if (lookup)
   {
      value = lookup;
      result = true;
...
bool rspfTiffInfo::getFalseEastingNorthing(const rspfString& gtiffPrefix,
                                            const rspfKeywordlist& gtiffKwl,
...
bool rspfTiffInfo::getScaleFactor(const rspfString& gtiffPrefix,
                                   const rspfKeywordlist& gtiffKwl,
                                   rspf_float64& value) const
{
   bool result = false;

   const char* lookup =
      gtiffKwl.find(gtiffPrefix.c_str(), rspfKeywordNames::SCALE_FACTOR_KW);
   
   if (lookup)
   {
      value = rspfString(lookup).toFloat64();
      result = true;
   }
...
bool rspfTiffInfo::getOriginLat(const rspfString& gtiffPrefix,
                                 const rspfKeywordlist& gtiffKwl,
                                 rspf_float64& value) const
{
   bool result = false;
   ...
   {
      value = rspfString(projOriginLatGeoKey).toFloat64();
      result = true;
   }
   else
   {
      ...
      {
         value = rspfString(projCenterLatGeoKey).toFloat64();
         result = true;
      }
      else
      {
         ...
         if (projFalseOriginLatGeoKey)
         {
            ...
            // origin?
            //---
            value = rspfString(projFalseOriginLatGeoKey).toFloat64();
            result = true;
         }
      }
   }
...
bool rspfTiffInfo::getCentralMeridian(const rspfString& gtiffPrefix,
                                       const rspfKeywordlist& gtiffKwl,
                                       rspf_float64& value) const
{
   bool result = false;
   ...
   {
      value = rspfString(projCenterLongGeoKey).toFloat64();
      result = true;
   }
   else
   {
      ...
      {
         value = rspfString(projOriginLongGeoKey).toFloat64();
         result = true;
      }
      else
      {
         ...
         if (projFalseOriginLongGeoKey)
         {
            ...
            // origin?
            //---
            value = rspfString(projFalseOriginLongGeoKey).toFloat64();
            result = true;
         }
      }
   }
...
void rspfTiffInfo::getTieSets(const std::vector<rspf_float64>& ties,
                               rspf_uint32 width,
                               rspf_uint32 height,
                               rspfTieGptSet& tieSet) const
{
   rspf_uint32 idx = 0;
   rspf_uint32 tieCount = (rspf_uint32)ties.size()/6;
...
bool rspfTiffInfo::hasOneBasedTiePoints(
   const std::vector<rspf_float64>& ties,
   rspf_uint32 width, rspf_uint32 height) const
{
   bool result = false;
   ...
   const rspf_uint32 SIZE = (rspf_uint32)ties.size();
