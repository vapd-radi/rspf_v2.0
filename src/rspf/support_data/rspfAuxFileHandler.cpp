...
//
//********************************************************************
// $Id: rspfAuxFileHandler.cpp 2644 2011-05-26 15:20:11Z oscar.kramer $

#include <cstdio>
#include <cstdlib>
#include <iostream>

#include <rspf/support_data/rspfAuxFileHandler.h>

#include <rspf/base/rspfFilename.h>
#include <rspf/base/rspfKeywordlist.h>
using namespace std;

void rspfAuxStandardFile( int nBytes, void * pData )
...
int rspfAuxGetDataTypeBits( int nDataType )
...
static char * rspfAuxGetDictionary(rspfAuxInfo* hHFA)
...
rspfAuxFileHandler::rspfAuxFileHandler()
{
  m_auxInfo = NULL;
}
...
bool rspfAuxFileHandler::isAuxFile(const rspfFilename& file)
{
  bool result = false;
...
bool rspfAuxFileHandler::open(const rspfFilename& file)
{
  FILE	*fp = fopen(file.c_str(), "r");
  char	header[16];
  rspf_uint32	nHeaderPos;
  ...
  //Create the rspfAuxInfo                                          
  m_auxInfo = new rspfAuxInfo();
  m_auxInfo->fp = fp;
 
  //Where is the header?						
  fread( &nHeaderPos, sizeof(rspf_int32), 1, fp );

  fseek( fp, nHeaderPos, SEEK_SET );

  fread( &(m_auxInfo->nVersion), sizeof(rspf_int32), 1, fp );
 
  fread( header, 4, 1, fp ); /* skip freeList */

  fread( &(m_auxInfo->nRootPos), sizeof(rspf_int32), 1, fp );
 
  fread( &(m_auxInfo->nEntryHeaderLength), sizeof(rspf_int16), 1, fp );
  fread( &(m_auxInfo->nDictionaryPos), sizeof(rspf_int32), 1, fp );
  
  fseek( fp, 0, SEEK_END );
  m_auxInfo->nEndOfFile = (rspf_int32) ftell(fp);

  m_auxInfo->m_Root = new rspfAuxEntry( m_auxInfo, m_auxInfo->nRootPos, NULL, NULL );

  m_auxInfo->dictionaryData = rspfAuxGetDictionary(m_auxInfo);
  m_auxInfo->m_Dictionary = new rspfAuxDictionary( m_auxInfo->dictionaryData );

  rspfAuxEntry	*node = m_auxInfo->m_Root->getChild();
  while( node != NULL )
  {
    ...
    if( typeLen == strLen)
    {
      rspfAuxEntry* projEntry = node->getNamedChild("Map_Info");
      if( projEntry == NULL )
      {
         rspfAuxEntry* childEntry;
         for( childEntry = node->getChild(); childEntry != NULL && projEntry == NULL; childEntry = childEntry->getNext() )
         {
            if (rspfString(childEntry->getType()) == "Eprj_MapInfo")
      ...
      if (proj != NULL)
      {
        m_projName = rspfString(proj);
      }
      if (units != NULL)
      {
         m_unitsType = rspfString(proj);
      }

      rspfAuxEntry* datumEntry = node->getNamedChild( "Projection.Datum" );
      ...
      if (datumStr != NULL)
      {
        m_datumName = rspfString(datumStr);;
      }
      break;
    }
    node = node->getNext();
...
rspfAuxEntry::rspfAuxEntry(rspfAuxInfo * auxInfoIn, rspf_uint32 nPos,
...

{
  ...
  rspf_int32	anEntryNums[6];
 
  if( fseek( m_auxInfo->fp, nFilePos, SEEK_SET ) == -1
    || fread( anEntryNums, sizeof(rspf_uint32), 6, m_auxInfo->fp ) < 1 )
  {
    return;
  }
...
void rspfAuxEntry::setName( const char *nodeName )
{
  memset( entryNodeName, 0, 64 );
  strncpy( entryNodeName, nodeName, 64 );
}

rspfAuxEntry *rspfAuxEntry::getChild()
{
  if( m_Child == NULL && nChildPos != 0 )
  {
    m_Child = new rspfAuxEntry( m_auxInfo, nChildPos, this, NULL );
  }

  return( m_Child );
}

rspfAuxEntry* rspfAuxEntry::getNext()
{
  if( m_Next == NULL && nNextPos != 0 )
  {
    ...
    m_Next = new rspfAuxEntry(m_auxInfo, nNextPos, m_Parent, this );
  }

  return( m_Next );
}

void rspfAuxEntry::loadData()
{
...
rspfAuxEntry* rspfAuxEntry::getNamedChild( const char* name )
{
  int		nNameLen;
  rspfAuxEntry	*entry;
...
int rspfAuxEntry::getFieldValue( const char * auxFieldPath,
                            char chReqType, void *reqReturn )
{
  rspfAuxEntry	*entry = this;
...
rspf_int16 rspfAuxEntry::getIntField(const char* auxFieldPath)
{
  rspf_int16	nIntValue;
...
const char* rspfAuxEntry::getStringField( const char* auxFieldPath)
{
  char *result = NULL;
...
int rspfAuxField::extractInstValue( const char* /* auxField */, int nIndexValue,
...
int rspfAuxField::getInstCount( char * entryData, int nDataSize )
{
  if( chPointer == '\0' )
    return nItemCount;
  else if( chItemType == 'b' )
  {
    rspf_int32 nRows, nColumns;

    if( nDataSize < 20 )
      return 0;

    memcpy( &nRows, entryData+8, 4 );
    rspfAuxStandardFile( 4, &nRows );
    memcpy( &nColumns, entryData+12, 4 );
    rspfAuxStandardFile( 4, &nColumns );
  ...
  else
  {
    ...
    rspfAuxStandardFile( 4, &nCount );
    return nCount;
  }
}

void rspfAuxField::completeDefn( rspfAuxDictionary * auxDict )
...
const char *rspfAuxField::initialize( const char * fieldInput )
{
  int		i;
...
int rspfAuxField::getInstBytes( char *entryData, int nDataSize )

{
  ...
  if( chItemType == 'b' && nCount != 0 ) // BASEDATA
  {
    ...
    rspfAuxStandardFile( 2, &nBaseItemType );

    nInstBytes += 12;

    nInstBytes += 
      ((rspfAuxGetDataTypeBits(nBaseItemType) + 7) / 8) * nRows * nColumns;
  }
  else if( m_auxItemObjectType == NULL )
  {
    nInstBytes += nCount * rspfAuxDictionary::getItemSize(chItemType);
  }
  else
  {
    int		i;
...
const char* rspfAuxType::initialize( const char * typeInput )
{
  int		i;
  if( *typeInput != '{' )
  {
  ...
  {
    rspfAuxField	*newField = new rspfAuxField();

    typeInput = newField->initialize( typeInput );
    if( typeInput != NULL )
    {
      m_auxFields = (rspfAuxField **)realloc(m_auxFields, sizeof(void*) * (nFields+1) );
...
void rspfAuxType::completeDefn( rspfAuxDictionary* auxDict )
{
  int		i;
...
int rspfAuxType::extractInstValue( const char * auxFieldPath,
                          char *entryData, rspf_uint32 nDataOffset, int nDataSize,
                          char chReqType, void *reqReturn )
...
int rspfAuxType::getInstBytes( char* entryData, int nDataSize )
{
  if( nBytes >= 0 )
  {
    return( nBytes );
  ...
  else
  {
    ...
    {
      rspfAuxField	*poField = m_auxFields[iField];
...
rspfAuxDictionary::rspfAuxDictionary( const char* auxDictStr )
{
  int		i;
  nTypes = 0;
  nTypesMax = 0;
  ...
  {
    rspfAuxType* newType = new rspfAuxType();
...
rspfAuxDictionary::~rspfAuxDictionary()
{
...
void rspfAuxDictionary::addType( rspfAuxType *type )

{
  if( nTypes == nTypesMax )
  {
    nTypesMax = nTypes * 2 + 10;
    m_auxTypes = (rspfAuxType **) realloc( m_auxTypes,
...
rspfAuxType* rspfAuxDictionary::findType( const char * name )
...
int rspfAuxDictionary::getItemSize( char chType )
