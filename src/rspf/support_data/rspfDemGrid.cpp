...
//
//********************************************************************
// $Id: rspfDemGrid.cpp 9094 2006-06-13 19:12:40Z dburken $
...
rspfDemGrid::rspfDemGrid(rspf_float32 missingDataValue)
...
rspfDemGrid::~rspfDemGrid()
{
   if(_grid) delete [] _grid; _grid =0;
}

long
rspfDemGrid::read(std::istream& dem, bool incrementalRead)
{
   if (_firstTime)
   {
      dem >> _header;
...
rspfDemHeader const&
rspfDemGrid::getHeader() const
{
   return _header;
}

long
rspfDemGrid::getWidth() const
{
   return _width;
}

long
rspfDemGrid::getHeight() const
{
   return _height;
}

rspf_float32 rspfDemGrid::getElevation(long x, long y) const
{
   if (_grid == 0)
      return _missDataVal;
...
rspf_float32 rspfDemGrid::getMissingDataValue() const
{
   return _missDataVal;
}

long rspfDemGrid::fillGeographic(std::istream& dem,bool incrementalRead)
{
   if (_firstTime) {
      _curProfile = 0;
   ...
   {
      _profiles.push_back(rspfDemProfile());
      dem >> _profiles.back();
      _curProfile++;
      if (incrementalRead)
   ...
   if(_grid) delete [] _grid; _grid =0;
   _grid = new rspf_float32[_width * _height];

   rspfDemPoint sw_corner = _profiles[0].getProfileLocation();
   ...
   {
      rspfDemElevationVector const& elev = _profiles[i].getElevations();
...
long
rspfDemGrid::fillUTM(std::istream& dem, bool incrementalRead)
{
   ...
   else
   {
      ...
      rspfNotify(rspfNotifyLevel_FATAL) << "FATAL rspfDemGrid::fillUTM: Yikes! All profiles have zero elevations!\n";
      return(1);
   }
   ...
   {
      rspfDemElevationVector const& elev = _profiles[x].getElevations();
...
void
rspfDemGrid::getGroundCoords(long x, long y, double& ground_x,
                              double& ground_y)
{
