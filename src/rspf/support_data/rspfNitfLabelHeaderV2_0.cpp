...
// 
//********************************************************************
// $Id: rspfNitfLabelHeaderV2_0.cpp 9094 2006-06-13 19:12:40Z dburken $
...
RTTI_DEF1(rspfNitfLabelHeaderV2_0,
          "rspfNitfLabelHeaderV2_0",
          rspfNitfLabelHeader)

rspfNitfLabelHeaderV2_0::rspfNitfLabelHeaderV2_0()
{
   clearFields();
}

void rspfNitfLabelHeaderV2_0::parseStream(std::istream &in)
{
   if(in)
   {
      clearFields();
      ...
      if(rspfString(theLabelSecurityDowngrade) == "999998")
      {
      ...
      long dataLength = rspfString(theExtendedHeaderDataLength).toLong();
      if(dataLength > 0)
      {
...
std::ostream& rspfNitfLabelHeaderV2_0::print(std::ostream &out)const
{
...
rspf_int32 rspfNitfLabelHeaderV2_0::getDisplayLevel()const
{
   return rspfString(theLabelDisplayLevel).toInt32();
}

rspfDrect rspfNitfLabelHeaderV2_0::getImageRect()const
{
   double ulx = rspfString((const char*)(&theLabelLocation[5]),
                            (const char*)(&theLabelLocation[10])).toDouble();
   double uly = rspfString((const char*)theLabelLocation,
                            (const char*)(&theLabelLocation[5])).toDouble();

   return rspfDrect(ulx,
                     uly,
                     ulx + rspfString(theLabelCellWidth).toDouble(),
                     uly + rspfString(theLabelCellHeight).toDouble());
}

void rspfNitfLabelHeaderV2_0::clearFields()
{
