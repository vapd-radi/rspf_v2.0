...
RTTI_DEF1(rspfNitfEngrdaTag, "rspfNitfEngrdaTag", rspfNitfRegisteredTag);

rspfNitfEngrdaTag::rspfNitfEngrdaTag()
   : rspfNitfRegisteredTag(std::string("ENGRDA"), 0)
{
   clearFields();
}
...
void rspfNitfEngrdaTag::parseStream(std::istream& in)
{
   clearFields();
   ...
   const rspf_uint16 ELEMENT_COUNT = rspfString(theReCnt).toUInt16();
   
   for (rspf_uint16 i = 0; i < ELEMENT_COUNT; ++i)
   {
      rspfString os;
      char buf[TMP_BUF_SIZE];
      rspfEngDataElement element;
      ...
      element.theEngDts = static_cast<rspf_uint8>(in.get());
      ...
      buf[ENGDATC_SIZE] = '\n';
      os = buf;
      rspf_uint32 engDatC = os.toUInt32();
   ...
   } // Matches: for (rspf_uint16 i = 0; i < ELEMENT_COUNT; ++i)
}

void rspfNitfEngrdaTag::writeStream(std::ostream& out)
{
...
void rspfNitfEngrdaTag::clearFields()
{
...
std::ostream& rspfNitfEngrdaTag::print(
   std::ostream& out, const std::string& prefix) const
{
   ...
   for (rspf_uint32 i = 0; i < theData.size(); ++i)
   {
...
std::ostream& rspfNitfEngrdaTag::printData(std::ostream& out,
                                            ...
                                            const std::string& /* prefix */) const
{
   ...
   {
      std::vector<rspf_uint8>::const_iterator i = element.theEngDat.begin();
      while (i != element.theEngDat.end())
      {
         out << static_cast<rspf_int8>(*i);
         ++i;
      }
      out << "\n";
   }
   else
   {
      rspfNotify(rspfNotifyLevel_WARN)
         << "rspfNitfEngrdaTag::printData WARNING unhandled data type."
...
template <class T>
void rspfNitfEngrdaTag::getValueAsString(
   T v, rspf_uint16 w, std::string& s) const
{
