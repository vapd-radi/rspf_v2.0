...
//
//********************************************************************
// $Id: rspfDoqq.cpp 19900 2011-08-04 14:19:57Z dburken $
...
static rspfTrace traceDebug("rspfDoqq:debug");
...
// CONSTRUCTOR
//**************************************************************************
rspfDoqq::rspfDoqq(rspfFilename file)
   :  theErrorStatus(RSPF_ERROR)
{
   open(file);
   theDoqFile.close();
...
//**************************************************************************
bool rspfDoqq::open(const rspfFilename& file)
{
   // Assume all kosher:
   ...
   if(strcmp((const char*)header, "BEGIN_USGS_DOQ_HEADER") == 0)
   {
      if(traceDebug())
      {
         rspfNotify(rspfNotifyLevel_DEBUG)
            << "rspfDoqq::rspfDoqq Loading DOQ version 2 header..."
            << std::endl;
      }
   ...
   else
   {
      ...
      {
         rspfNotify(rspfNotifyLevel_DEBUG)
            << "rspfDoqq::rspfDoqq  Loading DOQ version 1 header..."
            << std::endl;
      }
...
void rspfDoqq::ldstr_v2(std::istream& in)
{
   static const char MODULE[] = "rspfDoqq::ldstr_v2(istream& in)";
			...
			(in.good()))
   {
      ...
      else if(strncmp(line, "SOURCE_IMAGE_DATE", 17) == 0)
      {
         ...
         theAcqYear  = rspfString(dum2);
         theAcqMonth = rspfString(dum3);
         theAcqDay   = rspfString(dum4);
      }
      ...
      {
         sscanf(line, "%s %s", dum1, dum2);
         theRMSE = rspfString(dum2).toDouble();
      }
...
void rspfDoqq::ldstr_v1(std::istream& in)
{
   static const char MODULE[] = "rspfDoqq::ldstr_v1(istream& in)";
   ...
   theGsd.x = std::abs(rspfString(tmp10, tmp10+GSD_SIZE).toDouble());
   in.seekg( (theRecordSize*3) + GSD_Y_OFFSET, std::ios::beg);
   in.get(tmp10, GSD_SIZE+1);
   theGsd.y = std::abs(rspfString(tmp10, tmp10+GSD_SIZE).toDouble());
   
}

rspf_float64 rspfDoqq::convertStr(const char* str) const
{
   //***
...
std::ostream& rspfDoqq::print(std::ostream& out) const
{
