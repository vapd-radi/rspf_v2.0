...
//
//********************************************************************
// $Id: rspfDtedAcc.cpp 17206 2010-04-25 23:20:40Z dburken $
...
// CONSTRUCTOR
//**************************************************************************
rspfDtedAcc::rspfDtedAcc(const rspfFilename& dted_file,
                           rspf_int32 offset)
{
   if(!dted_file.empty())
   {
      ...
      if(!dted_file.exists())
      {
         ...
         rspfNotify(rspfNotifyLevel_WARN) << "WARNING rspfDtedAcc::rspfDtedAcc: The DTED file does not exist: " << dted_file << std::endl;
         return;
      }
      ...
      {
         theErrorStatus = rspfErrorCodes::RSPF_ERROR;
         rspfNotify(rspfNotifyLevel_WARN) << "WARNING rspfDtedAcc::rspfDtedAcc: The DTED file is not readable: " << dted_file << std::endl;
         return;
      }
      ...
      {
         theErrorStatus = rspfErrorCodes::RSPF_ERROR;
         rspfNotify(rspfNotifyLevel_WARN) << "WARNING rspfDtedAcc::rspfDtedAcc: Can't open the DTED file: " << dted_file << std::endl;
         return;
      }
      in.seekg(offset);
      parse(in);
...
rspfDtedAcc::rspfDtedAcc(std::istream& in)
{
   parse(in);
}

void rspfDtedAcc::clearFields()
{
...
rspfRefPtr<rspfProperty> rspfDtedAcc::getProperty(
   const rspfString& name) const
{
   rspfRefPtr<rspfProperty> result = 0;
   if (name == "recognition_sentinel")
   {
      result = new rspfStringProperty(name, theRecSen);
      
   }
   else if (name == "absolute_ce")
   {
      result = new rspfStringProperty(name, theAbsoluteCE);
   }
   else if (name == "absolute_le")
   {
      result = new rspfStringProperty(name, theAbsoluteLE); 
   }
   else if (name == "relative_ce")
   {
      result = new rspfStringProperty(name, theRelativeCE);
   }
   else if (name == "relative_le")
   {
      result = new rspfStringProperty(name, theRelativeLE);
   }
   return result;
}

void rspfDtedAcc::getPropertyNames(
   std::vector<rspfString>& propertyNames) const
{
   propertyNames.push_back(rspfString("recognition_sentinel"));
   propertyNames.push_back(rspfString("absolute_ce"));
   propertyNames.push_back(rspfString("absolute_le"));
   propertyNames.push_back(rspfString("relative_ce"));
   propertyNames.push_back(rspfString("relative_le"));
}

std::ostream& rspfDtedAcc::print(std::ostream& out,
...
rspf_int32 rspfDtedAcc::absCE() const
{
   return (theAbsoluteCE ? atoi(theAbsoluteCE) : 0);
}

rspf_int32 rspfDtedAcc::absLE() const
{
   return (theAbsoluteLE ? atoi(theAbsoluteLE) : 0);
}

rspf_int32 rspfDtedAcc::relCE() const
{
   return (theRelativeCE ? atoi(theRelativeCE) : 0);
}

rspf_int32 rspfDtedAcc::relLE() const
{
   return (theRelativeLE ? atoi(theRelativeLE) : 0);
}
   
rspf_int32 rspfDtedAcc::startOffset() const
{
   return theStartOffset;
}

rspf_int32 rspfDtedAcc::stopOffset()  const
{
...
// operator <<
//**************************************************************************
std::ostream& operator<<( std::ostream& os, const rspfDtedAcc& acc)
{
   std::string prefix;
   return acc.print(os, prefix);
}

rspfDtedAcc::rspfDtedAcc(const rspfDtedAcc& /* source */)
{}

const rspfDtedAcc& rspfDtedAcc::operator=(const rspfDtedAcc& rhs)
{
   return rhs;
}
