...
// 
//----------------------------------------------------------------------------
// $Id: rspfNitfHistoaTag.cpp 22013 2012-12-19 17:37:20Z dburken $
...
rspfNitfHistoaProcessingEvent::rspfNitfHistoaProcessingEvent()
{
   clearFields();
}

rspf_uint32 rspfNitfHistoaProcessingEvent::getSizeInBytes()const
{
   rspf_uint32 sizeInBytes = (PDATE_SIZE + PSITE_SIZE + PAS_SIZE + NIPCOM_SIZE);
...
void rspfNitfHistoaProcessingEvent::parseStream(std::istream& in)
{
   clearFields();
   ...
   rspf_uint32 nIpcom = rspfString(m_nIpcom).toUInt32();
   if(nIpcom < 10)
   {
      rspf_uint32 idx = 0;
      for(;idx < nIpcom;++idx)
   ...
   if(rspfString(m_asymFlag).toBool())
   {
   ...
   if(rspfString(m_sharpFlag).toBool())
   {
   ...
   if(rspfString(m_ttcFlag).toBool())
   {
...
void rspfNitfHistoaProcessingEvent::writeStream(std::ostream& out)
{
   ...
   {
      for( rspf_uint32 i = 0; i < nIpcom; ++i )
      {
...
std::ostream& rspfNitfHistoaProcessingEvent::print(std::ostream& out,
                                                    const std::string& prefix) const
{
   ...
   if( ( nIpcom > 0 ) && ( nIpcom < 10 ) )
   {
      for( rspf_uint32 i = 0; i < nIpcom; ++i )
      {
         ...
         //---
         // NOTE:
         // Doing  s += rspfString::toString( i ).string() core dumping linux gcc version 4.7.0.
         //---
         std::string s2 = rspfString::toString( i ).string();
         s += s2;
         s += ":";
...
void rspfNitfHistoaProcessingEvent::clearFields()
{
...
rspfNitfHistoaTag::rspfNitfHistoaTag()
   : rspfNitfRegisteredTag( std::string("HISTOA"), 0 )
{
}

void rspfNitfHistoaTag::parseStream(std::istream& in)
{
   clearFields();
   ...
   rspf_uint32 nEvents = rspfString(m_nEvents).toUInt32();
   if(nEvents < 100)
   {
      for(rspf_uint32 idx = 0; idx < nEvents; ++idx)
      {
         rspfNitfHistoaProcessingEvent evt;
         evt.parseStream(in);
...
void rspfNitfHistoaTag::writeStream(std::ostream& out)
{
...
rspf_uint32 rspfNitfHistoaTag::getSizeInBytes()const
{
   // Make this dynamic for constructed HISTOA tags instead of using theTagLength.
   rspf_uint32 sizeInBytes = (SYSTYPE_SIZE + PC_SIZE + PE_SIZE + 
                               REMAP_FLAG_SIZE + LUTID_SIZE + NEVENTS_SIZE);

   for ( rspf_uint32 idx = 0; idx < m_eventList.size(); ++idx)
   {
...
void rspfNitfHistoaTag::clearFields()
{
...
std::ostream& rspfNitfHistoaTag::print(std::ostream& out,
                                        const std::string& prefix) const
{
   std::string pfx = prefix;
   pfx += getTagName();
   ...
   for ( rspf_uint32 idx = 0; idx < m_eventList.size(); ++idx )
   {
      m_eventList[idx].print(out, pfx+"EVENT"+rspfString::toString(idx));
   }
