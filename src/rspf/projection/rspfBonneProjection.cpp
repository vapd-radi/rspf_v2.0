...
#include <rspf/projection/rspfSinusoid.h>
#include <rspf/base/rspfKeywordNames.h>
RTTI_DEF1(rspfBonneProjection, "rspfBonneProjection", rspfMapProjection)
/***************************************************************************/
/*
 *                               DEFINES
 */
...
rspfBonneProjection::rspfBonneProjection(const rspfEllipsoid& ellipsoid,
...
void rspfBonneProjection::update()
{
...
void rspfBonneProjection::setFalseEasting(double falseEasting)
{
   Bonn_False_Easting = falseEasting;
   update();
}
void rspfBonneProjection::setFalseNorthing(double falseNorthing)
{
   Bonn_False_Northing = falseNorthing;
   update();
}
void rspfBonneProjection::setFalseEastingNorthing(double falseEasting,
...
void rspfBonneProjection::setDefaults()
{
...
rspfGpt rspfBonneProjection::inverse_do(const rspfDpt &eastingNorthing,const rspfGpt &latLon)const
{
   double lat, lon;
...
rspfGpt rspfBonneProjection::inverse(const rspfDpt &eastingNorthing)const
{
   double lat, lon;
   Convert_Bonne_To_Geodetic(eastingNorthing.x,
                             eastingNorthing.y,
...
rspfDpt rspfBonneProjection::forward(const rspfGpt &latLon)const
{
   double easting  = 0.0;
   double northing = 0.0;
   rspfGpt gpt = latLon;
...
bool rspfBonneProjection::saveState(rspfKeywordlist& kwl, const char* prefix) const
{
   return rspfMapProjection::saveState(kwl, prefix);
}
bool rspfBonneProjection::loadState(const rspfKeywordlist& kwl, const char* prefix)
{
   bool flag = rspfMapProjection::loadState(kwl, prefix);
   
   const char* type          = kwl.find(prefix, rspfKeywordNames::TYPE_KW);
   setDefaults();
   if(rspfString(type) == STATIC_TYPE_NAME(rspfBonneProjection))
   {
 ...
 *                              FUNCTIONS     
 */
long rspfBonneProjection::Set_Bonne_Parameters(double a,
                                                double f,
...
void rspfBonneProjection::Get_Bonne_Parameters(double *a,
                                                double *f,
                                                double *Origin_Latitude,
                                                double *Central_Meridian,
                                                double *False_Easting,
...
bool rspfBonneProjection::operator==(const rspfProjection& proj) const
{
   if (!rspfMapProjection::operator==(proj))
      return false;
   rspfBonneProjection* p = PTR_CAST(rspfBonneProjection, &proj);
   if (!p)
      return false;
   if (!rspf::almostEqual(Bonn_False_Easting, p->Bonn_False_Easting) ) return false;
   if (!rspf::almostEqual(Bonn_False_Northing,p->Bonn_False_Northing)) return false;
   if (!rspf::almostEqual(Bonn_Delta_Northing,p->Bonn_Delta_Northing)) return false;
   return true;
}
