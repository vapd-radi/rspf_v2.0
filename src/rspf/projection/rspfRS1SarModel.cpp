...
static rspfTrace traceDebug("rspfRS1SarModel:debug");
...
// Default.
//******************************************************************************
rspfRS1SarModel::rspfRS1SarModel()
:  theCeosData        (0),
...
//******************************************************************************
// Takes a rspfFilename& directory containing support data, initializes the model,
// and writes out the initial geometry file.
//******************************************************************************
rspfRS1SarModel::rspfRS1SarModel(const rspfFilename& imageFile)
   :
      theCeosData        (0),
      theImagingMode     (UNKNOWN_MODE)
{
   static const char MODULE[] = "Constructor rspfRS1SarModel(rspfFilename)";
   if (traceDebug())  CLOG << "entering..." << endl;

   clearErrorStatus();

   // Parse the CEOS data files:
   rspfFilename dataDirName (imageFile.path());
...
//  loadSate()
//******************************************************************************
bool rspfRS1SarModel::loadState(const rspfKeywordlist& kwl, const char* prefix)
{
   // NOT YET IMPLEMENTED
   setErrorStatus();
   return false;
...
//  saveState()
//******************************************************************************
bool rspfRS1SarModel::saveState(rspfKeywordlist& kwl, const char* prefix) const
{
   // NOT YET IMPLEMENTED
   setErrorStatus();
   return false;
...
//*************************************************************************************************
void rspfRS1SarModel::imagingRay(const rspfDpt& image_point, rspfEcefRay& image_ray) const
{
   const char* MODULE = "rspfRS1SarModel::imagingRay()";

   // Apply offset, scale, skew, and rotation to image point:
   rspfDpt offset (theImageSize.line/2.0, theImageSize.samp/2.0);
   rspfDpt ip1 = image_point - offset;
   rspfDpt ip2;
   ...
   rspfEcefPoint localORP;
   if ((theImagingMode == SCN) || (theImagingMode == SCW))
   {
      rspfGpt gpt (theLatGrid(ip1.x, ip1.y), theLonGrid(ip1.x, ip1.y), theRefHeight);
      localORP = rspfEcefPoint(gpt);
   }
   else
   {
      NEWMAT::ColumnVector orpPos(3);
      theLocalOrpInterp->interpolate(ip1.line, orpPos);
      localORP = rspfEcefPoint(orpPos[0], orpPos[1], orpPos[2]);
   }
   ...
   rspfEcefPoint ecfArpPos (arpEph[0], arpEph[1], arpEph[2]);

   theArpVelInterp->interpolate(arpTime, arpEph);
   rspfEcefVector ecfArpVel (arpEph[0], arpEph[1], arpEph[2]);

   // Need to correct the velocity vector by the earth rotational velocity:
   rspfEcefVector earthVel 
   ...
   rspfEcefVector rangeVector     = localORP - ecfArpPos;
   rspfEcefVector rangeNormal     = rangeVector.cross(ecfArpVel);
   rspfEcefVector zDirection      = rangeNormal.cross(rangeVector);
   rspfLsrSpace localVehicleSpace (rspfEcefPoint(ecfArpPos + thePosCorrection), // origin
...
//*************************************************************************************************
// PUBLIC METHOD: rspfRS1SarModel::lineSampleHeightToWorld(image_point, height, is_inside_image)
//
//  Performs image to ground projection. 
//*************************************************************************************************
void rspfRS1SarModel::lineSampleHeightToWorld(const rspfDpt& image_point,
                                               const double&   height_ellip,
                                               rspfGpt&       worldPt) const
{
   static const char MODULE[] = "rspfRS1SarModel::lineSampleHeightToWorld()";
   if (traceDebug())  CLOG << "entering..." << endl;
...
//******************************************************************************
void rspfRS1SarModel::setImagingMode(char* modeStr)
{
   static const char MODULE[] = "rspfRS1SarModel::setImagingMode(modeStr)";
   if (traceDebug())  CLOG << "entering..." << endl;
...
//******************************************************************************
void rspfRS1SarModel::initFromCeos(const rspfFilename& fname)
{
   static const char MODULE[] = "rspfRS1SarModel::initFromCeos()";
   if (traceDebug())  CLOG << "entering..." << endl;

   // Instantiate a CeosData object:
   theCeosData = new rspfCeosData(fname);
   
   if(theCeosData->errorStatus() != rspfErrorCodes::RSPF_OK) return;
...
//******************************************************************************
void rspfRS1SarModel::initAdjParms()
{
   // Adjustable model not yet implemented
#if 0

   static const char* MODULE = "rspfRS1SarModel::initAdjParms()";
   if (traceDebug())  CLOG << "entering..." << endl;
...
//******************************************************************************
void rspfRS1SarModel::establishEphemeris()
{
   static const char MODULE[] = "rspfRS1SarModel::establishEphemeris()";
   if (traceDebug())  CLOG << "entering..." << endl;
   ...
   NEWMAT::Matrix xform;
   theArpPosInterp = new rspfLagrangeInterpolator;
   theArpVelInterp = new rspfLagrangeInterpolator;
...
//******************************************************************************
void rspfRS1SarModel::eciToEcfXform(const double& acq_time, NEWMAT::Matrix& xform) const
{
   static const char MODULE[] = "rspfRS1SarModel::eciToEcfXform(acq_time)";
   if (traceDebug())  CLOG << "entering..." << endl;
   ...
   // Establish rotation:
   double cos_gha = rspf::cosd(gha);
   double sin_gha = rspf::sind(gha);

   // Establish rotation matrix:
   xform = rspfMatrix3x3::create(cos_gha,  sin_gha,  0.0,
                                 -sin_gha,  cos_gha,  0.0,
                                      0.0,      0.0,  1.0);
   if (traceDebug())
   {
...
//******************************************************************************
// PRIVATE METHOD: rspfRS1SarModel::establishOrpInterp()
//  
...
//  
//******************************************************************************
void rspfRS1SarModel::establishOrpInterp()
{
   static const char MODULE[] = "rspfRS1SarModel::establishOrpInterp()";
   if (traceDebug())  CLOG << "entering..." << endl;
   ...
   char            buf[] = "123456";
   rspfGpt localOrp (0.0, 0.0, theRefHeight);
   ...
   int index;
   theLocalOrpInterp = new rspfLagrangeInterpolator;
   for (index=0; index<NUM_DATA_POINTS; index++)
   {
      ...
      theLocalOrpInterp->addData(line_number, rspfEcefPoint(localOrp).toVector());
...
//  space at the ARP.
//******************************************************************************
void rspfRS1SarModel::establishVehicleSpace()
{
   static const char MODULE[] = "rspfRS1SarModel::establishVehicleSpace()";
   if (traceDebug())  CLOG << "entering..." << endl;
   ...
   rspfEcefPoint ecfArpPos  (xform * arpPos);
   rspfEcefVector ecfArpVel (xform * arpVel);

   // Need to correct the velocity vector by the earth rotational velocity:
   rspfEcefVector earthVel(-ecfArpPos.y()*EARTH_ANGULAR_VELOCITY,
                             ecfArpPos.x()*EARTH_ANGULAR_VELOCITY,
                             0.0);
   ecfArpVel = ecfArpVel - earthVel;

   // Establish LSR space for intrack-crosstrack-radial at vehicle:
   rspfEcefVector intrackDir (ecfArpVel);
   rspfEcefVector crtrackDir (ecfArpPos.data().cross(intrackDir.data())); 
   theVehicleSpace = rspfLsrSpace (ecfArpPos, intrackDir, crtrackDir, 0);
...
//******************************************************************************
void rspfRS1SarModel::interpolatedScanORP(const rspfDpt& orp, rspfEcefPoint& orp_ecf) const
{
   static const char MODULE[] = "rspfRS1SarModel::interpolatedScanORP(gDblPoint)";
   if (traceDebug())  CLOG << "entering..." << endl;
...
//******************************************************************************
// PRIVATE METHOD: rspfRS1SarModel::establishOrpGrid()
//
...
//
//******************************************************************************
void rspfRS1SarModel::establishOrpGrid()
{
   static const char MODULE[] = "rspfRS1SarModel::establishOrpGrid()";
   if (traceDebug())  CLOG << "entering..." << endl;
   ...
   int             headerSize    = sizeOfDescRec + sizeOfPrefRec;
   char            buf[] = "123456";
   rspfDpt gridSize ((double) NUM_GRID_POINTS_U, 3.0);
   rspfDpt cellSize (theImageSize.line/gridSize.u, theImageSize.samp/gridSize.v);
   ...
   theLatGrid.initialize(gridSize, rspfDpt(0,0), rspfDpt(1,1));
   theLonGrid.initialize(gridSize, rspfDpt(0,0), rspfDpt(1,1));
...
//  
//******************************************************************************
void rspfRS1SarModel::deallocateMemory()
{
   static const char MODULE[] = "rspfRS1SarModel::deallocateMemory()";
   if (traceDebug())  CLOG << "entering..." << endl;
