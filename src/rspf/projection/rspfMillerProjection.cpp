...
RTTI_DEF1(rspfMillerProjection, "rspfMillerProjection", rspfMapProjection)
/***************************************************************************/
/*
 *                               DEFINES
 */
...
rspfMillerProjection::rspfMillerProjection(const rspfEllipsoid& ellipsoid,
...
void rspfMillerProjection::update()
{
...
void rspfMillerProjection::setFalseEasting(double falseEasting)
{
   Mill_False_Easting = falseEasting;
   update();
}
void rspfMillerProjection::setFalseNorthing(double falseNorthing)
{
   Mill_False_Northing = falseNorthing;
   update();
}
void rspfMillerProjection::setDefaults()
{
...
void rspfMillerProjection::setCentralMeridian(double centralMeridian)
{
  Mill_Origin_Long = centralMeridian;
  update();
}
void rspfMillerProjection::setFalseEastingNorthing(double falseEasting,
                                                    double falseNorthing)
{
   Mill_False_Easting  = falseEasting;
   Mill_False_Northing = falseNorthing;
...
rspfGpt rspfMillerProjection::inverse_do(const rspfDpt &eastingNorthing,const rspfGpt &latLon)const
{
   double lat = 0.0;
   double lon = 0.0;
...
rspfGpt rspfMillerProjection::inverse(const rspfDpt &eastingNorthing)const
{
   double lat = 0.0;
   double lon = 0.0;
   Convert_Miller_To_Geodetic(eastingNorthing.x,
...
rspfDpt rspfMillerProjection::forward(const rspfGpt &latLon)const
{
   double easting  = 0.0;
   double northing = 0.0;
   rspfGpt gpt = latLon;
...
bool rspfMillerProjection::saveState(rspfKeywordlist& kwl, const char* prefix) const
{
   return rspfMapProjection::saveState(kwl, prefix);
}
bool rspfMillerProjection::loadState(const rspfKeywordlist& kwl,
                                      const char* prefix)
{
   bool flag = rspfMapProjection::loadState(kwl, prefix);
   const char* type          = kwl.find(prefix, rspfKeywordNames::TYPE_KW);
   setDefaults();
   if(rspfString(type) == STATIC_TYPE_NAME(rspfMillerProjection))
   {
 ...
 *                              FUNCTIONS
 */
long rspfMillerProjection::Set_Miller_Parameters(double a,
                                                  double f,
...
void rspfMillerProjection::Get_Miller_Parameters(double *a,
                                                  double *f,
                                                  double *Central_Meridian,
                                                  double *False_Easting,
...
long rspfMillerProjection::Convert_Geodetic_To_Miller (double Latitude,
                                                        double Longitude,
                                                        double *Easting,
...
long rspfMillerProjection::Convert_Miller_To_Geodetic(double Easting,
                                                       double Northing,
                                                       double *Latitude,
