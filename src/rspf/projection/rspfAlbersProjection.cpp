...
RTTI_DEF1(rspfAlbersProjection, "rspfAlbersProjection", rspfMapProjection)
#ifdef PI_OVER_2
#  undef PI_OVER_2
#endif   
...
rspfAlbersProjection::rspfAlbersProjection(const rspfEllipsoid& ellipsoid,
...
rspfObject* rspfAlbersProjection::dup() const
{
   return new rspfAlbersProjection(*this);
}
rspfAlbersProjection::~rspfAlbersProjection()
{
}
void rspfAlbersProjection::update()
{
...
rspfGpt rspfAlbersProjection::inverse(const rspfDpt &eastingNorthing)const
{
   double lat, lon;
...
rspfGpt rspfAlbersProjection::inverse_do(const rspfDpt &eastingNorthing,const rspfGpt &latLon)const
{
   double lat, lon;
...
rspfDpt rspfAlbersProjection::forward(const rspfGpt &latLon)const
{
   double easting  = 0.0;
   double northing = 0.0;
   rspfGpt gpt = latLon;
...
void rspfAlbersProjection::setStandardParallel1(double degree)
{
   Albers_Std_Parallel_1 = degree*RAD_PER_DEG;
   update();
}
void rspfAlbersProjection::setStandardParallel2(double degree)
{
   Albers_Std_Parallel_2 = degree*RAD_PER_DEG;
   update();
}
void rspfAlbersProjection::setStandardParallels(double parallel1Degree,
...
void rspfAlbersProjection::setFalseEasting(double falseEasting)
{
   Albers_False_Easting = falseEasting;
   update();
}
void rspfAlbersProjection::setFalseNorthing(double falseNorthing)
{
   Albers_False_Northing = falseNorthing;
   update();
}
void rspfAlbersProjection::setFalseEastingNorthing(double falseEasting,
                                                    double falseNorthing)
{
   Albers_False_Easting = falseEasting;
   Albers_False_Northing = falseNorthing;  
...
void rspfAlbersProjection::setParameters(double parallel1,
                                          double parallel2,
                                          double falseEasting,
                                          double falseNorthing)
{
...
double rspfAlbersProjection::getStandardParallel1()const
{
   return  Albers_Std_Parallel_1/RAD_PER_DEG;
}
double rspfAlbersProjection::getStandardParallel2()const
{
   return  Albers_Std_Parallel_2/RAD_PER_DEG;
}
void rspfAlbersProjection::setDefaults()
{
...
bool rspfAlbersProjection::saveState(rspfKeywordlist& kwl, const char* prefix) const
{
   kwl.add(prefix,
...
bool rspfAlbersProjection::loadState(const rspfKeywordlist& kwl, const char* prefix)
{
   ...
   if(rspfString(type) == STATIC_TYPE_NAME(rspfAlbersProjection))
   {
      Albers_False_Easting  = theFalseEastingNorthing.x;
      Albers_False_Northing = theFalseEastingNorthing.y;
      if(stdParallel1)
      {
         Albers_Std_Parallel_1 = rspfString(stdParallel1).toDouble()*RAD_PER_DEG;
      }
      else
      {
         Albers_Std_Parallel_1 = 29.5*RAD_PER_DEG;         
      ...
      if(stdParallel2)
      {
         Albers_Std_Parallel_2 = rspfString(stdParallel2).toDouble()*RAD_PER_DEG;
      }
      else
      {
         Albers_Std_Parallel_2 = 45.5*RAD_PER_DEG;
 ...
 *                              FUNCTIONS
 */
long rspfAlbersProjection::Set_Albers_Parameters(double a,
                                                  double f,
...
void rspfAlbersProjection::Get_Albers_Parameters(double *a,
                                                  double *f,
                                                  double *Origin_Latitude,
                                                  double *Central_Meridian,
                                                  double *Std_Parallel_1,
...
long rspfAlbersProjection::Convert_Geodetic_To_Albers (double Latitude,
                                                        double Longitude,
                                                        double *Easting,
...
long rspfAlbersProjection::Convert_Albers_To_Geodetic(double Easting,
                                                       double Northing,
                                                       double *Latitude,
...
bool rspfAlbersProjection::operator==(const rspfProjection& proj) const
{
   if (!rspfMapProjection::operator==(proj)) return false;
   rspfAlbersProjection* p = PTR_CAST(rspfAlbersProjection, &proj);
   if (!p) return false;
   if (!rspf::almostEqual(Albers_Std_Parallel_1,p->Albers_Std_Parallel_1)) return false;
   if (!rspf::almostEqual(Albers_Std_Parallel_2,p->Albers_Std_Parallel_2)) return false;
   if (!rspf::almostEqual(Albers_Delta_Northing,p->Albers_Delta_Northing)) return false;
   if (!rspf::almostEqual(Albers_Delta_Easting, p->Albers_Delta_Easting) ) return false;
   return true;
}
