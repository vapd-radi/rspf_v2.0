...
//*******************************************************************
//  $Id$
#include <rspf/projection/rspfPpjFrameSensor.h>
...
static rspfTrace traceExec ("rspfPpjFrameSensor:exec");
static rspfTrace traceDebug("rspfPpjFrameSensor:debug");

RTTI_DEF1(rspfPpjFrameSensor, "rspfPpjFrameSensor", rspfSensorModel);
...
rspfPpjFrameSensor::rspfPpjFrameSensor()
   :
   m_ecef2Cam(),
   m_ecef2CamInverse(),
   ...
   m_averageProjectedHeight(0.0)
{
   if (traceDebug())
   {
      rspfNotify(rspfNotifyLevel_DEBUG)
         << "rspfPpjFrameSensor::rspfPpjFrameSensor DEBUG:" << std::endl;
   }
...
rspfPpjFrameSensor::rspfPpjFrameSensor(const rspfPpjFrameSensor& src)
   :
   rspfSensorModel(src),
...
rspfObject* rspfPpjFrameSensor::dup()const
{
   return new rspfPpjFrameSensor(*this);
}
   
void rspfPpjFrameSensor::setFocalLength(double focX, double focY)
{
   m_focalLengthX = focX;
   m_focalLengthY = focY;
...
void rspfPpjFrameSensor::setCameraPosition(const rspfGpt& value)
{
   m_cameraPositionEllipsoid = value;
   m_ecefCameraPosition = value;
}
   
void rspfPpjFrameSensor::setPrincipalPoint(const rspfDpt& value)
{
   m_principalPoint = value;
}

void rspfPpjFrameSensor::setecef2CamMatrix(const NEWMAT::Matrix& value)
{
   m_ecef2Cam = value;
   m_ecef2CamInverse = m_ecef2Cam.t();
}

void rspfPpjFrameSensor::setAveragePrjectedHeight(double averageProjectedHeight)
{
   m_averageProjectedHeight = averageProjectedHeight;
}

void rspfPpjFrameSensor::imagingRay(const rspfDpt& imagePoint,
                                     rspfEcefRay& imageRay) const
{
   // Form camera frame LOS vector
   rspfColumnVector3d camLOS(imagePoint.x - m_principalPoint.x,
                              imagePoint.y - m_principalPoint.y,
                              m_adjustedFocalLength);   

   // Rotate to ECF
   rspfColumnVector3d ecfLOS = m_ecef2CamInverse * camLOS;
   ...
   {
      rspfNotify(rspfNotifyLevel_DEBUG)
         << "rspfPpjFrameSensor::imagingRay DEBUG:\n"
         << "  camLOS = " << camLOS << "\n"
...
void rspfPpjFrameSensor::lineSampleToWorld(const rspfDpt& imagePoint,
                                                rspfGpt& worldPt) const
{
   rspfEcefRay ray;
   imagingRay(imagePoint, ray);
   rspfElevManager::instance()->intersectRay(ray, worldPt, m_averageProjectedHeight);
}  

void rspfPpjFrameSensor::lineSampleHeightToWorld(const rspfDpt& imagePoint,
                                                  const double&   heightEllipsoid,
                                                        rspfGpt& worldPt) const
{
   rspfEcefRay ray;
   imagingRay(imagePoint, ray);
   double h = (rspf::isnan(heightEllipsoid)||rspf::almostEqual(heightEllipsoid, 0.0))?m_averageProjectedHeight:heightEllipsoid;
   rspfEcefPoint pecf(ray.intersectAboveEarthEllipsoid(h));
   worldPt = rspfGpt(pecf);

   if (traceDebug())
   {
      rspfNotify(rspfNotifyLevel_DEBUG) << "rspfPpjFrameSensor::lineSampleHeightToWorld DEBUG:" << std::endl;
...
void rspfPpjFrameSensor::worldToLineSample(const rspfGpt& world_point,
                                             rspfDpt&       image_point) const
{   
   rspfGpt wpt = world_point;
   if(wpt.isHgtNan())
   {
      wpt.height(m_averageProjectedHeight);
   }
   rspfEcefPoint gnd_ecf(wpt);
   rspfEcefPoint cam_ecf(m_adjustedCameraPosition);
   rspfEcefVector ecfRay(gnd_ecf - cam_ecf);
   rspfColumnVector3d camRay(m_ecef2Cam*ecfRay.data());   
...
void rspfPpjFrameSensor::updateModel()
{
   if (traceDebug())
   {
      rspfNotify(rspfNotifyLevel_DEBUG)
         << "rspfPpjFrameSensor::updateModel DEBUG:" << std::endl;
   }
   ...
   m_adjustedCameraPosition = rspfGpt(m_cameraPositionEllipsoid.latd()   + deltap,
   ...
   {
      rspfNotify(rspfNotifyLevel_DEBUG)
         << "rspfPpjFrameSensor::updateModel complete..." << std::endl;
   }
   /*
   rspfGpt gpt;
...
void rspfPpjFrameSensor::initAdjustableParameters()
{
   if (traceExec())
      rspfNotify(rspfNotifyLevel_DEBUG) << "DEBUG rspfPpjFrameSensor::initAdjustableParameters: returning..." << std::endl;
...
bool rspfPpjFrameSensor::loadState(const rspfKeywordlist& kwl, const char* prefix)
{
   if (traceDebug())
   {
      rspfNotify(rspfNotifyLevel_DEBUG)
         << "rspfPpjFrameSensor::loadState DEBUG:" << std::endl;
   }
   ...
   rspfString focal_length_x = kwl.find(prefix, "focal_length_x");
   rspfString focal_length_y = kwl.find(prefix, "focal_length_y");
   rspfString number_samples = kwl.find(prefix, "number_samples");
   rspfString number_lines = kwl.find(prefix, "number_lines");
   rspfString ecf_to_cam_row1 = kwl.find(prefix, "ecf_to_cam_row1");
   rspfString ecf_to_cam_row2 = kwl.find(prefix, "ecf_to_cam_row2");
   rspfString ecf_to_cam_row3 = kwl.find(prefix, "ecf_to_cam_row3");
   rspfString platform_position = kwl.find(prefix, "ecef_camera_position");
   rspfString averageProjectedHeight = kwl.find(prefix, "average_projected_height");
   ...
   if(!platform_position.empty())
   {
      ...
      m_cameraPositionEllipsoid = rspfGpt(m_ecefCameraPosition);
   }
   ...
   if(theGSD.isNan())
   {
      ...
      catch (const rspfException& e)
      {
         ...
         {
            rspfNotify(rspfNotifyLevel_DEBUG)
            << "rspfPpjFrameSensor::loadState Caught Exception:\n"
            << e.what() << std::endl;
         }
      }
   }
   ...
   {
      rspfNotify(rspfNotifyLevel_DEBUG)
         << "rspfPpjFrameSensor::loadState complete..." << std::endl;
   }
   
   return result;
}

bool rspfPpjFrameSensor::saveState(rspfKeywordlist& kwl, const char* prefix)const
{
   ...
   kwl.add(prefix, "focal_length_x", rspfString::toString(m_focalLengthX) ,true);
   kwl.add(prefix, "focal_length_y", rspfString::toString(m_focalLengthY) ,true);
   ...
   rspfString row1 = rspfString::toString(m_ecef2Cam[0][0]) + " " +
                      rspfString::toString(m_ecef2Cam[0][1]) + " " +
                      rspfString::toString(m_ecef2Cam[0][2]);
   kwl.add(prefix, "ecf_to_cam_row1", row1, true);
   rspfString row2 = rspfString::toString(m_ecef2Cam[1][0]) + " " +
                      rspfString::toString(m_ecef2Cam[1][1]) + " " +
                      rspfString::toString(m_ecef2Cam[1][2]);
   kwl.add(prefix, "ecf_to_cam_row2", row2, true);
   rspfString row3 = rspfString::toString(m_ecef2Cam[2][0]) + " " +
                      rspfString::toString(m_ecef2Cam[2][1]) + " " +
                      rspfString::toString(m_ecef2Cam[2][2]);
