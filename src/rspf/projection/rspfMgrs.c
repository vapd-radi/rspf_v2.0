...
#define RSPF_MGRS_LETTERS            3  /* NUMBER OF LETTERS IN MGRS              */
...
#define RSPF_MGRS_ZONE_AND_LETTERS   5  /* NUM. OF CHARS. IN ZONE AND LETTERS     */
#define RSPF_MGRS_MINIMUM            9  /* MINIMUM NUMBER OF CHARS FOR MGRS       */
#define RSPF_MGRS_MAXIMUM           15  /* MAXIMUM NUMBER OF CHARS FOR MGRS       */
...
double RSPF_MGRS_a = 6378137.0;    /* Semi-major axis of ellipsoid in meters */
double RSPF_MGRS_f = 1 / 298.257223563; /* Flattening of ellipsoid           */
double RSPF_MGRS_recpf = 298.257223563;
char   RSPF_MGRS_Ellipsoid_Code[3] = {'W','E',0};
const char* RSPF_CLARKE_1866 = "CC";
const char* RSPF_CLARKE_1880 = "CD";
const char* RSPF_BESSEL_1841 = "BR";
/* 
 *    RSPF_CLARKE_1866 : Ellipsoid code for RSPF_CLARKE_1866
 *    RSPF_CLARKE_1880 : Ellipsoid code for RSPF_CLARKE_1880
 *    RSPF_BESSEL_1841 : Ellipsoid code for RSPF_BESSEL_1841
 */
/***************************************************************************/
/*
 *                              FUNCTIONS     
...
{ /* BEGIN UTMSET */
  ...
  igroup = 1;
  if (!strcmp(RSPF_MGRS_Ellipsoid_Code,RSPF_CLARKE_1866) || !strcmp(RSPF_MGRS_Ellipsoid_Code, RSPF_CLARKE_1880) || !strcmp(RSPF_MGRS_Ellipsoid_Code,RSPF_BESSEL_1841))
  {
    igroup = 2;
  }
  else if (!strcmp(RSPF_MGRS_Ellipsoid_Code, RSPF_CLARKE_1866) && (izone >= 47) && (izone <= 55 ))
  {
    igroup = 1;
  }
  if ((iset == 1) || (iset == 4))
...
{ /* BEGIN UTMMGRS */
  ...
  */
  Set_UTM_Parameters(RSPF_MGRS_a,RSPF_MGRS_f,izone);
...
{ /* BEGIN GRID_UPS */
  ...
  izone = 0;
  sphi = spsou;
  Set_UPS_Parameters(RSPF_MGRS_a,RSPF_MGRS_f);
...
{ /* BEGIN LTR2UTM */
   ...
   *    yltr   : Northing for 100,000 meter grid square.
   *    ierr   : Error code.
   *    ltrnum : Values of the letters in the RSPF_MGRS coordinate.
   *    ltrlow : Low number.
   *    ltrhi  : High number.
   *    fnltr  : False northing for 3rd letter.
...
/*  double slam;*/
  ...
  Set_UTM_Parameters(RSPF_MGRS_a,RSPF_MGRS_f,*Zone);
...
long Round_RSPF_MGRS (double value)
...
long Make_RSPF_MGRS_String (char* RSPF_MGRS, 
                       long Zone, 
                       long ltrnum[RSPF_MGRS_LETTERS], 
                       double Easting, 
                       double Northing,
                       long Precision)
/* Construct an RSPF_MGRS string from its component parts */
{ /* Make_RSPF_MGRS_String */
  int i;
  int j;
  int error_code = RSPF_MGRS_NO_ERROR;
  double divisor;
  long int east;  /* double data type? (drb) */
  long int north; /* double data type? (drb) */
  i = 0;
  if (Zone)
    i = sprintf (RSPF_MGRS+i,"%2.2ld",Zone);
  for (j=0;j<3;j++)
    RSPF_MGRS[i++] = ALBET[ltrnum[j]];
  divisor = pow (10.0, (5 - Precision));
  Easting = fmod (Easting, 100000.0);
  if (Easting >= 99999.5)
    Easting = 99999.0;
  east = (long int)(Easting/divisor);
  i += sprintf (RSPF_MGRS+i, "%*.*ld", (int)Precision, (int)Precision, east);
  Northing = fmod (Northing, 100000.0);
  if (Northing >= 99999.5)
    Northing = 99999.0;
  north = (long int)(Northing/divisor);
  i += sprintf (RSPF_MGRS+i, "%*.*ld", (int)Precision, (int)Precision, north);
  return (error_code);
} /* Make_RSPF_MGRS_String */
long Break_RSPF_MGRS_String (const char* RSPF_MGRS,
                        long* Zone,
                        long Letters[RSPF_MGRS_LETTERS],
                        double* Easting,
                        double* Northing,
                        long* Precision)
/* Break down an RSPF_MGRS string into its component parts */
{ /* Break_RSPF_MGRS_String */
  long error_code = RSPF_MGRS_NO_ERROR;
  long i = 0;
  long j;
  long num_digits;
  long num_letters;
  ...
  if (num_digits <= 2)
    if (num_digits > 0)
    {
      char zone_string[3];
      /* get zone */
      strncpy (zone_string, RSPF_MGRS+j, 2);
      zone_string[2] = 0;
      sscanf (zone_string, "%ld", Zone);  
      if ((*Zone < 1) || (*Zone > 60))
        error_code |= RSPF_MGRS_STRING_ERROR;
    }
    else
      *Zone = 0;
  else
    error_code |= RSPF_MGRS_STRING_ERROR;
  j = i;
  while (isalpha(RSPF_MGRS[i]))
    i++;
  num_letters = i - j;
  if (num_letters == 3)
  {
    /* get letters */
    Letters[0] = (toupper(RSPF_MGRS[j]) - (long)'A');
    if ((Letters[0] == LETTER_I) || (Letters[0] == LETTER_O))
      error_code |= RSPF_MGRS_STRING_ERROR;
    Letters[1] = (toupper(RSPF_MGRS[j+1]) - (long)'A');
    if ((Letters[1] == LETTER_I) || (Letters[1] == LETTER_O))
      error_code |= RSPF_MGRS_STRING_ERROR;
    Letters[2] = (toupper(RSPF_MGRS[j+2]) - (long)'A');
  ...
  if ((num_digits <= 10) && (num_digits%2 == 0))
  {
    ...
    *Precision = n;
    if (n > 0)
    {
      strncpy (east_string, RSPF_MGRS+j, n);
      east_string[n] = 0;
      sscanf (east_string, "%ld", &east);
      strncpy (north_string, RSPF_MGRS+j+n, n);
      north_string[n] = 0;
...
long Set_RSPF_MGRS_Parameters (double a,
                          double f,
                          const char* Ellipsoid_Code)
/*
 * The function SET_RSPF_MGRS_PARAMETERS receives the ellipsoid parameters and sets
 * the corresponding state variables. If any errors occur, the error code(s)
 * are returned by the function, otherwise RSPF_MGRS_NO_ERROR is returned.
 *
 *   a                : Semi-major axis of ellipsoid in meters  (input)
 *   f                : Flattening of ellipsoid                 (input)
 *   Ellipsoid_Code   : 2-letter code for ellipsoid             (input)
 */
{ /* Set_RSPF_MGRS_Parameters  */
  double inv_f = 1 / f;
  long Error_Code = RSPF_MGRS_NO_ERROR;
  if (a <= 0.0)
  { /* Semi-major axis must be greater than zero */
    Error_Code |= RSPF_MGRS_A_ERROR;
  }
  if ((inv_f < 250) || (inv_f > 350))
  { /* Inverse flattening must be between 250 and 350 */
    Error_Code |= RSPF_MGRS_INV_F_ERROR;
  }
  if (!Error_Code)
  { /* no errors */
    RSPF_MGRS_a = a;
    RSPF_MGRS_f = f;
    RSPF_MGRS_recpf = inv_f;
    strcpy (RSPF_MGRS_Ellipsoid_Code, Ellipsoid_Code);
  }
  return (Error_Code);
}  /* Set_RSPF_MGRS_Parameters  */
void Get_RSPF_MGRS_Parameters (double *a,
                          double *f,
                          char* Ellipsoid_Code)
/*
 * The function Get_RSPF_MGRS_Parameters returns the current ellipsoid
 * parameters.
 *
 ...
 */
{ /* Get_RSPF_MGRS_Parameters */
  *a = RSPF_MGRS_a;
  *f = RSPF_MGRS_f;
  strcpy (Ellipsoid_Code, RSPF_MGRS_Ellipsoid_Code);
  return;
} /* Get_RSPF_MGRS_Parameters */
long Convert_Geodetic_To_RSPF_MGRS (double Latitude,
                               double Longitude,
                               long Precision,
                               char* RSPF_MGRS)
/*
 ...
 *    RSPF_MGRS       : RSPF_MGRS coordinate string           (output)
 *  
 */
{ /* Convert_Geodetic_To_RSPF_MGRS */
  long error_code = RSPF_MGRS_NO_ERROR;
  long zone;
  char hemisphere;
  double easting;
  ...
  { /* Latitude out of range */
    error_code |= RSPF_MGRS_LAT_ERROR;
  }
  if ((Longitude < -PI) || (Longitude > (2*PI)))
  { /* Longitude out of range */
    error_code |= RSPF_MGRS_LON_ERROR;
  }
  if ((Precision < 0) || (Precision > MAX_PRECISION))
    error_code |= RSPF_MGRS_PRECISION_ERROR;
  if (!error_code)
  {
    if ((Latitude < MIN_UTM_LAT) || (Latitude > MAX_UTM_LAT))
    {
      ...
      error_code |= Convert_UPS_To_RSPF_MGRS (hemisphere, easting, northing, Precision, RSPF_MGRS);
    }
    else
    {
      Set_UTM_Parameters (RSPF_MGRS_a, RSPF_MGRS_f, 0);
      error_code |= Convert_Geodetic_To_UTM (Latitude, Longitude, &zone, &hemisphere, &easting, &northing);
      error_code |= Convert_UTM_To_RSPF_MGRS (zone, hemisphere, easting, northing, Precision, RSPF_MGRS);
    }
  }
  return (error_code);
} /* Convert_Geodetic_To_RSPF_MGRS */
long Convert_RSPF_MGRS_To_Geodetic (const char* RSPF_MGRS, 
                               double *Latitude, 
                               double *Longitude)
/*
 *    RSPF_MGRS       : RSPF_MGRS coordinate string           (output)
...
{ /* Convert_RSPF_MGRS_To_Geodetic */
  ...
  double Northing;
  long In_Precision;
  error_code = Break_RSPF_MGRS_String (RSPF_MGRS, &Zone, Letters, &Easting, &Northing, &In_Precision);
  if (!error_code)
  {
     if (Zone)
     {
        error_code |= Convert_RSPF_MGRS_To_UTM (RSPF_MGRS, &Zone, &Hemisphere, &Easting, &Northing);
        Set_UTM_Parameters (RSPF_MGRS_a, RSPF_MGRS_f, 0);
        error_code |= Convert_UTM_To_Geodetic (Zone, Hemisphere, Easting, Northing, Latitude, Longitude);
     }
     else
     {
        error_code |= Convert_RSPF_MGRS_To_UPS (RSPF_MGRS, &Hemisphere, &Easting, &Northing);
        Set_UPS_Parameters (RSPF_MGRS_a, RSPF_MGRS_f);
        error_code |= Convert_UPS_To_Geodetic (Hemisphere, Easting, Northing, Latitude, Longitude);
     }
  }
  return (error_code);
} /* END OF Convert_RSPF_MGRS_To_Geodetic */
long Convert_UTM_To_RSPF_MGRS (long Zone,
                          char Hemisphere,
                          double Easting,
                          double Northing,
                          long Precision,
                          char* RSPF_MGRS)
/*
 * The function Convert_UTM_To_RSPF_MGRS converts UTM (zone, easting, and
 * northing) coordinates to an RSPF_MGRS coordinate string, according to the 
 ...
 *    Precision  : Precision level of RSPF_MGRS string   (input)
 *    RSPF_MGRS       : RSPF_MGRS coordinate string           (output)
 */
{ /* Convert_UTM_To_RSPF_MGRS */
  long error_code = RSPF_MGRS_NO_ERROR;
  long temp_error = RSPF_MGRS_NO_ERROR;
  long Letters[RSPF_MGRS_LETTERS]; /* Number location of 3 letters in alphabet   */
  double Latitude;           /* Latitude of UTM point */
  double Longitude;          /* Longitude of UTM point */
  double divisor;
  if ((Zone < 1) || (Zone > 60))
    error_code |= RSPF_MGRS_ZONE_ERROR;
  if ((Hemisphere != 'S') && (Hemisphere != 'N'))
    error_code |= RSPF_MGRS_HEMISPHERE_ERROR;
  if ((Easting < MIN_EASTING) || (Easting > MAX_EASTING))
    error_code |= RSPF_MGRS_EASTING_ERROR;
  if ((Northing < MIN_NORTHING) || (Northing > MAX_NORTHING))
    error_code |= RSPF_MGRS_NORTHING_ERROR;
  if ((Precision < 0) || (Precision > MAX_PRECISION))
    error_code |= RSPF_MGRS_PRECISION_ERROR;
  if (!error_code)
  {
	...
	else
	{
	  ...
	  divisor = pow (10.0, (5 - Precision));
	  Easting = Round_RSPF_MGRS (Easting/divisor) * divisor;
	  Northing = Round_RSPF_MGRS (Northing/divisor) * divisor;
	  UTMRSPF_MGRS (Zone, Letters, Latitude, Easting, Northing);
	  /* UTM checks - these should be done in UTMRSPF_MGRS */
	  if ((Zone == 31) && (Letters[0] == LETTER_V))
	    if (Easting > 500000)
		  Easting = 500000;
	  if (Northing > 10000000)
	    Northing = 10000000;
	  Make_RSPF_MGRS_String (RSPF_MGRS, Zone, Letters, Easting, Northing, Precision);
	}
  }
  return (error_code);
} /* Convert_UTM_To_RSPF_MGRS */
long Convert_RSPF_MGRS_To_UTM (const char   *RSPF_MGRS,
                          long   *Zone,
                          char   *Hemisphere,
                          double *Easting,
                          double *Northing)
/*
 * The function Convert_RSPF_MGRS_To_UTM converts an RSPF_MGRS coordinate string
 * to UTM projection (zone, hemisphere, easting and northing) coordinates 
 * according to the current ellipsoid parameters.  If any errors occur, 
 * the error code(s) are returned by the function, otherwise UTM_NO_ERROR 
 * is returned.
 *
 *    RSPF_MGRS       : RSPF_MGRS coordinate string           (input)
 *    Zone       : UTM zone                         (output)
 *    Hemisphere : North or South hemisphere        (output)
 *    Easting    : Easting (X) in meters            (output)
 *    Northing   : Northing (Y) in meters           (output)
 */
{ /* Convert_RSPF_MGRS_To_UTM */
  long error_code = RSPF_MGRS_NO_ERROR;
  long Letters[RSPF_MGRS_LETTERS];
  long In_Precision;
  long Error = 0;
...
long Convert_UPS_To_RSPF_MGRS (char   Hemisphere,
                          double Easting,
                          double Northing,
                          long   Precision,
                          char*  RSPF_MGRS)
/*
 *  The function Convert_UPS_To_RSPF_MGRS converts UPS (hemisphere, easting, 
 *  and northing) coordinates to an RSPF_MGRS coordinate string according to 
 ...
 *    RSPF_MGRS          : RSPF_MGRS coordinate string           (output)
 */
{ /* Convert_UPS_To_RSPF_MGRS */
  double divisor;
  long error_code = RSPF_MGRS_NO_ERROR;
  long ltrnum[RSPF_MGRS_LETTERS]; /* Number location of 3 letters in alphabet   */
  ...
  if (!error_code)
  {
    ...
    Northing = Round_RSPF_MGRS (Northing/divisor) * divisor;
    if (Hemisphere == 'S')
      UPS (RSPF_MGRS, ltrnum, Easting, Northing, UPS_SOUTH);
    else
      UPS (RSPF_MGRS, ltrnum, Easting, Northing, UPS_NORTH);
    Make_RSPF_MGRS_String (RSPF_MGRS, 0, ltrnum, Easting, Northing, Precision);
  }
  return (error_code);
} /* Convert_UPS_To_RSPF_MGRS */
long Convert_RSPF_MGRS_To_UPS ( const char   *RSPF_MGRS,
                           char   *Hemisphere,
                           double *Easting,
                           double *Northing)
/*
 *  The function Convert_RSPF_MGRS_To_UPS converts an RSPF_MGRS coordinate string
