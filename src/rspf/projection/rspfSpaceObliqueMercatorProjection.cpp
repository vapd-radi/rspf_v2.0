...
#endif
RTTI_DEF1(rspfSpaceObliqueMercatorProjection, "rspfSpaceObliqueMercatorProjection", rspfMapProjection);
rspfSpaceObliqueMercatorProjection::rspfSpaceObliqueMercatorProjection
   (rspfSatelliteType type,
    double pathNumber,
    const rspfEllipsoid& ellipsoid)
      :
      rspfMapProjection(ellipsoid)
{
   setParameters(type, pathNumber);
}
void rspfSpaceObliqueMercatorProjection::setParameters(rspfSatelliteType type,
                                                        double path)
{
   thePath = path;
...
void rspfSpaceObliqueMercatorProjection::seraz0(double lam,
                                                 double mult)
{
...
rspfDpt
rspfSpaceObliqueMercatorProjection::forward(const rspfGpt &worldPoint) const
{
...
rspfGpt
rspfSpaceObliqueMercatorProjection::inverse(const rspfDpt &projectedPoint)
   const
{
   int nn;
   ...
   if (fabs(sa) < TOL)
      ...
      phi = atan((tan(lamdp) * cos(lamt) - ca * sin(lamt)) /
                    (one_es * sa));
   return rspfGpt(phi*DEG_PER_RAD,
                   (lam+lam0)*DEG_PER_RAD, 0.0, theDatum);
   
}

rspfGpt
rspfSpaceObliqueMercatorProjection::inverse_do(const rspfDpt &projectedPoint,const rspfGpt &worldPoint)
   const
{
   int nn;
   double lamt, sdsq, s, lamdp, phidp, sppsq, dd, sd, sl, fac, scl, sav, spp;
...
void rspfSpaceObliqueMercatorProjection::update()
{
   
}
bool rspfSpaceObliqueMercatorProjection::saveState(rspfKeywordlist& kwl,
...
bool rspfSpaceObliqueMercatorProjection::loadState(const rspfKeywordlist& kwl,
                                                    const char* prefix)
{
   bool result = true;
   ...
   if(path)
   {
      thePath = rspfString(path).toDouble();
   }
   else
   {
      result = false;
   ...
   if(type)
   {
      theSatelliteType = (rspfSatelliteType)rspfString(type).toInt();
   }
   else
   {
      result = false;
   ...
   if(result)
   {
      result = rspfMapProjection::loadState(kwl, prefix);
   }
   else
   {
...
bool rspfSpaceObliqueMercatorProjection::operator==(const rspfProjection& proj) const
{
   if (!rspfMapProjection::operator==(proj))
      return false;
   rspfSpaceObliqueMercatorProjection* p = PTR_CAST(rspfSpaceObliqueMercatorProjection, &proj);
   if (!p) return false;
   if (theSatelliteType != p->theSatelliteType) return false;
   if (!rspf::almostEqual(thePath,p->thePath)) return false;
   return true;
}
