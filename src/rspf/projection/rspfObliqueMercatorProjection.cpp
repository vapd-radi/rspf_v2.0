...
RTTI_DEF1(rspfObliqueMercatorProjection, "rspfObliqueMercatorProjection", rspfMapProjection)
#ifndef PI_OVER_2
...
rspfObliqueMercatorProjection::rspfObliqueMercatorProjection(const rspfEllipsoid& ellipsoid,
...
void rspfObliqueMercatorProjection::update()
{
...
void rspfObliqueMercatorProjection::setFalseEasting(double falseEasting)
{
   OMerc_False_Easting = falseEasting;
   update();
}
void rspfObliqueMercatorProjection::setFalseNorthing(double falseNorthing)
{
   OMerc_False_Northing = falseNorthing;
   update();  
}
void rspfObliqueMercatorProjection::setFalseEastingNorthing(double falseEasting, double falseNorthing)
{
   OMerc_False_Easting = falseEasting;
   OMerc_False_Northing = falseNorthing;
   update();  
...
void rspfObliqueMercatorProjection::setCentralPoint1(const rspfGpt& point)
{
   theCentralPoint1 = point;
   update();
}
void rspfObliqueMercatorProjection::setCentralPoint2(const rspfGpt& point)
{
   theCentralPoint2 = point;
   update();
}
void rspfObliqueMercatorProjection::setScaleFactor(double scaleFactor)
{
   OMerc_Scale_Factor = scaleFactor;
   update();
}
void rspfObliqueMercatorProjection::setParameters(const rspfGpt& point1,
                                                   const rspfGpt& point2,
                                                   double falseEasting,
                                                   double falseNorthing,
                                                   double scaleFactor)
...
void rspfObliqueMercatorProjection::setDefaults()
{
...
rspfGpt rspfObliqueMercatorProjection::inverse(const rspfDpt &eastingNorthing)const
{
   double lat = 0.0;
   double lon = 0.0;
...
rspfGpt rspfObliqueMercatorProjection::inverse_do(const rspfDpt &eastingNorthing,const rspfGpt &latLon)const
{
   double lat = 0.0;
   double lon = 0.0;
...
rspfDpt rspfObliqueMercatorProjection::forward(const rspfGpt &latLon)const
{
   double easting  = 0.0;
   double northing = 0.0;
   rspfGpt gpt = latLon;
...
bool rspfObliqueMercatorProjection::saveState(rspfKeywordlist& kwl, const char* prefix) const
{
   kwl.add(prefix,
           ...
           true);
   kwl.add(prefix,
           rspfKeywordNames::CENTRAL_POINT1_LAT_KW,
           theCentralPoint1.latd(),
           true);
   kwl.add(prefix,
           rspfKeywordNames::CENTRAL_POINT1_LON_KW,
           theCentralPoint1.lond(),
           true);
   kwl.add(prefix,
           rspfKeywordNames::CENTRAL_POINT2_LAT_KW,
           theCentralPoint2.latd(),
           true);
   kwl.add(prefix,
           rspfKeywordNames::CENTRAL_POINT2_LON_KW,
...
bool rspfObliqueMercatorProjection::loadState(const rspfKeywordlist& kwl,
                                             const char* prefix)
{
   bool flag = rspfMapProjection::loadState(kwl, prefix);
   const char* type          = kwl.find(prefix, rspfKeywordNames::TYPE_KW);
   const char* point1Lat     = kwl.find(prefix, rspfKeywordNames::CENTRAL_POINT1_LAT_KW);
   const char* point1Lon     = kwl.find(prefix, rspfKeywordNames::CENTRAL_POINT1_LON_KW);
   const char* point2Lat     = kwl.find(prefix, rspfKeywordNames::CENTRAL_POINT2_LAT_KW);
   const char* point2Lon     = kwl.find(prefix, rspfKeywordNames::CENTRAL_POINT2_LON_KW);
   const char* scale         = kwl.find(prefix, rspfKeywordNames::SCALE_FACTOR_KW);
   
   setDefaults();
   if(rspfString(type) == STATIC_TYPE_NAME(rspfObliqueMercatorProjection))
   {
      ...
      {
         theCentralPoint1.latd(rspfString(point1Lat).toDouble());
         theCentralPoint1.lond(rspfString(point1Lon).toDouble());
         theCentralPoint2.latd(rspfString(point2Lat).toDouble());
         theCentralPoint2.lond(rspfString(point2Lon).toDouble());
      }
      if(scale)
      {
         OMerc_Scale_Factor = rspfString(scale).toDouble();
      }
   }
 ...
 *                              FUNCTIONS     
 */
long rspfObliqueMercatorProjection::Set_Oblique_Mercator_Parameters(double a,
                                                                     double f,
...
void rspfObliqueMercatorProjection::Get_Oblique_Mercator_Parameters(double *a,
                                                                     double *f,
                                                                     double *Origin_Latitude,
                                                                     double *Latitude_1,
                                                                     double *Longitude_1,
...
long rspfObliqueMercatorProjection::Convert_Geodetic_To_Oblique_Mercator(double Latitude,
                                                                          double Longitude,
                                                                          double *Easting,
...
long rspfObliqueMercatorProjection::Convert_Oblique_Mercator_To_Geodetic(double Easting,
                                                                          double Northing,
                                                                          double *Latitude,
...
bool rspfObliqueMercatorProjection::operator==(const rspfProjection& proj) const
{
   if (!rspfMapProjection::operator==(proj))
      return false;
   rspfObliqueMercatorProjection* p = PTR_CAST(rspfObliqueMercatorProjection, &proj);
   if (!p) return false;
   if (theCentralPoint1 != p->theCentralPoint1) return false;
   if (theCentralPoint2 != p->theCentralPoint2) return false;
   if (!rspf::almostEqual(OMerc_Scale_Factor,p->OMerc_Scale_Factor)) return false;
   return true;
}
