#include <rspf/projection/rspfSmacCallibrationSystem.h>
#include <cmath>
#include <iostream>
void rspfSmacCallibrationSystem::undistort(const rspfDpt& input, rspfDpt& output)const
{
...
void rspfSmacCallibrationSystem::test()
{
   double k0 = -.2165e-3;
   double k1 = .4230e-7;
   double k2 = -.1652e-11;
   ...
   double p3 = -.1464e-18;
   double p4 = .1233e-38;
   rspfSmacCallibrationSystem smac(k0, k1, k2, k3, k4,
                                    p1, p2, p3, p4,
                                    rspfDpt(.003, -.001));
   rspfDpt output;
   smac.undistort(rspfDpt(62.142, -62.336), output);
   std::cout << "Input: " << rspfDpt(62.142, -62.336) << std::endl;
   std::cout << "Output should be close to x = 62.136 and y = -62.332 <====> " << output << std::endl;
   rspfDpt input = output;
   std::cout << "Now test the inverse!" << std::endl;
   smac.distort(input, output);
   
   std::cout << "Inverse should be close to " << rspfDpt(62.142, -62.336) << " <=====> " << output << std::endl;
}
