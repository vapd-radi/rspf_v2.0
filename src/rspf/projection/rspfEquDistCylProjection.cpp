...
//*******************************************************************
//  $Id: rspfEquDistCylProjection.cpp 20542 2012-02-06 17:41:05Z dburken $
...
static rspfTrace traceDebug("rspfEquDistCylProjection:debug");

RTTI_DEF1(rspfEquDistCylProjection,
...
rspfEquDistCylProjection::rspfEquDistCylProjection(const rspfEllipsoid& ellipsoid,
                                                     const rspfGpt& origin)
//  :rspfLlxyProjection(ellipsoid, origin)
...
void rspfEquDistCylProjection::update()
{
   ...
   {
      rspf_uint32 datum_code = theDatum->epsgCode();
      if ((datum_code >= 6000) && (datum_code < 7000))
         thePcsCode = datum_code - 2000;
   }
}

void rspfEquDistCylProjection::setFalseEasting(double falseEasting)
{
   Eqcy_False_Easting = falseEasting;
   update();
}

void rspfEquDistCylProjection::setFalseNorthing(double falseNorthing)
{
   Eqcy_False_Northing = falseNorthing;
   update();
}

void rspfEquDistCylProjection::setFalseEastingNorthing(double falseEasting,
                                                        double falseNorthing)
{
   Eqcy_False_Easting  = falseEasting;
   Eqcy_False_Northing = falseNorthing;
...
void rspfEquDistCylProjection::setDefaults()
{
   theProjectionUnits = RSPF_DEGREES;
...
void rspfEquDistCylProjection::lineSampleHeightToWorld(const rspfDpt &lineSample,
                                                        ...
                                                        rspfGpt&      gpt)const
{
   ...
   {
      rspfMapProjection::lineSampleHeightToWorld(lineSample, hgtEllipsoid, gpt);
      return;
   }
   else
   {
...
void rspfEquDistCylProjection::worldToLineSample(const rspfGpt &worldPoint,
                                                  rspfDpt&       lineSample)const
{
   if(theModelTransformUnitType != RSPF_UNIT_UNKNOWN)
   {
      rspfMapProjection::worldToLineSample(worldPoint, lineSample);
      return;
   }
                                                   ...
                                                   rspfDpt&       lineSample ) const
{
   if( theModelTransformUnitType == RSPF_UNIT_UNKNOWN )
   {
          ...
          !imageSize.isNan() )
      {
         ...
         rspfDpt gptEastingNorthing = forward(gpt);

         if( !gptEastingNorthing.isNan() )
         {
            if ( imageSize.x > 0.0 )
            {
               rspfGpt edge(gpt.lat, -180.0, 0.0);
               rspfDpt leftProjectionEdge = forward(edge);
               
               edge.lon = 180;
               rspfDpt rightProjectionEdge = forward(edge);

               // Right edge Easting of image from tie.
               rspf_float64 leftImageX = theUlEastingNorthing.x  - (0.5*theMetersPerPixel.x);
               rspf_float64 rightImageX = leftImageX + (imageSize.x * theMetersPerPixel.x);
               ...
               else // Crossed date line:
               {
                  ...
                  else
                  {
                     ...
                     rspf_float64 deltaToLeft  = theUlEastingNorthing.x - gptEastingNorthing.x;
                     rspf_float64 deltaToRight = gptEastingNorthing.x - normRightX;
   ...
   } // Matches: if( theModelTransformUnitType == RSPF_UNIT_UNKNOWN )
   else 
   {
      // Has transform:
      rspfMapProjection::worldToLineSample(worldPoint, lineSample);
...
} // End: rspfEquDistCylProjection::worldToLineSample(worldPoint, lineSample, imageSize)

rspfGpt rspfEquDistCylProjection::inverse(const rspfDpt &eastingNorthing)const
{
   double lat = 0.0;
   double lon = 0.0;
...
rspfDpt rspfEquDistCylProjection::forward(const rspfGpt &latLon)const
{
   double easting  = 0.0;
   double northing = 0.0;
   rspfGpt gpt = latLon;
...
bool rspfEquDistCylProjection::saveState(rspfKeywordlist& kwl, const char* prefix) const
{
   return rspfMapProjection::saveState(kwl, prefix);
}

bool rspfEquDistCylProjection::loadState(const rspfKeywordlist& kwl, const char* prefix)
{
   if (traceDebug())
   {
      rspfNotify(rspfNotifyLevel_DEBUG) << "DEBUG rspfEquDistCylProjection::loadState: Input keyword list is \n" << kwl << endl;
   }

   //   rspfLlxyProjection::loadState(kwl, prefix);
   rspfMapProjection::loadState(kwl, prefix);
   theProjectionUnits = RSPF_DEGREES;

   // Make sure the origin.lat is defined since it is needed to relate degrees/meter:
   if (rspf::isnan(theOrigin.lat))
   {
   ...
   //
   if(rspfString(type) == STATIC_TYPE_NAME(rspfEquDistCylProjection))
   {
...
long rspfEquDistCylProjection::Set_Equidistant_Cyl_Parameters(double a,
                                                               double f,
                                                               double Std_Parallel,
...
void rspfEquDistCylProjection::Get_Equidistant_Cyl_Parameters(double *a,
                                                               double *f,
                                                               double *Std_Parallel,
                                                               double *Central_Meridian,
                                                               double *False_Easting,
...
long rspfEquDistCylProjection::Convert_Geodetic_To_Equidistant_Cyl (double Latitude,
                                                                     double Longitude,
                                                                     double *Easting,
...
long rspfEquDistCylProjection::Convert_Equidistant_Cyl_To_Geodetic(double Easting,
                                                                    double Northing,
                                                                    double *Latitude,
