...
#include <rspf/projection/rspfTranmerc.h>
...
RTTI_DEF1(rspfBngProjection, "rspfBngProjection", rspfMapProjection);
rspfBngProjection::rspfBngProjection()
   :rspfMapProjection(*rspfEllipsoidFactory::instance()->create("AA"),
                       rspfGpt(49.0, -2.0, rspf::nan(), 
                       rspfDatumFactory::instance()->create(rspfString("OGB-M"))))
{
   string_Broken = 0;
   theDatum = rspfDatumFactory::instance()->create(rspfString("OGB-M"));
   update();
}
rspfGpt rspfBngProjection::inverse_do(const rspfDpt &eastingNorthing,const rspfGpt &latLon)const
{
   char s[100];
   double lat;
   double lon;
   ...
   Convert_BNG_To_Geodetic(s,
                           &lat,
                           &lon);
   rspfGpt result(49, -2, rspf::nan(),theDatum);
   result.latr(lat);
   result.lonr(lon);
   return result;
}
rspfGpt rspfBngProjection::inverse(const rspfDpt &eastingNorthing)const
{
   char s[100];
   double lat;
   double lon;
...
rspfDpt rspfBngProjection::forward(const rspfGpt &latLon)const
{
   char BNG[100];
   string_Broken = 0;
   Set_Transverse_Mercator_Parameters(BNG_a, BNG_f, BNG_Origin_Lat,
...
void rspfBngProjection::update()
{
   ...
   theOrigin = rspfGpt(49.0, -2.0, rspf::nan(), theDatum);
 ...
 */
bool rspfBngProjection::saveState(rspfKeywordlist& kwl,
 ...
 */
bool rspfBngProjection::loadState(const rspfKeywordlist& kwl,
                                   const char* prefix)
{
...
long rspfBngProjection::Find_Index (char letter, 
 ...
 */
  rspf_uint32 i = 0;
  rspf_uint32 not_Found = 1;
  rspf_uint32 length = (rspf_uint32)strlen(letter_Array);
  rspf_uint32 Error_Code = BNG_NO_ERROR;
...
long rspfBngProjection::Round_BNG (double value)const
...
long rspfBngProjection::Make_BNG_String (char ltrnum[4],
                                          long Easting, 
                                          long Northing,
                                          char* BNG,
...
long rspfBngProjection::Break_BNG_String (char* BNG,
                                           char Letters[3],
                                           double* Easting,
                                           double* Northing,
                                           long* Precision)const
...
long rspfBngProjection::Check_Out_Of_Area(char BNG500,
                                           char BNG100)const
...
long rspfBngProjection::Set_BNG_Parameters(char *Ellipsoid_Code)
...
void rspfBngProjection::Get_BNG_Parameters(char *Ellipsoid_Code)
...
long rspfBngProjection::Convert_Geodetic_To_BNG (double Latitude,
                                                  double Longitude,
                                                  long Precision,
                                                  char* BNG)const
...
long rspfBngProjection::Convert_BNG_To_Geodetic (char *BNG, 
                                                  double *Latitude, 
...
long rspfBngProjection::Convert_Transverse_Mercator_To_BNG(double Easting,
                                                            double Northing,
                                                            long Precision,
                                                            char *BNG)const
...
long rspfBngProjection::Convert_BNG_To_Transverse_Mercator(char *BNG,
                                                            double *Easting,
