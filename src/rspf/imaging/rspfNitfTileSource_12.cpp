...
//
//*******************************************************************
//  $Id: rspfNitfTileSource_12.cpp 958 2010-06-03 23:00:32Z ming.su $
...
bool rspfNitfTileSource_12::uncompressJpeg12Block(rspf_uint32 x, rspf_uint32 y,
                                                   ...
                                                   rspf_uint32 bands)
{
   ...
   const rspf_uint32 LINES_TO_READ =
      min(static_cast<rspf_uint32>(cacheSize.y), cinfo.output_height);
   ...
   std::vector<rspf_uint16*> destinationBuffer(bands);
   rspf_uint32 band = 0;
   ...
   std::vector<rspf_uint16> lineBuffer(ROW_STRIDE);
   JSAMPROW jbuf[1];
...
bool rspfNitfTileSource_12::loadJpegQuantizationTables(rspfNitfImageHeader* hdr,
                                jpeg_decompress_struct& cinfo)
{
  if (!hdr)
  {
  ...
  if (comrat.size() >= 4)
  {
    ...
    else
    {
      rspfNotify(rspfNotifyLevel_WARN)
        << "rspfNitfTileSource_12::loadJpegQuantizationTables WARNING\n"
...
bool rspfNitfTileSource_12::loadJpegHuffmanTables(jpeg_decompress_struct& cinfo)
{
  if ( (cinfo.ac_huff_tbl_ptrs[0] != NULL) &&
