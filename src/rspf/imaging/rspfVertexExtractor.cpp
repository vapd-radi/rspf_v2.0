...
//
//*************************************************************************
// $Id: rspfVertexExtractor.cpp 21184 2012-06-29 15:13:09Z dburken $

#include <fstream>
using namespace std;

#include <rspf/imaging/rspfVertexExtractor.h>
#include <rspf/imaging/rspfImageSource.h>
#include <rspf/imaging/rspfImageHandler.h>
#include <rspf/base/rspfTrace.h>
#include <rspf/base/rspfNotifyContext.h>

static rspfTrace traceDebug("rspfVertexExtractor:degug");

RTTI_DEF2(rspfVertexExtractor, "rspfVertexExtractor",
          rspfSource, rspfProcessInterface);

rspfVertexExtractor::rspfVertexExtractor(rspfImageSource* inputSource)
      :
      rspfOutputSource(0, // owner
                        1,
                        0,
                        true,
                        true),
      ...
      theRightEdge(0)
{
   if (inputSource == 0)
   {
      rspfNotify(rspfNotifyLevel_WARN) << "rspfVertexExtractor::rspfVertexExtractor ERROR"
...
rspfVertexExtractor::~rspfVertexExtractor()
{
   if (theLeftEdge)
   {
      delete [] theLeftEdge;
...
bool rspfVertexExtractor::execute()
{
   static const char MODULE[] = "rspfVertexExtractor::execute";

   if (traceDebug()) CLOG << " Entered..." << endl;
   
   if (theFilename == rspfFilename::NIL)
   {
      rspfNotify(rspfNotifyLevel_WARN) << "rspfVertexExtractor::execute WARNING:"
   ...
   {
      rspfNotify(rspfNotifyLevel_NOTICE) << MODULE << " NOTICE rspfVertexExtractor::execute:"
...
bool rspfVertexExtractor::scanForEdges()
{
   static const char MODULE[] = "rspfVertexExtractor::scanForEdges";
   ...
   if (!src)
   {
      rspfNotify(rspfNotifyLevel_WARN)
         << "WARN rspfVertexExtractor::scanForEdges():"
   ...
   theLeftEdge  = new rspf_int32[theAreaOfInterest.height()];
   theRightEdge = new rspf_int32[theAreaOfInterest.height()];
   for (rspf_int32 i=0; i<(int)theAreaOfInterest.height(); ++i)
   {
      theLeftEdge[i]  = RSPF_INT_NAN;
      theRightEdge[i] = RSPF_INT_NAN;
   }
   ...
   rspfNotify(rspfNotifyLevel_INFO) << "Scanning image source for edges..." << std::endl;
   ...
   for (rspf_int32 line = START_LINE; line < STOP_LINE; line += TILE_HEIGHT)
   {
      const rspf_int32 START_INDEX = line - START_LINE;
      const rspf_int32 STOP_INDEX  = ( (line+TILE_HEIGHT-1) < STOP_LINE ) ?
         (START_INDEX+TILE_HEIGHT-1) :
         (theAreaOfInterest.height()-1);
      const rspf_int32 TILE_LINE_STOP = ((line+TILE_HEIGHT-1) < STOP_LINE) ?
         (line+TILE_HEIGHT-1) :  STOP_LINE;
      
      // See if all the edges have been found for these tile lines.
      bool all_edges_found  = true;
      for (rspf_int32 i = START_INDEX; i <= STOP_INDEX; ++i)
      {
         if (theLeftEdge[i] == RSPF_INT_NAN)
         {
      ...
      if (all_edges_found == false)
      {
         ...
         // (left edges).
         //***
         for (rspf_int32 samp = START_SAMP; samp < STOP_SAMP; samp += TILE_WIDTH)
         {
            // Get the tile.
            rspfRefPtr<rspfImageData> id =
               src->getTile(rspfIpt(samp, line));
            
            if (!id.valid() || id->getDataObjectStatus() == RSPF_NULL ||
                id->getDataObjectStatus() == RSPF_EMPTY)
            {
               continue; // Nothing to do...
            }
            
            // Line loop through tile.
            const rspf_int32 TILE_SAMP_STOP = ((samp+TILE_WIDTH-1) < STOP_SAMP) ?
               (samp+TILE_WIDTH-1) : STOP_SAMP;
            
            for (rspf_int32 y = line; y <= TILE_LINE_STOP; ++y)
            {
               rspf_int32 edge_index = y - START_LINE;
               if (theLeftEdge[edge_index] == RSPF_INT_NAN)
               {
                  for (rspf_int32 x = samp; x <= TILE_SAMP_STOP; ++x)
                  {
                     if (! (id->isNull(rspfIpt(x,y))))
                     {
      ...
      for (rspf_int32 i = START_INDEX; i <= STOP_INDEX; ++i)
      {
         if (theRightEdge[i] == RSPF_INT_NAN)
         {
            all_edges_found = false;
            break;
         }
      ...
      if (all_edges_found == false)
      {
         ...
         const rspf_int32 END_TILE_X = START_SAMP + ((theAreaOfInterest.width()/
                                                 TILE_WIDTH)*TILE_WIDTH);
         

         for (rspf_int32 samp = END_TILE_X; samp >= START_SAMP; samp -= TILE_WIDTH)
         {
            // Get the tile.
            rspfRefPtr<rspfImageData> id =
               src->getTile(rspfIpt(samp, line));
            ...
            {
               rspf_int32 edge_index = y - START_LINE;
               if (theRightEdge[edge_index] == RSPF_INT_NAN)
               {
                  for (rspf_int32 x = TILE_SAMP_STOP; x >= samp; --x)
                  {
                     if (! (id->isNull(rspfIpt(x,y))))
                     {
                        // Capture the first valid pixel.
   ...
   if (traceDebug())
   {
      ...
      for (rspf_int32 i=0; i<(int)theAreaOfInterest.height(); ++i)
      {
         rspfNotify(rspfNotifyLevel_DEBUG) << "DEBUG: left[" << i << "]:  "
                                             << theLeftEdge[i]
                                             << " right[" << i << "]:  "
                                             << theRightEdge[i]
                                             << std::endl;
...
bool rspfVertexExtractor::extractVertices()
{
   //***
   ...
   //***
   static const char MODULE[] = "rspfVertexExtractor::extractVertices";

   if (traceDebug()) CLOG << " Entered..." << endl;

   if (!theLeftEdge || !theRightEdge)
   {
      rspfNotify(rspfNotifyLevel_WARN) << "ERROR rspfVertexExtractor::extractVertices():"
   ...
   rspf_int32   total_lines = theAreaOfInterest.height();
   
   rspf_int32   topLine             = 0;    
   rspf_int32   bottomLine          = 0;
   
   rspf_int32   firstLeftSample     = 0;
   rspf_int32   firstRightSample    = 0;
   
   rspf_int32   lastLeftSample      = 0;
   rspf_int32   lastRightSample     = 0;
   
   rspf_int32   leftMostSample      = 0;
   rspf_int32   leftMostSampleLine  = 0;
   
   rspf_int32   rightMostSample     = 0;
   rspf_int32   rightMostSampleLine = 0;
   
   rspf_int32   leftCorner_A        = 0;
   rspf_int32   leftCornerLine_A    = 0;
   rspf_int32   leftCorner_B        = 0;
   rspf_int32   leftCornerLine_B    = 0;
   
   rspf_int32   rightCorner_A       = 0;
   rspf_int32   rightCornerLine_A   = 0;
   rspf_int32   rightCorner_B       = 0;
   rspf_int32   rightCornerLine_B   = 0;
   ...
   rspf_int32 first_non_null_line = 0;
   rspf_int32 last_non_null_line = 0;
   ...
   rspf_int32 k = 0;
   rspf_int32 start = first_non_null_line;
   rspf_int32 end = last_non_null_line;
   rspf_int32 sizeDeltaSlope = last_non_null_line - first_non_null_line;
   double* rightSlope = new double[sizeDeltaSlope];
  
   for(rspf_int32 i = start; i <= (end - 100); i += 100)
   {
      rspf_int32  run = theRightEdge[i] - theRightEdge[i + 100];
      if(run != 0)
      {
   ...
   {
      CLOG << "DEBUG:" << endl;
      for (rspf_int32 i=0; i<= k-1; i++)
      {
          rspfNotify(rspfNotifyLevel_DEBUG)<< setprecision(15) << "rightSlope[" << i << "]:  "
                                             << rightSlope[i]
                                             << std::endl;
      }
   }
   ...
   for(rspf_int32 i = start; i <= (end - 100); i += 100)
   {
      rspf_int32  run = theLeftEdge[i] - theLeftEdge[i + 100];
      if(run != 0)
      {
   ...
   if (traceDebug())
   {
      ...
      for (rspf_int32 i=0; i<= k-1; i++)
      {
         rspfNotify(rspfNotifyLevel_DEBUG) << setprecision(15) << "leftSlope[" << i << "]:  "
                                             << leftSlope[i]
                                             << std::endl;
      }
   }
   rspf_int32 sizeOftheSlope = k-1;
   rspf_int32 counter = 0;


      // Find the right most sample
   rspf_int32 rightMostEdgeTemp = theRightEdge[topLine];
   rspf_int32 rightMostEdgeTempLine = 0;

   for(rspf_int32 i= (topLine+1); i<bottomLine; i++)
   {     
   ...
   rspf_int32 leftMostEdgeTemp = theLeftEdge[topLine];
   rspf_int32 leftMostEdgeTempLine = 0;
   ...
   for(rspf_int32 i=0; i < sizeOftheSlope; ++i)
   {
   ...
   rspf_int32 halfsamps = (rightMostSample - leftMostSample ) / 2;
   ...
   { 
      if( (firstRightSample - firstLeftSample) > halfsamps )
      {
         ...
         { 
            rspfNotify(rspfNotifyLevel_DEBUG)<<"\n\n***Case = RHOMBUS***"<<endl;
            rspfNotify(rspfNotifyLevel_DEBUG)<<"here"<<endl;
         }
   ...
   {//if not square or rhombus
      ...
      {//if tilted right
         ...
         rspf_int32 line1_pt4 = rightMostSampleLine;
         rspf_int32 line1_pt0 = topLine;
         rspf_int32 line1_pt2 = (line1_pt0 + line1_pt4)/2;
         rspf_int32 line1_pt1 = (line1_pt2 + line1_pt0)/2;
         rspf_int32 line1_pt3 = (line1_pt2 + line1_pt4)/2;
         ...
         for(rspf_int32 i = 0; i < num_elements_line1; ++i)
         {
         ...
         if(traceDebug())
         {
            rspfNotify(rspfNotifyLevel_DEBUG)<<" a_line1:  "<< a_line1 <<std::endl;
            rspfNotify(rspfNotifyLevel_DEBUG)<<" b_line1:  "<< b_line1 <<std::endl;
            
            
            for(rspf_int32 i = 0; i < num_elements_line1; ++i)
            {
               
               rspfNotify(rspfNotifyLevel_DEBUG) << setprecision(15) << "line1RegPtsLines[" << i
                                                   << "]:  " <<line1RegPtsLines[i]
         ...
         //Regression Line 2
         rspf_int32 line2_pt0 = rightMostSampleLine;
         rspf_int32 line2_pt4 = bottomLine;
         rspf_int32 line2_pt2 = (line2_pt0 + line2_pt4)/2;
         rspf_int32 line2_pt1 = (line2_pt2 + line2_pt0)/2;
         rspf_int32 line2_pt3 = (line2_pt2 + line2_pt4)/2;
 
         
         rspf_int32 minValuePt1;
         rspf_int32 minOffsetPt1;
         ...
         rspf_int32 minValuePt2;
         rspf_int32 minOffsetPt2;
         ...
         rspf_int32 minValuePt3;
         rspf_int32 minOffsetPt3;
         ...
         for(rspf_int32 i = 0; i < num_elements_line2; ++i)
         {
         ...
         if(traceDebug())
         {
            for(rspf_int32 i = 0; i < num_elements_line2; ++i)
            {
               rspfNotify(rspfNotifyLevel_DEBUG) << setprecision(15) << "line2RegPtsLines[" << i << "]:  "
                                                   <<line2RegPtsLines[i]
         ...
         rspf_int32 line3_pt0 = leftMostSampleLine;
         rspf_int32 line3_pt4 = bottomLine;
         rspf_int32 line3_pt2 = (line3_pt0 + line3_pt4)/2;
         rspf_int32 line3_pt1 = (line3_pt2 + line3_pt0)/2;
         rspf_int32 line3_pt3 = (line3_pt2 + line3_pt4)/2;
         ...
         for(rspf_int32 i = 0; i < num_elements_line3; ++i)
         {
         ...
         if(traceDebug())
         {
            ...
            rspfNotify(rspfNotifyLevel_DEBUG) << " b_line3:  " << b_line3 <<std::endl;
               
               
            for(rspf_int32 i = 0; i < num_elements_line3; ++i)
            {
                  
               rspfNotify(rspfNotifyLevel_DEBUG) << setprecision(15) << "line3RegPtsLines[" << i << "]:  "
                                                   <<line3RegPtsLines[i]
         ...
         rspf_int32 line4_pt0 = leftMostSampleLine;
         rspf_int32 line4_pt4 = topLine;
         rspf_int32 line4_pt2 = (line4_pt0 + line4_pt4)/2;
         rspf_int32 line4_pt1 = (line4_pt2 + line4_pt0)/2;
         rspf_int32 line4_pt3 = (line4_pt2 + line4_pt4)/2;
...
//                //***
         ...
         rspf_int32 maxValueLine4Pt1;
         rspf_int32 maxOffsetLine4Pt1;
         ...
         rspf_int32 maxValueLine4Pt2;
         rspf_int32 maxOffsetLine4Pt2;
         ...
         rspf_int32 maxValueLine4Pt3;
         rspf_int32 maxOffsetLine4Pt3;
         ...
         for(rspf_int32 i = 0; i < num_elements_line4; ++i)
         {
         ...
         if(traceDebug())
         {
            ...
            for(rspf_int32 i = 0; i < num_elements_line4; ++i)
            {
               ...
               rspfNotify(rspfNotifyLevel_DEBUG) << setprecision(15) << "line4RegPtsLines[" << i << "]:  "
                                                   <<line4RegPtsLines[i]
         ...
         leftCornerLine_A = (rspf_int32)ul_y;
         leftCorner_A = (rspf_int32)ul_x; //0
            
         rightCornerLine_A = (rspf_int32)ur_y;
         rightCorner_A = (rspf_int32)ur_x; //1
            
         rightCornerLine_B = (rspf_int32)lr_y;
         rightCorner_B =(rspf_int32)lr_x; //2
            
         leftCornerLine_B = (rspf_int32)ll_y;
         leftCorner_B =(rspf_int32)ll_x; //3
      ...
      {//begin tilted left
         ...
         if (traceDebug())
         {
            rspfNotify(rspfNotifyLevel_DEBUG)<<" line1_pt1:   "<<line1_pt0<<std::endl;
            rspfNotify(rspfNotifyLevel_DEBUG)<<" line1_pt2:   "<<line1_pt1<<std::endl;
            rspfNotify(rspfNotifyLevel_DEBUG)<<" line1_pt3:   "<<line1_pt2<<std::endl;
            rspfNotify(rspfNotifyLevel_DEBUG)<<" line1_pt4:   "<<line1_pt3<<std::endl;
         }
         
         rspf_int32 minValueLine1Pt1;
         rspf_int32 minOffsetLine1Pt1;
         ...
         if (traceDebug())
         {
            rspfNotify(rspfNotifyLevel_DEBUG) << " line1_pt1:  "<< line1_pt1
                                                <<"   minValueLine1Pt1:  "<<minValueLine1Pt1<<"\n\n";
         }
         
         rspf_int32 minValueLine1Pt2;
         rspf_int32 minOffsetLine1Pt2;
         ...
         if (traceDebug())
         {
            rspfNotify(rspfNotifyLevel_DEBUG) << " line1_pt2:  "<< line1_pt2
                                                <<"   minValueLine1Pt2:  "<<minValueLine1Pt2<<"\n\n";
         }
         
         
         rspf_int32 minValueLine1Pt3;
         rspf_int32 minOffsetLine1Pt3;
         ...
         if (traceDebug())
         {
            rspfNotify(rspfNotifyLevel_DEBUG) << " line1_pt3:  "<< line1_pt3
         ...
         if(traceDebug())
         {
            rspfNotify(rspfNotifyLevel_DEBUG)<<"a_line1:  "<< a_line1
         ...
         if(traceDebug())
         {
            rspfNotify(rspfNotifyLevel_DEBUG) << "a_line2:  "<< a_line2
                                                << "\nb_line2:  "<< b_line2 <<std::endl;
         ...
         rspf_int32 maxValueLine3Pt1;
         rspf_int32 maxOffsetLine3Pt1;
         ...
         rspf_int32 maxValueLine3Pt2;
         rspf_int32 maxOffsetLine3Pt2;
         ...
         rspf_int32 maxValueLine3Pt3;
         rspf_int32 maxOffsetLine3Pt3;
         ...
         if(traceDebug())
         {
            ...
            rspfNotify(rspfNotifyLevel_DEBUG)<< " b_line4:  "<< b_line4 <<std::endl;
         ...
         if (traceDebug())
         {
            rspfNotify(rspfNotifyLevel_DEBUG) <<"Initial corners for tilted left:   "
                                                <<"\ncorner0:  ("<<ul_x<<", "<<ul_y<<")"
                                                <<"\ncorner1:  ("<<ur_x<<", "<<ur_y<<")"
                                                <<"\ncorner2:  ("<<lr_x<<", "<<lr_y<<")"
   ...
   if( (square == 0) && (rhombus == 0) && (right == 0) && (left == 0))
   {
      ...
      rspfImageHandler* src = reinterpret_cast<rspfImageHandler*>(getInput(0));
      
      rspfIrect tileRectUL(leftMostSample,
                            topLine,
                            leftMostSample+50,
                            (topLine + 50) );
      
      rspfRefPtr<rspfImageData> dataUL = src->getTile(tileRectUL);
      // rspfIrect imageRectangleUL = dataUL->getImageRectangle();
      
      rspf_int32 tempSamp_corner0a = 0;
      rspf_int32 tempLine_corner0a = 0;
      if (dataUL.valid())
      {
         for (rspf_int32 line = tileRectUL.ul().y; line <= tileRectUL.lr().y; ++line)
         {
            for (rspf_int32 samp = tileRectUL.ul().x; samp <=tileRectUL.ur().x; ++samp)
            {
               // uses band 0 corner points
               if(!(dataUL->isNull(rspfIpt(samp,line),0)))
               {
         ...
         rspf_int32 tempSamp_corner0b = 0;
         rspf_int32 tempLine_corner0b = 0;
         for (rspf_int32 samp = tileRectUL.ul().x; samp <=tileRectUL.ur().x; ++samp)
         {
            for (rspf_int32 line = tileRectUL.ul().y; line <= tileRectUL.lr().y; ++line)
            {
      ...
      rspfIrect tileRectUR((rightMostSample - 50),
                            topLine,
                            rightMostSample,
                            (topLine + 50) );
      
      rspfRefPtr<rspfImageData> dataUR = src->getTile(tileRectUR);
      // rspfIrect imageRectangleUR = dataUR->getImageRectangle();

      rspf_int32 tempSamp_corner1a = 0;
      rspf_int32 tempLine_corner1a = 0;

      if (dataUR.valid())
      {
         for (rspf_int32 line = tileRectUR.ul().y; line <= tileRectUR.lr().y; ++line)
         {
            for (rspf_int32 samp = tileRectUR.ur().x; samp > tileRectUR.ul().x; --samp)
            {
               
               if(!(dataUR->isNull(rspfIpt(samp,line),0)))
               {
         ...
         // Search in the other direction to handle the case of 'jaggered' edges.
         rspf_int32 tempSamp_corner1b = 0;
         rspf_int32 tempLine_corner1b = 0;
      ...
      rspfIrect tileRectLR((rightMostSample - 50),
                            bottomLine - 50,
                            rightMostSample,
                            (bottomLine) );
      
      rspfRefPtr<rspfImageData> dataLR = src->getTile(tileRectLR);
      // rspfIrect imageRectangleLR = dataLR->getImageRectangle();
      
      rspf_int32 tempSamp_corner2a = 0;
      rspf_int32 tempLine_corner2a = 0;

      if (dataLR.valid())
      {
         for (rspf_int32 line = tileRectLR.lr().y; line >= tileRectLR.ur().y; --line)
         {
            for (rspf_int32 samp = tileRectLR.lr().x; samp > tileRectLR.ul().x; --samp)
            {
               
               if(!(dataLR->isNull(rspfIpt(samp,line),0)))
               {
         ...
         rspf_int32 tempSamp_corner2b = 0;
         rspf_int32 tempLine_corner2b = 0;
      ...
      rspfIrect tileRectLL((leftMostSample),
                            (bottomLine - 50),
                            (leftMostSample +50),
                            (bottomLine) );
      
      rspfRefPtr<rspfImageData> dataLL = src->getTile(tileRectLL);
      // rspfIrect imageRectangleLL = dataLL->getImageRectangle();
      
      rspf_int32 tempSamp_corner3a = 0;
      rspf_int32 tempLine_corner3a = 0;

      if (dataLL.valid())
      {
         for (rspf_int32 samp = tileRectLL.ul().x; samp <= tileRectLL.ur().x; ++samp)
         {
            
            for (rspf_int32 line = tileRectLL.lr().y; line >= tileRectLL.ul().y; --line)
               
            {
               
               if(!(dataLL->isNull(rspfIpt(samp,line),0)))
               {
         ...
         rspf_int32 tempSamp_corner3b = 0;
         rspf_int32 tempLine_corner3b = 0;
         for (rspf_int32 line = tileRectLL.lr().y; line >= tileRectLL.ul().y; --line)
         {
...
bool rspfVertexExtractor::writeVertices()
{
   static const char MODULE[] = "rspfVertexExtractor::writeVertices";
...
void rspfVertexExtractor::setOutputName(const rspfString& filename)
{
...
void rspfVertexExtractor::setAreaOfInterest(const rspfIrect& rect)
{
   theAreaOfInterest = rect;
}

bool rspfVertexExtractor::isOpen()const
{
   return const_cast<ofstream*>(&theFileStream)->is_open();
}

bool rspfVertexExtractor::open()
{
   if(isOpen())
   {
      close();
   ...
   if (theFilename == rspfFilename::NIL)
   {
      rspfNotify(rspfNotifyLevel_WARN) << "rspfVertexExtractor::open WARNING:"
...
void rspfVertexExtractor::close()
{
   if (isOpen()) theFileStream.close();
}

template<class T1>
void rspfVertexExtractor::getMinAndIndex(T1* start, T1* end,
                                          T1& minValue, rspf_int32& offsetFromStart)
{
   
   offsetFromStart =0;
   rspf_int32 currentOffset = 0;
   minValue  = *start;
   while(start < end)
   {
      if(*start < minValue)
...
template<class T2>
void rspfVertexExtractor::getMaxAndIndex(T2* start, T2* end,
