 ...
 */
int rspf_strcasecmp(const char *s1, const char *s2);

/**
 * Written to replace non-portable "strncasecmp".  Similar to
 * rspf_strcasecmp,  except  it  only  compares  the first n characters.
 *
 * @return An integer -1, 0, or 1 if s1 is found, respectively, to be less
 * than, to match, or be greater than s2.
 */
int rspf_strncasecmp(const char *s1, const char *s2, unsigned int n);
