...
typedef struct {
   ...
   rspf_int32  count;      /* Number of items in this column (-1 =>variable)*/
...
typedef struct {
   ...
   rspf_int32       nfields;         /* Number of fields */
   ...
   rspf_int32       nrows;           /* Number of rows in the table */
   row_type       *row;            /* Array of table rows */
   rspf_int32       reclen;          /* Table record length (-1 => variable */
   rspf_int32       ddlen;           /* Data definition string length */
...
char *get_string(rspf_int32 *ind,char *src,char delimit);
char  get_char  (rspf_int32 *ind,char *src);
rspf_int32   get_number(rspf_int32 *ind,char *src,char delemit);

rspf_int32   parse_data_def( vpf_table_type *table );
...
rspf_int32  index_pos( rspf_int32 row_number,
		     vpf_table_type table );
#if 0
rspf_int32 row_offset( int field,
		     row_type row,
		     vpf_table_type table);
#endif
...
rspf_int32 table_pos( const char *field_name,
                    vpf_table_type table );
...
rspf_int32 is_vpf_table( const char *fname );
...
void nullify_table_element( rspf_int32 field, row_type row,
			    vpf_table_type table );

rspf_int32 put_table_element( rspf_int32 field, row_type row,
			    vpf_table_type table,
			    void *value, rspf_int32 count );
void vpf_dump_table( char *tablename, char *outname );
