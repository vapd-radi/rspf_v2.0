...
//*******************************************************************
//  $Id$
#ifndef rspfIpodSensor_HEADER
#define rspfIpodSensor_HEADER
#include "rspfSensorModel.h"
#include "rspfUtmProjection.h"
#include <rspf/base/rspfDpt3d.h>
class RSPF_DLL rspfIpodSensor : public rspfSensorModel
{
public:
   rspfIpodSensor();
   rspfIpodSensor(const rspfIpodSensor& src)
   :rspfSensorModel(src),
   ...
   {
      return new rspfIpodSensor(*this);
   }
   
   virtual void imagingRay(const rspfDpt& image_point,
                           rspfEcefRay&   image_ray) const;
   
   void lineSampleHeightToWorld(const rspfDpt& image_point,
