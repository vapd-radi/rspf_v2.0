#ifndef rspfTransMercatorProjection_HEADER
#define rspfTransMercatorProjection_HEADER
#include <rspf/projection/rspfMapProjection.h>
class RSPFDLLEXPORT rspfTransMercatorProjection : public rspfMapProjection
{
public:
   rspfTransMercatorProjection(const rspfEllipsoid& ellipsoid = rspfEllipsoid(6378137, 6356752.3142),
                                const rspfGpt& origin = rspfGpt());
   rspfTransMercatorProjection(const rspfEllipsoid& ellipsoid,
                                const rspfGpt& origin,
                                double falseEasting,
                                double falseNorthing,
                                double scaleFactor);
   ~rspfTransMercatorProjection(){}
   virtual rspfObject *dup()const{return new rspfTransMercatorProjection(*this);}
