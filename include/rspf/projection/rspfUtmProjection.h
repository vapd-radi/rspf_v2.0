#ifndef rspfUtmProjection_HEADER
#define rspfUtmProjection_HEADER
#include <rspf/projection/rspfMapProjection.h>
class RSPFDLLEXPORT rspfUtmProjection : public rspfMapProjection
{
public:
   rspfUtmProjection(const rspfEllipsoid& ellipsoid = rspfEllipsoid(),
                      const rspfGpt& origin = rspfGpt());
   
   rspfUtmProjection(const rspfEllipsoid& ellipsoid,
                      const rspfGpt& origin,
                      rspf_int32 zone,
                      char hemisphere);
   rspfUtmProjection(rspf_int32 zone);
   rspfUtmProjection(const rspfUtmProjection& src);
   ...
   void setZone(const rspfGpt& ground);
   void setZone(rspf_int32 zone);
   void setHemisphere(const rspfGpt& ground);
   void setHemisphere(char hemisphere);
   static rspf_int32 computeZone(const rspfGpt& gpt);
   /**
    * Return in decimal degrees the zone meridian.
    */
   static double computeZoneMeridian(rspf_int32 zone);
   rspf_int32 getZone()const;
...
private:
    ...
    */
   rspf_int32 Set_Transverse_Mercator_Parameters(double a,      
                                                  double f,
    ...
    */
   rspf_int32 Convert_Geodetic_To_Transverse_Mercator (double Latitude,
                                                        double Longitude,
                                                        double *Easting,
    ...
    */
   rspf_int32 Convert_Transverse_Mercator_To_Geodetic(double Easting,
                                                       double Northing,
                                                       double *Latitude,
