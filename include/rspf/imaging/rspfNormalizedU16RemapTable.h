...
// Description:
//
// Contains class declaration for  rspfNormalizedU16RemapTable.  Table for
// normalizing unsigned 16 bit data.
// 
//*******************************************************************
//  $Id: rspfNormalizedU16RemapTable.h 16034 2009-12-03 14:49:25Z dburken $

#ifndef rspfNormalizedU16RemapTable_HEADER
#define rspfNormalizedU16RemapTable_HEADER

#include <rspf/imaging/rspfNormalizedRemapTable.h>
#include <rspf/base/rspfCommon.h> /* for round */

/**
 * @class rspfNormalizedU16RemapTable
 *
 * @brief Unsigned 16 bit normalized remap table to go to/from normalized value
 * to pixel value.
 */
class RSPF_DLL rspfNormalizedU16RemapTable : public rspfNormalizedRemapTable
{
public:

   /** @brief default constructor */
   rspfNormalizedU16RemapTable();
   /** @brief virtual destructor */
   virtual ~rspfNormalizedU16RemapTable();
...
inline rspf_int32 rspfNormalizedU16RemapTable::getEntries() const
{
   return TABLE_ENTRIES;
}

inline rspf_float64 rspfNormalizedU16RemapTable::operator[](
   rspf_int32 pix) const
{
   return ( (pix < TABLE_ENTRIES) ? (pix >= 0 ? theTable[pix] : 0.0) : 1.0);  
}

inline rspf_float64 rspfNormalizedU16RemapTable::normFromPix(
   rspf_int32 pix) const
{
   return ( (pix < TABLE_ENTRIES) ? (pix >= 0 ? theTable[pix] : 0.0) : 1.0);
}

inline rspf_int32 rspfNormalizedU16RemapTable::pixFromNorm(
