...
#endif
#define A2DKBOOLDLLEXP RSPF_DLL
...
#define KBOOL_LOG 0
#if RSPF_SIZEOF_LONG_INT == 8
#  define KBOOL_INT64 1
#elif RSPF_SIZEOF_LONG_LONG == 8
#  define KBOOL_INT64 1
#else
...
#endif
#ifdef KBOOL_INT64
typedef rspf_int64 B_INT;		   // 8 bytes integer
const B_INT MAXB_INT = std::numeric_limits<rspf_int64>::max();
const B_INT MINB_INT = std::numeric_limits<rspf_int64>::min();
#else /* not defined KBOOL_INT64 */
typedef rspf_int32 B_INT;		   // 8 bytes integer
const B_INT MAXB_INT = std::numeric_limits<rspf_int32>::max();
const B_INT MINB_INT = std::numeric_limits<rspf_int32>::min();
