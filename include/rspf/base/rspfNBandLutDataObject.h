...
//
//*************************************************************************
// $Id: rspfNBandLutDataObject.h 19732 2011-06-06 22:24:54Z dburken $
#ifndef rspfNBandLutDataObject_HEADER
#define rspfNBandLutDataObject_HEADER 1
 ...
 * number_bands:  3
 * number_entries:  216
 * scalar_type:  rspf_uint8
 * type:  rspfNBandLutDataObject
 *
 *
 * NOTES:
 * 
 ...
 *   rspf_float32 precision and if you use the LUT_ENTRY_TYPE then your code will not have to change.
 *   
 * </pre>
 */
class RSPF_DLL rspfNBandLutDataObject : public rspfObject
{
public:
   
   typedef rspf_int32 LUT_ENTRY_TYPE;
    ...
    */
   rspfNBandLutDataObject(rspf_uint32 numberOfEntries=0,
                           rspf_uint32 numberOfBands=0,
                           rspfScalarType bandScalarType = RSPF_UINT8,
                           rspf_int32 nullPixelIndex=-1);
   rspfNBandLutDataObject(const rspfNBandLutDataObject& lut);
   rspfObject* dup()const
   {
      return new rspfNBandLutDataObject(*this);
   }
   void create(rspf_uint32 numberOfEntries,
               rspf_uint32 numberOfBands=3);
   const LUT_ENTRY_TYPE* operator[](rspf_uint32 idx)const
   {
      if(theLut)
      {
   ...
   LUT_ENTRY_TYPE* operator[](rspf_uint32 idx)
   {
      if(theLut)
      {
         return &theLut[idx*theNumberOfBands];
   ...
   const LUT_ENTRY_TYPE* operator[](rspf_int32 idx)const
   {
      if(theLut)
      {
         return &theLut[idx*theNumberOfBands];
   ...
   LUT_ENTRY_TYPE* operator[](rspf_int32 idx)
   {
      if(theLut)
      {
         return &theLut[idx*theNumberOfBands];
   ...
   {
      rspf_uint32 idx = int(normalizedIndex*(theNumberOfEntries-1));
    ...
    */
   void getMinMax(rspf_uint32 band,
    ...
    */
   rspf_int32 getFirstNullAlphaIndex() const;
    ...
    */
   void setNullPixelIndex(rspf_int32 idx){theNullPixelIndex = idx;}
    ...
    *         yet.
    */ 
   rspf_int32 getNullPixelIndex()const{return theNullPixelIndex;}

   /**
    * @return Return the number of bands this lut is indexed for.
    */ 
   rspf_uint32 getNumberOfBands()const{return theNumberOfBands;}
    ...
    */ 
   rspf_uint32 findIndex(rspf_int32* values) const;
    ...
    * @return Returns the nearest index to the passed in value.  
    */ 
   rspf_uint32 findIndex(rspf_int32* values, rspf_uint32 size) const;
    ...
    */ 
   const rspfNBandLutDataObject& operator =(const rspfNBandLutDataObject& lut);
   ...
   std::vector<rspfString> getEntryLabels(rspf_uint32 band);

   void setEntryLables(rspf_uint32 band, std::vector<rspfString> entryLabels);
...
protected:
   virtual ~rspfNBandLutDataObject();

   LUT_ENTRY_TYPE *theLut;
   rspf_uint32    theNumberOfEntries;
   rspf_uint32    theNumberOfBands;
   rspfScalarType theBandScalarType;
   rspf_int32     theNullPixelIndex;
   std::map<rspf_uint32, std::vector<rspfString> >  m_entryLabels;
