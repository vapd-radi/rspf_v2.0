...
#ifndef rspfPolynom_HEADER
#define rspfPolynom_HEADER
...
public:
   ...
   rspfPolynom(const T& epsilon = 0) //! must supply epsilon value, default 0
      : theEpsilon(epsilon)
   {}

   rspfPolynom(const rspfPolynom& p) : 
   ...
   const rspfPolynom& operator=(const rspfPolynom< T, DIM >& pt)
   {
      if (this != &pt)
      {
    ...
    */
   bool operator==(const rspfPolynom& pt)const
   {
    ...
    */
   void pdiff(int pdim, rspfPolynom& result)const
   {
      result.nullify();
      int ord;
      //loop on monoms
    ...
    */
   const rspfPolynom& operator*=(const T& k)
   {
      //loop on monoms
      for ( typename MONOM_MAP::const_iterator it = getMonoms().begin(); it != getMonoms().end() ; ++it )
      {
    ...
    */
   rspfPolynom operator+(const rspfPolynom& p)const
   {
      rspfPolynom< T , DIM > sum(*this);
      return (sum+=p);
   }
   rspfPolynom operator-(const rspfPolynom& p)const
   {
      rspfPolynom< T , DIM > diff(*this);
      return (diff-=p);
   }

   const rspfPolynom& operator+=(const rspfPolynom& p)
   {
      typename MONOM_MAP::const_iterator it;
      //loop on p monoms
   ...
   rspfPolynom operator*(const rspfPolynom& p)const
   {
      ...
      rspfPolynom< T , DIM > prod(getEpsilon());
      T coeff;
      //loop on p monoms
      for ( typename MONOM_MAP::const_iterator it = getMonoms().begin(); it != getMonoms().end() ; ++it )
      {      
   ...
   std::istream& import(std::istream& is) //! note that it can only import for the template type T and dimesnion DIM
   {
      ...
      if (!is) 
      {
         std::cerr<<"rspfPolynom::import ERROR, cannot read left bracket ["<<std::endl;
         return is;
      }
      if (tempChar != '[')
      {
         std::cerr<<"rspfPolynom::import ERROR, missing left bracket ["<<std::endl;
         return is;
      }
      ...
      if (!is)
      {
         std::cerr<<"rspfPolynom::import ERROR, cannot read after left bracket ["<<std::endl;
         return is;
      }     
      if (is.gcount() >= BUFSIZE-1)
      {
         std::cerr<<"rspfPolynom::import ERROR, cannot find right bracket ] after "<<BUFSIZE-1<<"characters"<<std::endl;
         return is;
      }
      tempString = buffer; //no more brackets

      //split string using semicolons
      std::vector< rspfString > subparts = tempString.explode(";");
      //loop on subparts
      for (typename std::vector< rspfString >::const_iterator it=subparts.begin() ; it!=subparts.end() ; ++it )
      {
         rspfString sp = it->trim();
         //check for epsilon
         rspfString aft_eps=sp.after("eps=");
         if (aft_eps.size()>0)
         {
            //get epsilon value
            aft_eps.trim();
            ...
            rspfString scalpart=sp.before("(");
            if (scalpart.size() < sp.size())
            {
              T coeff = static_cast< T >(scalpart.toDouble());
              rspfString expopart = ((sp.before(")")).after("(")).trim();
              if (expopart.size() == 0)
              {
                 std::cerr<<"rspfPolynom::import ERROR, cannot find ) in monom or empty monom"<<std::endl;
                 return is;
              }
              std::vector< rspfString > vexpo = expopart.explode(",");
              if (vexpo.size() != DIM)
              {
                 std::cerr<<"rspfPolynom::import ERROR, bad number of exponents in monom (need "<<DIM<<"): "<<vexpo.size()<<std::endl;
                 return is;
              }
              //store all exponents
              EXP_TUPLE expt(DIM);
              int d;
              std::vector< rspfString >::const_iterator eit;
              ...
              {
                 std::cerr<<"rspfPolynom::import ERROR, duplicate exponent tuple in polynom"<<std::endl;
                 return is;
              }
              theMonoms[expt] = coeff;

            } else {
               std::cerr<<"rspfPolynom::import ERROR, cannot find left parenthesis ( in monom "<<std::endl;
               return is;
            }
         }
      }
 ...
 */
 EXPT_SET builExpSet(const EXP_TUPLE& orders)const
 {
    ...
    if (orders.size() != DIM)
    {
       std::cerr<<"rspfPolynom::import ERROR bad dimension for parameter, need "<<DIM<<" elements"<<std::endl;
       return eset;
    }
       ...
       T*                             prms = NULL)
{
   ...
   {
      std::cerr<<"rspfPolynom::LMSfit ERROR observation input/output must have the same size"<<std::endl;
      return false;
   }
   if (nobs<=0)
   {
      std::cerr<<"rspfPolynom::LMSfit ERROR observation count is zero"<<std::endl;
      return false;
   }
   int ncoeff = (int)expset.size();  
   if (ncoeff<=0)
   {
      std::cerr<<"rspfPolynom::LMSfit ERROR exponent count is zero"<<std::endl;
      return false;
   }
   
   //construct LMS linear system (using RSPF matrices)
   // M.k = v
   // M : monom matrix
...
protected:
   ...
   /** 
    * invert stolen from rspfRpcSolver
    */
   NEWMAT::Matrix 
...
operator<<(std::ostream& os, const rspfPolynom<T,DIM>& pt)
{
   return pt.print(os);
}

template < class T, int DIM > std::istream&
operator>>(std::istream& is, rspfPolynom<T,DIM>& pt)
{
   return pt.import(is);
}
