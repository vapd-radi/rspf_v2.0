...
//
//********************************************************************
// $Id: rspfDemGrid.h 9305 2006-07-19 17:47:04Z dburken $

#ifndef rspfDemGrid_HEADER
#define rspfDemGrid_HEADER
...
public:
    ...
    */
   rspfDemGrid(rspf_float32 missingDataValue = -32768.0);
   ...
   // Accessors
   rspfDemHeader const& getHeader() const;
    ...
    */
   rspf_float32 getElevation(long x, long y) const;

   /*!
    *  Returns the value used to indicate missing data.
    */
   rspf_float32 getMissingDataValue() const;
...
private:
   ...
   rspf_float32   _missDataVal;
   rspfDemHeader  _header;
   long            _width;
   long            _height;
   rspf_float32*  _grid;
   bool            _firstTime; // Whether or not we've started reading the DEM.
   long            _curProfile;
   std::vector<rspfDemProfile> _profiles;  // Used by fillUTM()
   double _northwest_x, _northwest_y;

   void setElevation(long x, long y, rspf_float32 val);
   long fillGeographic(std::istream& dem, bool incrementalRead);
   long fillUTM(std::istream& dem, bool incrementalRead);
};

inline void rspfDemGrid::setElevation(long x, long y, rspf_float32 val)
{
   if (_grid == 0)
      return;
...
#endif  // #ifndef rspfDemGrid_HEADER
