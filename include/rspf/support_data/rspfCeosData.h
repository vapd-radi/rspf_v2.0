...
//*******************************************************************
//  $Id:$
#ifndef rspfCeosData_HEADER
#define rspfCeosData_HEADER
...
{
   rspf_int32   rec_seq;               // Record sequence number (B4) 
   rspf_uint8   rec_sub1;              // First record sub-type code (B1) 
   rspf_uint8   rec_type;              // Record type code (B1) 
   rspf_uint8   rec_sub2;              // Second record sub-type code (B1) 
   rspf_uint8   rec_sub3;              // Third record sub-type code (B1) 
   rspf_int32   length;                // Length of this record, bytes (B4) 
};
...
{
   rspf_int32  acq_year;              // Acquisition year (B4) 
   rspf_int32  acq_day;               // Acquisition day of year (B4) 
   rspf_int32  acq_msec;              // Acquisition msecs of day (B4) 
};

struct sdr_prefix_rec            // Signal data prefix data                    
{
   rspf_int32  line_num;              // Image data line number (B4) 
   rspf_int32  rec_num;               // Image data record index (B4) 
   rspf_int32  n_left_pixel;          // Left fill pixel count (B4) 
   rspf_int32  n_data_pixel;          // Data pixel count (B4) 
   rspf_int32  n_right_pixel;         // Right fill pixel count. (B4) 
   rspf_int32  sensor_updf;           // Sensor parameter update flag (B4) 
   struct acq_date_rec acq_date; // Acquisition date                           
   rspf_int16  sar_chan_ind;          // SAR channel indicator (B2) 
   rspf_int16  sar_chan_code;         // SAR channel code (B2) 
   rspf_int16  tran_polar;            // Transmitted polarization (B2) 
   rspf_int16  recv_polar;            // Received polarization (B2) 
   rspf_int32  prf;                   // Pulse repetition frequency, Hz (B4) 
   rspf_uint8   spare1[4];             // Unused (B4) 
   rspf_int16  obrc;                  // On-board range compressed flag (B2) 
   rspf_int16  pulse_type;            // Pulse type designator (B2) 
   rspf_int32  chp_len;               // Chirp length, ns (B4) 
   rspf_int32  chp_coef1;             // Chirp constant coefficient,Hz (B4) 
   rspf_int32  chp_coef2;             // Chirp linear coefficient,Hz/usec (B4) 
   rspf_int32  chp_coef3;             // Chirp quadratic coefficient,Hz/usec^2 (B4) 
   rspf_uint8   spare2[2][4];          // Spare (2B4) 
   rspf_int32  recv_gain;             // Receiver gain (B4) 
   rspf_int32  nt_line;               // Nought line flag (B4) 
   rspf_int32  ele_nadir;             // Elec. nadir angle, 10**-6 deg (B4) 
   rspf_int32  mec_nadir;             // Mech. nadir angle, 10**-6 deg (B4) 
   rspf_int32  ele_squint;            // Elec. squint angle, 10**-6 deg (B4) 
   rspf_int32  mec_squint;            // Mech. squint angle, 10**-6 deg (B4) 
   rspf_int32  sr_first;              // First sample slant range, m (B4) 
   rspf_int32  dr_window;             // Data record window time, ns (B4) 
   rspf_uint8   spare3[4];             // Spare (B4) 
   rspf_int32  plat_updf;             // Platform position update flag (B4) 
   rspf_int32  plat_lat;              // Platform latitude, 10**-6 deg (B4) 
   rspf_int32  plat_long;             // Platform longitude, 10**-6 deg (B4) 
   rspf_int32  plat_alt;              // Platform altitude, m (B4) 
   rspf_int32  plat_speed;            // Platform speed, cm/s (B4) 
   rspf_int32  plat_vel[3];           // Platform velocity, cm/s (3B4) 
   rspf_int32  plat_acc[3];           // Platform acceleration, cm/s (3B4) 
   rspf_int32  plat_track;            // Platform track, 10**-6 deg (B4) 
   rspf_int32  plat_head;             // Platform heading, 10**-6 deg (B4) 
   rspf_int32  plat_pitch;            // Platform pitch, 10**-6 deg (B4) 
   rspf_int32  plat_roll;             // Platform roll, 10**-6 deg (B4) 
   rspf_int32  plat_yaw;              // Platform yaw, 10**-6 deg (B4) 
};
...
struct sdr_data_rec              // Signal data record                  
{
   ...
   rspf_uint8   sdr_data;              // SAR signal data (jBk) 
};
...
struct pdr_prefix_rec            // Processed data prefix data
{
   ...
   rspf_int32  prf;                   // Pulse repetition frequency, Hz (B4) 
   rspf_uint8   spare1[4];             // Unused (B4) 
   rspf_int32  sr_first;              // Slant range to first pixel, m (B4) 
   rspf_int32  sr_mid;                // Slant range to mid-pixel, m (B4) 
   rspf_int32  sr_last;               // Slant range to last pixel, m (B4) 
   rspf_int32  fdc_first;             // First pixel Doppler centroid, Hz (B4) 
   rspf_int32  fdc_mid;               // Mid-pixel Doppler centroid, Hz (B4) 
   rspf_int32  fdc_last;              // Last pixel Doppler centroid, Hz (B4) 
   rspf_int32  ka_first;              // First pixel azimuth FM rate, Hz (B4) 
   rspf_int32  ka_mid;                // Mid-pixel azimuth FM rate, Hz (B4) 
   rspf_int32  ka_last;               // Last pixel azimuth  FM rate, Hz (B4) 
   rspf_int32  nadir_ang;             // Nadir look angle, 10**-6 deg (B4) 
   rspf_int32  squint_ang;            // Azimuth squint angle, 10**-6 deg (B4) 
   rspf_int32  null_f;                // Null line flag - 1 for NULL line, 0 for otherwise (B4) 
   rspf_uint8   spare2[4][4];          // Unused (4B4) 
   rspf_int32  geo_updf;              // Geographic parameter update flag (B4) 
   rspf_int32  lat_first;             // First pixel latitude, millionths of deg (B4)
   rspf_int32  lat_mid;               // Mid-pixel latitude (B4) 
   rspf_int32  lat_last;              // Last pixel latitude (B4) 
   rspf_int32  long_first;            // First pixel longitude, millionths of deg (B4) 
   rspf_int32  long_mid;              // Mid pixel longitue (B4) 
   rspf_int32  long_last;             // Last pixel longitude (B4) 
   rspf_int32  north_first;           // Northing of first pixel, m (B4) 
   rspf_uint8   spare3[4];             // Unused (B4) 
   rspf_int32  north_last;            // Northing of last pixel, m (B4) 
   rspf_int32  east_first;            // Easting of first pixel, m (B4) 
   rspf_uint8   spare4[4];             // Spare (B4) 
   rspf_int32  east_last;             // Easting of last pixel, m (B4) 
   rspf_int32  heading;               // Line heading, millionths of deg (B4) 
   rspf_uint8   spare5[8];             // Unused (B8) 
};
...
class rspfCeosData : public rspfReferenced
{
public:
   ...
   rspfCeosData(const rspfFilename& volDirFile,
            const rspfFilename& leaderFile,
            const rspfFilename& imageFile,
            const rspfFilename& trailerFile);

   rspfCeosData(const rspfFilename& imageDir);

   bool errorStatus()   const { return theErrorStatus; }

   const rspfFilename& volDirFile()    const { return theVolDirFileName; }
   const rspfFilename& leaderFile()    const { return theLeaderFileName; }
   const rspfFilename& imageFile()     const { return theImageFileName; }
   const rspfFilename& trailerFile()   const { return theTrailerFileName; }
...
private:
   ...
   rspfFilename          theVolDirFileName;
   rspfFilename          theLeaderFileName;
   rspfFilename          theImageFileName;
   rspfFilename          theTrailerFileName;
