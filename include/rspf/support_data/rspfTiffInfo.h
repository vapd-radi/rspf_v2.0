...
// 
//----------------------------------------------------------------------------
// $Id: rspfTiffInfo.h 2673 2011-06-06 14:57:24Z david.burken $
#ifndef rspfTiffInfo_HEADER
#define rspfTiffInfo_HEADER 1
 ...
 */
class RSPF_DLL rspfTiffInfo : public rspfInfoBase
{
public:
    ...
    */
   bool getImageGeometry(rspfKeywordlist& geomKwl,
    ...
    * rspfProjectionFactoryRegistry::createProjection(kwl).
    *
    ...
    */
   void getImageGeometry(rspf_uint64   geoKeyLength,
...
private:
    ...
    * @param entryIndex Entry to get geometry from. 
    */
   bool getImageGeometry(const rspfKeywordlist& gtiffKwl,
                         rspfKeywordlist& geomKwl,
                         rspf_uint32 entryIndex) const;
   
   /** Initializes s reference.  Does byte swapping as needed. */
   void readShort(rspf_uint16& s, std::istream& str) const;
   
   /** Initializes l reference.  Does byte swapping as needed. */
   void readLong(rspf_uint32& l, std::istream& str) const;
   
   /** Initializes l reference.  Does byte swapping as needed. */
   void readLongLong(rspf_uint64& l, std::istream& str) const;
    ...
    * @return true if stream is good, false if not.
    */
   bool getValue(rspf_uint64& val, std::istream& str, WordType type,
                 rspf_uint16 version) const;
   
   rspf_uint64 getArraySizeInBytes(rspf_uint64 length,
                                    rspf_uint16 type) const;
   
   /**
    * @return The byte size of each tiff type or 0 if unhandled.
    */
   rspf_uint16 getTypeByteSize(rspf_uint16 type) const;
   
   /**
    * Eats the value field.  Either 4 or 8 bytes depending on the version.
    */
   void eatValue(std::istream& str, rspf_uint16 version) const;
   
   void swapBytes(rspf_uint8* v, rspf_uint16 type, rspf_uint64 count) const;
   ...
   void getDirPrefix(rspf_int32 dirIndex, std::string& prefix) const;
    ...
    * @return true on success, false on error.
    */
   bool getPixelScale(const rspfString& gtiffPrefix,
    ...
    * @return true on success, false on error.
    */
   bool getTiePoint(const rspfString& gtiffPrefix,
                    const rspfKeywordlist& gtiffKwl,
    ...
    * @return true on success, false on error.
    */
   bool getModelTransform(const rspfString& gtiffPrefix,
                          const rspfKeywordlist& gtiffKwl,
    ...
    * will return false.
    */
   bool  getFloats(const rspfString& gtiffPrefix,
    ...
    * @param floats Array to stuff.  Will have size of zero on failure.
    */
   bool getFloats(const rspfString& line,
                  std::vector<rspf_float64>& floats) const;
    ...
    * @return true on success, false on error.
    */
   bool getDatumCode(const rspfString& gtiffPrefix,
                     const rspfKeywordlist& gtiffKwl,
    ...
    * @return true on success, false on error.
    */
   bool getDatumCode(rspf_int32 code, rspfString& datumCode) const;
    ...
    * @return true on success, false on error.
    */
   bool getPcsCode(const rspfString& gtiffPrefix,
                   const rspfKeywordlist& gtiffKwl,
    ...
    * @return true on success, false on error.
    */
   bool getUnits(const rspfString& gtiffPrefix,
                 const rspfKeywordlist& gtiffKwl,
    ...
    * @return true on success, false on error.
    */
   bool getLinearUnits(const rspfString& gtiffPrefix,
                       const rspfKeywordlist& gtiffKwl,
    ...
    * @return true on success, false on error.
    */
   bool getVerticalUnits(const rspfString& gtiffPrefix,
                         const rspfKeywordlist& gtiffKwl,
    ...
    * @return true on success, false on error.
    */
   bool getAngularUnits(const rspfString& gtiffPrefix,
                        const rspfKeywordlist& gtiffKwl,
                        rspfString& units) const;
    ...
    * @return true on success, false on error.
    */   
   bool getPixelType(const rspfString& gtiffPrefix,
                     const rspfKeywordlist& gtiffKwl,
    ...
    * @return true on success, false on error.
    */   
   bool getModelType(const rspfString& gtiffPrefix,
                     const rspfKeywordlist& gtiffKwl,
                     rspfString& modeType) const;

   /**
    * @brief Gets the rspfProjection from keyword list as a string.
    * @param gtiffPrefix Prefix for gtiffKwl.
    * @param gtiffKwl The keyword list with all tiff keywords.
    * @param rspfProj The string to initialize with pixel type.
    * @return true on success, false on error.
    */   
   bool getOssimProjectionName(const rspfString& gtiffPrefix,
                               const rspfKeywordlist& gtiffKwl,
    ...
    */   
   rspf_uint32 getLines(const rspfString& gtiffPrefix,
    ...
    * @return Number of lines or 0 if lines not found.
    */   
   rspf_uint32 getSamples(const rspfString& gtiffPrefix,
                           const rspfKeywordlist& gtiffKwl) const;
    ...
    */   
   bool getStdParallelOne(const rspfString& gtiffPrefix,
                          const rspfKeywordlist& gtiffKwl,
    ...
    * @param value Initialized with result.
    */
   bool getStdParallelTwo(const rspfString& gtiffPrefix,
                          const rspfKeywordlist& gtiffKwl,
                          rspfString& value) const;
    ...
    * @param value Initialized with result.
    */
   bool getFalseEasting(const rspfString& gtiffPrefix,
                        const rspfKeywordlist& gtiffKwl,
                        rspfString& value) const;
    ...
    * @param value Initialized with result.
    */
   bool getFalseNorthing(const rspfString& gtiffPrefix,
                         const rspfKeywordlist& gtiffKwl,
                         rspfString& value) const;
    ...
    */
   bool getFalseEastingNorthing(const rspfString& gtiffPrefix,
                                const rspfKeywordlist& gtiffKwl,
    ...
    * @param value Initialized with result.
    */
   bool getScaleFactor(const rspfString& gtiffPrefix,
                       const rspfKeywordlist& gtiffKwl,
    ...
    * @param value Initialized with result.
    */
   bool getOriginLat(const rspfString& gtiffPrefix,
                     const rspfKeywordlist& gtiffKwl,
                     rspf_float64& value) const;
    ...
    * @param value Initialized with result.
    */
   bool getCentralMeridian(const rspfString& gtiffPrefix,
                           const rspfKeywordlist& gtiffKwl,
                           rspf_float64& value) const;
    ...
    */
   void getTieSets(const std::vector<rspf_float64>& ties,
                   rspf_uint32 width,
                   rspf_uint32 height,
    ...
    */
   bool hasOneBasedTiePoints(const std::vector<rspf_float64>& ties,
                             rspf_uint32 width,
                             rspf_uint32 height) const;
   
   rspfFilename          theFile;
   mutable rspfEndian*   theEndian;
};

#endif /* End of "#ifndef rspfTiffInfo_HEADER" */
