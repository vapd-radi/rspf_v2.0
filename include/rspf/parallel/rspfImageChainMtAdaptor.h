...
//*************************************************************************************************
// $Id$
#ifndef rspfImageChainMtAdaptor_HEADER
#define rspfImageChainMtAdaptor_HEADER
...
//*************************************************************************************************
class RSPFDLLEXPORT rspfImageChainMtAdaptor : public rspfImageChain
{
   friend class rspfMultiThreadSequencer; //### FOR DEBUG ONLY

public:
   rspfImageChainMtAdaptor();
   ...
   //! 
   rspfImageChainMtAdaptor(rspfImageChain* original, rspf_uint32 num_threads=0);
   ~rspfImageChainMtAdaptor();
   ...
   void setOriginalChain(rspfImageChain* original);
   ...
   rspfImageSource* getClone(rspf_uint32 index);

   //! Returns the number of clones available. The original chain always counts as the first clone.
   rspf_uint32 getNumberOfClones() const { return (rspf_uint32) m_clones.size(); }
   ...
   virtual rspfConnectableObject::ConnectableObjectList& imageChainList();
   virtual const rspfConnectableObject::ConnectableObjectList& imageChainList() const;
...
protected:
   ...
   bool connectSharedHandlers(rspf_uint32 index);

   //! This is the adaptee image chain.
   rspfRefPtr<rspfImageChain> m_adaptedChain;
   ...
   std::vector< rspfRefPtr<rspfImageSource> > m_clones;
   ...
   std::vector< rspfRefPtr<rspfConnectableContainer> > m_chainContainers;
   ...
   typedef std::vector<rspfRefPtr<rspfImageHandlerMtAdaptor> > SharedHandlerList;
